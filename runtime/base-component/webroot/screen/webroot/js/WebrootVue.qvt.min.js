...
moqui.handleAjaxError=function(a,b,c,d){if(!d){if("blob"===a.responseType){var e=new FileReader;e.onload=function(g){moqui.handleAjaxError(a,b,c,g.target.result)};e.readAsText(a.response);return}d=a.responseText}try{e=JSON.parse(d)}catch(g){}console.warn("ajax "+b+" ("+a.status+"), message "+c);var f=!1;401===a.status?f=moqui.notifyMessages(null,"No user authenticated"):e&&moqui.isPlainObject(e)?f=moqui.notifyMessages(e.messageInfos,e.errors,e.validationErrors):d&&moqui.isString(d)&&d.length&&(f=
moqui.notifyMessages(d));401===a.status?moqui.webrootVue&&moqui.webrootVue.reLoginCheckShow?moqui.webrootVue.reLoginCheckShow():window.location.reload(!0):0===a.status?0>c.indexOf("abort")&&(moqui.webrootVue.$q.notify($.extend({},moqui.notifyOptsError,{message:"Could not connect to server"})),moqui.webrootVue.addNotify("Could not connect to server","negative")):(moqui.webrootVue&&moqui.webrootVue.getCsrfToken&&moqui.webrootVue.getCsrfToken(a),f||(e="Error: "+c+" ("+b+")",moqui.webrootVue.$q.notify($.extend({},
moqui.notifyOptsError,{message:e})),moqui.webrootVue.addNotify(e,"negative")))};moqui.notifyGrowl=function(a){a&&(moqui.webrootVue.$q.notify($.extend({},moqui.notifyOptsInfo,{type:a.type,message:a.title,actions:[{label:"View",color:"white",handler:function(){moqui.webrootVue.setUrl(a.link)}}]})),moqui.webrootVue.addNotify(a.title,a.type,a.link,a.icon))};moqui.componentCache=new moqui.LruMap(50);
...
moqui.loadComponent=function(a,b,c){var d=moqui.urlExtensions.js,e=moqui.urlExtensions.vue,f=moqui.urlExtensions.vuet;if("string"===typeof a){var g=a.indexOf("?");if(0<g){var h=a.slice(0,g);var l=a.slice(g+1)}else h=a}else{h=a.path;var n=a.extraPath;l=a.search;var m=a.bodyParameters}console.log("Load Component "+JSON.stringify(a)+" Window Width "+window.innerWidth+" Quasar Platform: "+JSON.stringify(Quasar.Platform.is)+" search: "+l);!(600>=window.innerWidth||Quasar.Platform.is.mobile)||l&&-1!==l.indexOf("_uiType")||
(l=(l||"")+"\x26_uiType\x3dmobile");if(g=moqui.componentCache.get(h))console.info("found cached component for path "+h+": "+JSON.stringify(g)),b(g);else{var p=h;a.renderModes&&0<=a.renderModes.indexOf(e)&&(p+="."+e);if(p.slice(-e.length)===e)return console.info("loadComponent vue "+p+(c?" id "+c:"")),a={type:"GET",url:p,error:moqui.handleLoadError,success:function(k,t,r){205===r.status?(k=r.getResponseHeader("X-Redirect-To"),moqui.webrootVue.setUrl(k)):(k||b(moqui.NotFound),t=0<=r.getResponseHeader("Cache-Control").indexOf("max-age"),
moqui.isString(k)&&0<k.length?(k=httpVueLoader.parse(k,p.substr(0,p.lastIndexOf("/")+1)),t&&moqui.componentCache.put(h,k),b(k)):b(moqui.NotFound))}},m&&!$.isEmptyObject(m)&&(a.type="POST",a.data=m),$.ajax(a);var q=h.slice(-d.length)===d;!q&&a.renderModes&&0<=a.renderModes.indexOf(d)&&(p+="."+d,q=!0);q||(p+="."+f);n&&0<n.length&&(p+="/"+n);l&&0<l.length&&(p+="?"+l);console.info("loadComponent "+p+(c?" id "+c:""));a={type:"GET",url:p,error:moqui.handleLoadError,success:function(k,t,r){if(205===r.status)k=
r.getResponseHeader("X-Redirect-To"),moqui.webrootVue.setUrl(k);else{k||b(moqui.NotFound);var v=(t=r.getResponseHeader("Cache-Control"))&&0<=t.indexOf("max-age");if(moqui.isString(k)&&0<k.length)if(q||"define("===k.slice(0,7)){console.info("loaded JS from "+p+(c?" id "+c:""));var u=eval(k);u.template?(v&&moqui.componentCache.put(h,u),b(u)):(k=(h.slice(-d.length)===d?h.slice(0,-d.length):h)+"."+f,$.ajax({type:"GET",url:k,error:moqui.handleLoadError,success:function(w){u.template=w;v&&moqui.componentCache.put(h,
u);b(u)}}))}else k=k.replace(/<script/g,"\x3cm-script").replace(/<\/script>/g,"\x3c/m-script\x3e").replace(/<link/g,"\x3cm-stylesheet"),console.info("loaded HTML template from "+p+(c?" id "+c:"")),k={template:"\x3cdiv"+(c&&0<c.length?' id\x3d"'+c+'"':"")+"\x3e"+k+"\x3c/div\x3e"},v&&moqui.componentCache.put(h,k),b(k);else moqui.isPlainObject(k)?k.screenUrl&&k.screenUrl.length?moqui.webrootVue.setUrl(k.screenUrl):k.redirectUrl&&k.redirectUrl.length&&window.location.replace(k.redirectUrl):b(moqui.NotFound)}}};
m&&!$.isEmptyObject(m)&&(a.type="POST",a.data=m);return $.ajax(a)}};moqui.NotFound=Vue.extend({template:'\x3cdiv id\x3d"current-page-root"\x3e\x3ch4\x3eScreen not found at {{this.$root.currentPath}}\x3c/h4\x3e\x3c/div\x3e'});moqui.EmptyComponent=Vue.extend({template:'\x3cdiv id\x3d"current-page-root"\x3e\x3cdiv class\x3d"spinner"\x3e\x3cdiv\x3e\x26nbsp;\x3c/div\x3e\x3c/div\x3e\x3c/div\x3e'});
...
function(c){b.curComponent=c},this.id)}else this.curComponent=moqui.EmptyComponent}},mounted:function(){this.$root.addContainer(this.id,this);this.curUrl=this.url}});
...
methods:{reload:function(){this.isShown&&(this.isShown=!0)},load:function(a){this.curUrl=a},hide:function(){this.isShown=!1}},watch:{curUrl:function(a){if(a&&0!==a.length){var b=this;if(moqui.isPlainObject(this.dynamicParams)){var c="";$.each(this.dynamicParams,function(d,e){(e=$("#"+e).val())&&e.length&&(c=c+(0<c.length?"\x26":"")+d+"\x3d"+e)});c.length&&(a=a+(0<a.indexOf("?")?"\x26":"?")+c)}moqui.loadComponent(a,function(d){d.mounted=function(){this.$nextTick(function(){b.$refs.dialog.focusFirst()})};
b.curComponent=d},this.id)}else this.curComponent=moqui.EmptyComponent},isShown:function(a){this.curUrl=a?this.url:""}},mounted:function(){this.$root.addContainer(this.id,this);this.openDialog&&(this.isShown=!0)}});
...
top:Object},data:function(){return{open:!1}},computed:{isFolder:function(){var a=this.model.children;return a?moqui.isArray(a)?0<a.length:!0:!1},hasChildren:function(){var a=this.model.children;return moqui.isArray(a)&&0<a.length},selected:function(){return this.top.currentPath===this.model.id}},watch:{open:function(a){if(a){var b=this.model.children;a=this.top.items;if(this.open&&b&&moqui.isBoolean(b)&&moqui.isString(a)){b=this.model.li_attr;b=$.extend({moquiSessionToken:this.$root.moquiSessionToken,
treeNodeId:this.model.id,treeNodeName:b&&b.treeNodeName?b.treeNodeName:"",treeOpenPath:this.top.currentPath},this.top.parameters);var c=this;$.ajax({type:"POST",dataType:"json",url:a,headers:{Accept:"application/json"},data:b,error:moqui.handleAjaxError,success:function(d){c.model.children=d}})}}}},methods:{toggle:function(){this.isFolder&&(this.open=!this.open)},setSelected:function(){this.top.currentPath=this.model.id;this.open=!0}},mounted:function(){this.model.state&&this.model.state.opened&&
(this.open=!0)}});
Vue.component("m-editable",{name:"mEditable",props:{id:{type:String,required:!0},labelType:{type:String,"default":"span"},labelValue:{type:String,required:!0},url:{type:String,required:!0},urlParameters:{type:Object,"default":{}},parameterName:{type:String,"default":"value"},widgetType:{type:String,"default":"textarea"},loadUrl:String,loadParameters:Object,indicator:{type:String,"default":"Saving"},tooltip:{type:String,"default":"Click to edit"},cancel:{type:String,"default":"Cancel"},submit:{type:String,
"default":"Save"}},mounted:function(){$.extend({moquiSessionToken:this.$root.moquiSessionToken,parameterName:this.parameterName},this.urlParameters)},render:function(a){return a(this.labelType,{attrs:{id:this.id,"class":"editable-label"},domProps:{innerHTML:this.labelValue}})}});
moqui.checkboxSetMixin={props:{checkboxCount:{type:Number,"default":100},checkboxParameter:String,checkboxListMode:Boolean,checkboxValues:Array},data:function(){for(var a=[],b=0;b<this.checkboxCount;b++)a[b]=!1;return{checkboxAllState:!1,checkboxLastIndex:null,checkboxLastChange:null,checkboxStates:a}},methods:{setCheckboxAllState:function(a){this.checkboxAllState=a;for(var b=this.checkboxStates.length,c=0;c<b;c++)this.checkboxStates[c]=a},clickCheckbox:function(a,b){console.warn("clickCheckbox idx "+
b+" shift "+a.shiftKey+" lastIdx "+this.checkboxLastIndex+" lastChange "+this.checkboxLastChange);if(a.shiftKey&&null!=this.checkboxLastIndex&&this.checkboxLastIndex!==b){a=b>this.checkboxLastIndex?1:-1;for(var c=this.checkboxLastChange,d=this.checkboxLastIndex;d!==b;d+=a)this.checkboxStates[d]=c}this.checkboxLastIndex=b;this.checkboxLastChange=!this.checkboxStates[b].checked},getCheckboxValueArray:function(){if(!this.checkboxValues)return[];for(var a=[],b=this.checkboxStates.length,c=0;c<b;c++)this.checkboxStates[c]&&
this.checkboxValues[c]&&a.push(this.checkboxValues[c]);return a},addCheckboxParameters:function(a,b,c){b=b||this.checkboxParameter;var d=null!==c&&void 0!==c&&c?c:this.checkboxListMode;c=this.getCheckboxValueArray();if(!c.length)return!1;if(d)a.set(b,c.join(","));else{for(d=0;d<c.length;d++)a.set(b+"_"+d,c[d]);a.set("_isMulti","true")}return!0}},watch:{checkboxStates:{deep:!0,handler:function(a){for(var b=!0,c=0;c<a.length&&(a[c]||(b=!1),b);c++);this.checkboxAllState=b}}}};
Vue.component("m-checkbox-set",{name:"mCheckboxSet",mixins:[moqui.checkboxSetMixin],template:'\x3cspan class\x3d"checkbox-set"\x3e\x3cslot :checkboxAllState\x3d"checkboxAllState" :setCheckboxAllState\x3d"setCheckboxAllState" :checkboxStates\x3d"checkboxStates" :clickCheckbox\x3d"clickCheckbox" :addCheckboxParameters\x3d"addCheckboxParameters"\x3e\x3c/slot\x3e\x3c/span\x3e'});
Vue.component("m-form",{name:"mForm",mixins:[moqui.checkboxSetMixin],props:{fieldsInitial:Object,action:{type:String,required:!0},method:{type:String,"default":"POST"},submitMessage:String,submitReloadId:String,submitHideId:String,focusField:String,noValidate:Boolean,excludeEmptyFields:Boolean,parentCheckboxSet:Object},data:function(){return{fields:Object.assign({},this.fieldsInitial),fieldsOriginal:Object.assign({},this.fieldsInitial),buttonClicked:null}},template:'\x3cq-form ref\x3d"qForm" @submit.prevent\x3d"submitForm" @reset.prevent\x3d"resetForm" autocapitalize\x3d"off" autocomplete\x3d"off"\x3e\x3cslot :fields\x3d"fields" :checkboxAllState\x3d"checkboxAllState" :setCheckboxAllState\x3d"setCheckboxAllState" :checkboxStates\x3d"checkboxStates" :clickCheckbox\x3d"clickCheckbox" :addCheckboxParameters\x3d"addCheckboxParameters" :blurSubmitForm\x3d"blurSubmitForm" :hasFieldsChanged\x3d"hasFieldsChanged" :fieldChanged\x3d"fieldChanged"\x3e\x3c/slot\x3e\x3c/q-form\x3e',
methods:{submitForm:function(){if(this.noValidate)this.submitGo();else{$(this.$el);var a=this;this.$refs.qForm.validate().then(function(b){b&&a.submitGo()})}},resetForm:function(){this.fields=Object.assign({},this.fieldsOriginal)},blurSubmitForm:function(a){this.hasFieldsChanged&&this.submitForm();return!0},submitGo:function(){var a=this,b=$(this.$el),c=null,d=null,e=$(this.buttonClicked||document.activeElement);e.length&&b.has(e)&&e.is('button[type\x3d"submit"], input[type\x3d"submit"], input[type\x3d"image"]')&&
(e.is("[name]")&&(c=e.attr("name"),d=e.val()),e.prop("disabled",!0),setTimeout(function(){e.prop("disabled",!1)},3E3));var f=Object.keys(this.fields).length?new FormData:new FormData(this.$refs.qForm.$el);$.each(this.fields,function(m,p){moqui.isArray(p)?p.forEach(function(q){f.append(m,q)}):f.set(m,p||"")});b=[];for(var g=f.entries()[Symbol.iterator]();;){var h=g.next();if(h.done)break;var l=h.value;h=l[0];l=l[1];moqui.isString(l)&&l.startsWith("__")&&f.set(h,"");!this.excludeEmptyFields||l&&l.length||
b.push(h)}for(g=0;g<b.length;g++)f["delete"](b[g]);f.set("moquiSessionToken",this.$root.moquiSessionToken);c&&f.set(c,d);this.parentCheckboxSet&&this.parentCheckboxSet.addCheckboxParameters&&this.parentCheckboxSet.addCheckboxParameters(f);this.$root.loading++;var n=new XMLHttpRequest;n.open(this.method,this.$root.appRootPath+this.action,!0);n.responseType="blob";n.withCredentials=!0;n.onload=function(){if(200===this.status){a.$root.loading--;var m=n.getResponseHeader("Content-Disposition");if(!m||
-1===m.indexOf("attachment")&&-1===m.indexOf("inline")){var p=new FileReader;p.onload=function(r){r=r.target.result;try{a.handleResponse(JSON.parse(r))}catch(v){a.handleResponse(r)}};p.readAsText(this.response)}else{var q=this.response;p="";m&&-1!==m.indexOf("attachment")&&(m=/filename[^;=\n]*=((['"]).*?\2|[^;\n]*)/.exec(m),null!=m&&m[1]&&(p=m[1].replace(/['"]/g,"")));if("undefined"!==typeof window.navigator.msSaveBlob)window.navigator.msSaveBlob(q,p);else{var k=window.URL||window.webkitURL,t=k.createObjectURL(q);
p?(q=document.createElement("a"),"undefined"===typeof q.download?window.location.href=t:(q.href=t,q.download=p,document.body.appendChild(q),q.click())):window.location.href=t;setTimeout(function(){k.revokeObjectURL(t)},100)}}}else moqui.handleLoadError(this,this.statusText,"")};n.setRequestHeader("Accept","application/json");n.send(f)},handleResponse:function(a){var b=!1;a&&moqui.isPlainObject(a)?(b=moqui.notifyMessages(a.messageInfos,a.errors),a.screenUrl&&0<a.screenUrl.length?this.$root.setUrl(a.screenUrl):
a.redirectUrl&&0<a.redirectUrl.length&&(window.location.href=a.redirectUrl)):console.warn("m-form no response or non-JSON response: "+JSON.stringify(a));(a=this.submitHideId)&&0<a.length&&this.$root.hideContainer(a);(a=this.submitReloadId)&&0<a.length&&this.$root.reloadContainer(a);(a=this.submitMessage)&&a.length?(b=eval('"'+a+'"'),moqui.webrootVue.$q.notify($.extend({},moqui.notifyOpts,{message:b})),moqui.webrootVue.addNotify(b,"success")):b||moqui.webrootVue.$q.notify($.extend({},moqui.notifyOpts,
{message:"Submit successful"}))},fieldChanged:function(a){var b=this.fields[a];a=this.fieldsOriginal[a];return moqui.isArray(b)?!moqui.arraysEqual(b,a,!0):!moqui.equalsOrPlaceholder(b,a)}},computed:{hasFieldsChanged:function(){return moqui.fieldValuesDiff(this.fields,this.fieldsOriginal)}},mounted:function(){var a=this,b=$(this.$el);this.focusField&&this.focusField.length&&b.find('[name^\x3d"'+this.focusField+'"]').addClass("default-focus").focus();b.find('button[type\x3d"submit"], input[type\x3d"submit"], input[type\x3d"image"]').on("click",
function(){a.buttonClicked=this})}});
Vue.component("m-form-link",{name:"mFormLink",props:{fieldsInitial:Object,action:{type:String,required:!0},focusField:String,noValidate:Boolean,bodyParameterNames:Array},data:function(){return{fields:Object.assign({},this.fieldsInitial),fieldsOriginal:Object.assign({},this.fieldsInitial)}},template:'\x3cq-form ref\x3d"qForm" @submit.prevent\x3d"submitForm" @reset.prevent\x3d"resetForm" autocapitalize\x3d"off" autocomplete\x3d"off"\x3e\x3cslot :clearForm\x3d"clearForm" :fields\x3d"fields" :hasFieldsChanged\x3d"hasFieldsChanged" :fieldChanged\x3d"fieldChanged"\x3e\x3c/slot\x3e\x3c/q-form\x3e',methods:{submitForm:function(){if(this.noValidate)this.submitGo();
else{var a=this;this.$refs.qForm.validate().then(function(b){b&&a.submitGo()})}},submitGo:function(){var a=null,b=null,c=$(document.activeElement);c.length&&c.is('button[type\x3d"submit"], input[type\x3d"submit"], input[type\x3d"image"]')&&(c.is("[name]")&&(a=c.attr("name"),b=c.val()),c.prop("disabled",!0),setTimeout(function(){c.prop("disabled",!1)},3E3));var d=Object.keys(this.fields).length?new FormData:new FormData(this.$refs.qForm.$el);$.each(this.fields,function(p,q){!q||moqui.isString(q)&&
q.startsWith("__")||(moqui.isArray(q)?q.forEach(function(k){d.append(p,k)}):d.set(p,q))});for(var e=[],f=[],g="",h=null,l=d.entries()[Symbol.iterator]();;){var n=l.next();if(n.done)break;var m=n.value;n=m[0];m=m[1];0===m.trim().length||"moquiSessionToken"===n||"moquiFormName"===n||0<n.indexOf("[]")||(0<n.indexOf("_op")||0<n.indexOf("_not")||0<n.indexOf("_ic")?e.push({name:n,value:m}):(f.push(n),this.bodyParameterNames&&0<=this.bodyParameterNames.indexOf(n)?(h||={},h[n]=m):(0<g.length&&(g+="\x26"),
g+=encodeURIComponent(n)+"\x3d"+encodeURIComponent(m))))}for(l=0;l<e.length;l++)n=e[l],m=n.name.substring(0,n.name.indexOf("_")),0<=f.indexOf(m)&&(0<g.length&&(g+="\x26"),g+=encodeURIComponent(n.name)+"\x3d"+encodeURIComponent(n.value));a&&b&&b.trim().length&&(0<g.length&&(g+="\x26"),g+=encodeURIComponent(a)+"\x3d"+encodeURIComponent(b));a=this.action;a=0<a.indexOf("?")?a+"\x26"+g:a+"?"+g;this.$root.setUrl(a,h)},resetForm:function(){this.fields=Object.assign({},this.fieldsInitial)},clearForm:function(){this.fields=
{}},fieldChanged:function(a){var b=this.fields[a];a=this.fieldsOriginal[a];return moqui.isArray(b)?!moqui.arraysEqual(b,a,!0):!moqui.equalsOrPlaceholder(b,a)}},computed:{hasFieldsChanged:function(){return moqui.fieldValuesDiff(this.fields,this.fieldsOriginal)}},mounted:function(){var a=$(this.$el);this.focusField&&0<this.focusField.length&&a.find("[name\x3d"+this.focusField+"]").addClass("default-focus").focus()}});
Vue.component("m-form-paginate",{name:"mFormPaginate",props:{paginate:Object,formList:Object},template:'\x3cdiv v-if\x3d"paginate \x26amp;\x26amp; paginate.count \x3e 1" class\x3d"q-pagination row no-wrap items-center"\x3e\x3ctemplate v-if\x3d"paginate.pageIndex \x3e 0"\x3e\x3cq-btn dense flat no-caps @click.prevent\x3d"setIndex(0)" icon\x3d"skip_previous"\x3e\x3c/q-btn\x3e\x3cq-btn dense flat no-caps @click.prevent\x3d"setIndex(paginate.pageIndex-1)" icon\x3d"fast_rewind"\x3e\x3c/q-btn\x3e\x3c/template\x3e\x3ctemplate v-else\x3e\x3cq-btn dense flat no-caps disabled icon\x3d"skip_previous"\x3e\x3c/q-btn\x3e\x3cq-btn dense flat no-caps disabled icon\x3d"fast_rewind"\x3e\x3c/q-btn\x3e\x3c/template\x3e\x3cq-btn v-for\x3d"prevIndex in prevArray" :key\x3d"prevIndex" dense flat no-caps @click.prevent\x3d"setIndex(prevIndex)" :label\x3d"prevIndex+1" color\x3d"primary"\x3e\x3c/q-btn\x3e\x3cq-btn dense flat no-caps disabled\x3e{{paginate.pageIndex+1}} / {{paginate.pageMaxIndex+1}} ({{paginate.pageRangeLow}}-{{paginate.pageRangeHigh}} / {{paginate.count}})\x3c/q-btn\x3e\x3cq-btn v-for\x3d"nextIndex in nextArray" :key\x3d"nextIndex" dense flat no-caps @click.prevent\x3d"setIndex(nextIndex)" :label\x3d"nextIndex+1" color\x3d"primary"\x3e\x3c/q-btn\x3e\x3ctemplate v-if\x3d"paginate.pageIndex \x3c paginate.pageMaxIndex"\x3e\x3cq-btn dense flat no-caps @click.prevent\x3d"setIndex(paginate.pageIndex+1)" icon\x3d"fast_forward"\x3e\x3c/q-btn\x3e\x3cq-btn dense flat no-caps @click.prevent\x3d"setIndex(paginate.pageMaxIndex)" icon\x3d"skip_next"\x3e\x3c/q-btn\x3e\x3c/template\x3e\x3ctemplate v-else\x3e\x3cq-btn dense flat no-caps disabled icon\x3d"fast_forward"\x3e\x3c/q-btn\x3e\x3cq-btn dense flat no-caps disabled icon\x3d"skip_next"\x3e\x3c/q-btn\x3e\x3c/template\x3e\x3c/div\x3e',
computed:{prevArray:function(){var a=this.paginate,b=[];if(!a||1>a.pageIndex)return b;var c=a.pageIndex;a=c-3;0>a&&(a=0);for(--c;a<=c;)b.push(a++);return b},nextArray:function(){var a=this.paginate,b=[];if(!a||a.pageIndex>=a.pageMaxIndex)return b;var c=a.pageIndex;a=a.pageMaxIndex;var d=c+1;c+=3;for(c>a&&(c=a);d<=c;)b.push(d++);return b}},methods:{setIndex:function(a){this.formList?this.formList.setPageIndex(a):this.$root.setParameters({pageIndex:a})}}});
...
Vue.component("m-date-period",{name:"mDatePeriod",props:{fields:{type:Object,required:!0},name:{type:String,required:!0},id:String,allowEmpty:Boolean,fromThruType:{type:String,"default":"date"},form:String,tooltip:String,label:String},data:function(){return{fromThruMode:!1,dateOffsets:moqui.dateOffsets.slice(),datePeriods:moqui.datePeriods.slice(),fieldsOriginal:Object.assign({},this.fields)}},template:'\x3cdiv v-if\x3d"fromThruMode" class\x3d"row"\x3e\x3cm-date-time :name\x3d"name+\'_from\'" :id\x3d"id+\'_from\'" :label\x3d"label+\' From\'" :form\x3d"form" :type\x3d"fromThruType" v-model\x3d"fields[name+\'_from\']" :bg-color\x3d"fieldChanged(name+\'_from\')?($q.dark.isActive?\'blue-10\':\'blue-1\'):\'\'"\x3e\x3c/m-date-time\x3e\x3cq-icon class\x3d"q-my-auto" name\x3d"remove"\x3e\x3c/q-icon\x3e\x3cm-date-time :name\x3d"name+\'_thru\'" :id\x3d"id+\'_thru\'" :label\x3d"label+\' Thru\'" :form\x3d"form" :type\x3d"fromThruType" v-model\x3d"fields[name+\'_thru\']" :bg-color\x3d"fieldChanged(name+\'_thru\')?($q.dark.isActive?\'blue-10\':\'blue-1\'):\'\'"\x3e\x3ctemplate v-slot:after\x3e\x3cq-btn dense flat icon\x3d"calendar_view_day" @click\x3d"toggleMode"\x3e\x3cq-tooltip\x3ePeriod Select Mode\x3c/q-tooltip\x3e\x3c/q-btn\x3e\x3cq-btn dense flat icon\x3d"clear" @click\x3d"clearAll"\x3e\x3cq-tooltip\x3eClear\x3c/q-tooltip\x3e\x3c/q-btn\x3e\x3c/template\x3e\x3c/m-date-time\x3e\x3c/div\x3e\x3cdiv v-else class\x3d"row"\x3e\x3cq-input dense outlined stack-label :label\x3d"label" v-model\x3d"fields[name+\'_pdate\']" mask\x3d"####-##-##" fill-mask :id\x3d"id" :name\x3d"name+\'_pdate\'" :form\x3d"form" style\x3d"max-width:max-content;" :bg-color\x3d"fieldChanged(name+\'_pdate\')?($q.dark.isActive?\'blue-10\':\'blue-1\'):\'\'"\x3e\x3cq-tooltip v-if\x3d"tooltip"\x3e{{tooltip}}\x3c/q-tooltip\x3e\x3ctemplate v-slot:before\x3e\x3cq-select class\x3d"q-pr-xs" dense outlined options-dense emit-value map-options v-model\x3d"fields[name+\'_poffset\']" :name\x3d"name+\'_poffset\'" :bg-color\x3d"fieldChanged(name+\'_poffset\')?($q.dark.isActive?\'blue-10\':\'blue-1\'):\'\'" stack-label label\x3d"Offset" :options\x3d"dateOffsets" :form\x3d"form" behavior\x3d"menu"\x3e\x3c/q-select\x3e\x3cq-select dense outlined options-dense emit-value map-options v-model\x3d"fields[name+\'_period\']" :name\x3d"name+\'_period\'" :bg-color\x3d"fieldChanged(name+\'_period\')?($q.dark.isActive?\'blue-10\':\'blue-1\'):\'\'" stack-label label\x3d"Period" :options\x3d"datePeriods" :form\x3d"form" behavior\x3d"menu"\x3e\x3c/q-select\x3e\x3c/template\x3e\x3ctemplate v-slot:prepend\x3e\x3cq-icon name\x3d"event" class\x3d"cursor-pointer"\x3e\x3cq-popup-proxy ref\x3d"qDateProxy" transition-show\x3d"scale" transition-hide\x3d"scale"\x3e\x3cq-date v-model\x3d"fields[name+\'_pdate\']" mask\x3d"YYYY-MM-DD" @input\x3d"function(){$refs.qDateProxy.hide()}"\x3e\x3c/q-date\x3e\x3c/q-popup-proxy\x3e\x3c/q-icon\x3e\x3c/template\x3e\x3ctemplate v-slot:after\x3e\x3cq-btn dense flat icon\x3d"date_range" @click\x3d"toggleMode"\x3e\x3cq-tooltip\x3eDate Range Mode\x3c/q-tooltip\x3e\x3c/q-btn\x3e\x3cq-btn dense flat icon\x3d"clear" @click\x3d"clearAll"\x3e\x3cq-tooltip\x3eClear\x3c/q-tooltip\x3e\x3c/q-btn\x3e\x3c/template\x3e\x3c/q-input\x3e\x3c/div\x3e',
methods:{toggleMode:function(){this.fromThruMode=!this.fromThruMode},clearAll:function(){this.fields[this.name+"_pdate"]=null;this.fields[this.name+"_poffset"]=null;this.fields[this.name+"_period"]=null;this.fields[this.name+"_from"]=null;this.fields[this.name+"_thru"]=null},fieldChanged:function(a){return!moqui.equalsOrPlaceholder(this.fields[a],this.fieldsOriginal[a])}},mounted:function(){var a=this.fields[this.name+"_from"],b=this.fields[this.name+"_thru"];if(a&&a.length||b&&b.length)this.fromThruMode=
!0}});
Vue.component("m-display",{name:"mDisplay",props:{value:String,display:String,valueUrl:String,valueParameters:Object,dependsOn:Object,dependsOptional:Boolean,valueLoadInit:Boolean,fields:{type:Object},tooltip:String,label:String,labelWrapper:Boolean,name:String,id:String},data:function(){return{curDisplay:this.display,loading:!1}},template:'\x3cq-input v-if\x3d"labelWrapper" dense outlined readonly stack-label autogrow :value\x3d"displayValue" :label\x3d"label" :id\x3d"id" :name\x3d"name" :loading\x3d"loading"\x3e\x3cq-tooltip v-if\x3d"tooltip"\x3e{{tooltip}}\x3c/q-tooltip\x3e\x3c/q-input\x3e\x3cspan v-else :id\x3d"id"\x3e\x3cq-tooltip v-if\x3d"tooltip"\x3e{{tooltip}}\x3c/q-tooltip\x3e\x3cslot\x3e\x3c/slot\x3e{{displayValue}}\x3c/span\x3e',methods:{serverData:function(){var a=
!0,b=this.dependsOn,c=this.valueParameters,d={moquiSessionToken:this.$root.moquiSessionToken};for(f in c)c.hasOwnProperty(f)&&(d[f]=c[f]);for(var e in b)if(b.hasOwnProperty(e)){if(this.fields)c=this.fields[b[e]];else{var f=$("#"+b[e]);(c=f.val())||(c=f.find("select").val())}c?d[e]=c:a=!1}d.hasAllParms=a;return d},populateFromUrl:function(a){a=this.serverData(a);if(this.valueUrl&&this.valueUrl.length)if(a.hasAllParms||this.dependsOptional){var b=this;this.loading=!0;$.ajax({type:"POST",url:this.valueUrl,
data:a,dataType:"text",headers:{Accept:"text/plain"},error:function(c,d,e){b.loading=!1;moqui.handleAjaxError(c,d,e)},success:function(c){b.loading=!1;var d="",e="";try{var f=JSON.parse(c);$.isArray(f)&&f.length?f=f[0]:$.isPlainObject(f)&&f.hasOwnProperty("options")&&f.options.length&&(f=f.options[0]);f.hasOwnProperty("label")&&(d=f.label);f.hasOwnProperty("value")&&(e=f.value)}catch(g){}d&&d.length||(d=c);e&&e.length||(e=c);moqui.isNumber(e)&&(e=e.toString());b.$emit("input",e);b.fields&&b.fields.length&&
b.name&&b.name.length&&(b.fields[b.name+"_display"]=d);b.curDisplay=d}})}else console.warn("In m-display for "+this.name+"  tried to populateFromUrl but not hasAllParms and not dependsOptional"),this.$emit("input",null),this.curDisplay=null;else console.warn("In m-display for "+this.name+" tried to populateFromUrl but no valueUrl")}},computed:{displayValue:function(){return this.curDisplay&&this.curDisplay.length?this.curDisplay:this.value}},mounted:function(){if(this.valueUrl&&this.valueUrl.length){var a=
this.dependsOn,b;for(b in a)a.hasOwnProperty(b)&&this.fields&&this.$watch("fields."+b,function(){this.populateFromUrl({term:this.value})});this.valueLoadInit&&this.populateFromUrl()}}});
Vue.component("m-drop-down",{name:"mDropDown",props:{value:[Array,String],options:{type:Array,"default":function(){return[]}},combo:Boolean,allowEmpty:Boolean,multiple:Boolean,requiredManualSelect:Boolean,submitOnSelect:Boolean,optionsUrl:String,optionsParameters:Object,optionsLoadInit:Boolean,serverSearch:Boolean,serverDelay:{type:Number,"default":300},serverMinLength:{type:Number,"default":1},labelField:{type:String,"default":"label"},valueField:{type:String,"default":"value"},dependsOn:Object,
dependsOptional:Boolean,form:String,fields:{type:Object},tooltip:String,label:String,name:String,id:String,disable:Boolean,bgColor:String,onSelectGoTo:String},data:function(){return{curOptions:this.options,allOptions:this.options,lastVal:null,lastSearch:null,loading:!1}},template:'\x3cq-select ref\x3d"qSelect" v-bind:value\x3d"value" v-on:input\x3d"handleInput($event)" dense outlined options-dense use-input :hide-selected\x3d"multiple" :name\x3d"name" :id\x3d"id" :form\x3d"form" input-debounce\x3d"500" @filter\x3d"filterFn" :clearable\x3d"allowEmpty||multiple" :disable\x3d"disable" :multiple\x3d"multiple" :emit-value\x3d"!onSelectGoTo" map-options behavior\x3d"menu" :rules\x3d"[val \x3d\x3e allowEmpty||multiple||val\x3d\x3d\x3d\'\'||(val\x26\x26val.length)||\'Please select an option\']" stack-label :label\x3d"label" :loading\x3d"loading" :bg-color\x3d"bgColor" :options\x3d"curOptions"\x3e\x3cq-tooltip v-if\x3d"tooltip"\x3e{{tooltip}}\x3c/q-tooltip\x3e\x3ctemplate v-slot:no-option\x3e\x3cq-item\x3e\x3cq-item-section class\x3d"text-grey"\x3eNo results\x3c/q-item-section\x3e\x3c/q-item\x3e\x3c/template\x3e\x3ctemplate v-if\x3d"multiple" v-slot:prepend\x3e\x3cdiv\x3e\x3cq-chip v-for\x3d"valueEntry in value" :key\x3d"valueEntry" dense size\x3d"md" class\x3d"q-my-xs" removable @remove\x3d"removeValue(valueEntry)"\x3e{{optionLabel(valueEntry)}}\x3c/q-chip\x3e\x3c/div\x3e\x3c/template\x3e\x3ctemplate v-slot:append\x3e\x3cslot name\x3d"append"\x3e\x3c/slot\x3e\x3c/template\x3e\x3ctemplate v-slot:after\x3e\x3cslot name\x3d"after"\x3e\x3c/slot\x3e\x3c/template\x3e\x3c/q-select\x3e',
methods:{handleInput:function(a){this.onSelectGoTo&&this.onSelectGoTo.length?a[this.onSelectGoTo]&&this.$root.setUrl(a[this.onSelectGoTo]):this.$emit("input",a);if(this.submitOnSelect){var b=this;b.$nextTick(function(){b.$parent.$parent.submitForm()})}},filterFn:function(a,b,c){if(this.serverSearch)this.lastSearch===a||this.serverMinLength&&(a?a.length:0)<this.serverMinLength?b():(this.lastSearch=a,this.populateFromUrl({term:a},b,c));else if(this.allOptions&&this.allOptions.length){var d=this;a&&
a.length?b(function(){var e=a.toLowerCase();d.curOptions=d.allOptions.filter(function(f){return f.label&&-1<f.label.toLowerCase().indexOf(e)})}):(d.curOptions?d.curOptions.length:0)===(d.allOptions?d.allOptions.length:0)?b():b(function(){d.curOptions=d.allOptions})}else this.optionsUrl&&this.optionsUrl.length?this.populateFromUrl({},b,c):(console.error("m-drop-down "+this.name+" has no options and no options-url"),c())},processOptionList:function(a,b,c){var d=[],e=this.labelField,f=this.valueField;
$.each(a,function(g,h){g=h[f];var l=h[e];d.push(Object.assign(h,{value:g||l,label:l||g}))});return d},serverData:function(a){var b=!0,c=this.dependsOn,d=this.optionsParameters,e={moquiSessionToken:this.$root.moquiSessionToken};for(g in d)d.hasOwnProperty(g)&&(e[g]=d[g]);for(var f in c)if(c.hasOwnProperty(f)){if(this.fields)d=this.fields[c[f]];else{var g=$("#"+c[f]);(d=g.val())||(d=g.find("select").val())}d?e[f]=d:b=!1}a?(e.term=a.term||"",e.pageIndex=(a.page||1)-1):this.serverSearch&&(e.term="",e.pageIndex=
0);e.hasAllParms=b;return e},processResponse:function(a,b){if(moqui.isArray(a))return{results:this.processOptionList(a,null,b.term)};b.page=b.page||1;var c=a.pageSize||20;return{results:this.processOptionList(a.options,b.page,b.term),pagination:{more:a.count?b.page*c<a.count:!1}}},populateFromUrl:function(a,b,c){var d=this.serverData(a);if(this.optionsUrl&&this.optionsUrl.length)if(d.hasAllParms||this.dependsOptional){var e=this;this.loading=!0;$.ajax({type:"POST",url:this.optionsUrl,data:d,dataType:"json",
headers:{Accept:"application/json"},error:function(f,g,h){e.loading=!1;moqui.handleAjaxError(f,g,h);c&&c()},success:function(f){e.loading=!1;f=moqui.isArray(f)?f:f.options;var g=e.processOptionList(f,null,a?a.term:null);f&&(b?b(function(){e.setNewOptions(g)}):(e.setNewOptions(g),e.$refs.qSelect&&e.$refs.qSelect.refresh()))}})}else console.warn("In m-drop-down tried to populateFromUrl but not hasAllParms and not dependsOptional"),this.curOptions=[],this.allOptions=[],c&&c();else console.warn("In m-drop-down tried to populateFromUrl but no optionsUrl"),
c&&c()},setNewOptions:function(a){this.curOptions=a;if(this.multiple&&this.allOptions&&this.allOptions.length&&this.value&&this.value.length&&moqui.isArray(this.value)){a=a.slice();for(var b=0;b<this.value.length;b++)for(var c=this.value[b],d=0;d<this.allOptions.length;d++){var e=this.allOptions[d];c===e.value&&a.push(e)}this.allOptions=a}else this.allOptions=a,this.checkCurrentValue(this.allOptions)},checkCurrentValue:function(a){var b=!1,c=moqui.isArray(this.value);if(this.value&&this.value.length&&
a)for(var d=0;d<a.length;d++){var e=a[d];if(c?this.value.includes(e.value):e.value===this.value){b=!0;break}}b||(this.allowEmpty||this.multiple||!a||!a.length||!a[0].value||this.requiredManualSelect&&(this.submitOnSelect||1!==a.length)?this.$emit("input",null):this.$emit("input",a[0].value))},optionLabel:function(a){var b=this.allOptions;if(!b||!b.length)return"";for(var c=0;c<b.length;c++){var d=b[c];if(a===d.value)return d.label}return""},removeValue:function(a){var b=this.value;if(moqui.isArray(b)){for(var c=
[],d=0;d<b.length;d++){var e=b[d];e!==a&&c.push(e)}b.length!==c.length&&this.$emit("input",c);if(this.submitOnSelect){var f=this;f.$nextTick(function(){f.$parent.$parent.submitForm()})}}else console.warn("Tried to remove value from m-drop-down multiple "+this.name+" but value is not an Array")},clearAll:function(){this.$emit("input",null)}},mounted:function(){this.serverSearch&&(this.optionsUrl||console.error("m-drop-down in form "+this.form+" has no options-url but has server-search\x3dtrue"));if(this.optionsUrl&&
this.optionsUrl.length){var a=this.dependsOn,b;for(b in a)if(a.hasOwnProperty(b)&&this.fields){var c=this;this.$watch("fields."+a[b],function(){c.$emit("input",null);c.populateFromUrl({term:c.lastSearch})})}this.optionsLoadInit&&(this.serverSearch?this.value&&this.value.length&&moqui.isString(this.value)&&this.populateFromUrl({term:this.value}):this.populateFromUrl())}this.multiple||this.allowEmpty||this.value&&this.value.length||!this.options||!this.options.length||this.requiredManualSelect&&(this.submitOnSelect||
1!==options.length)||this.$emit("input",this.options[0].value)}});
Vue.component("m-text-line",{name:"mTextLine",props:{value:String,type:{type:String,"default":"text"},id:String,name:String,size:String,fields:{type:Object},dense:Boolean,outlined:Boolean,bgColor:String,label:String,tooltip:String,prefix:String,disable:Boolean,mask:String,fillMask:String,reverseFillMask:Boolean,rules:Array,defaultUrl:String,defaultParameters:Object,dependsOn:Object,dependsOptional:Boolean,defaultLoadInit:Boolean},data:function(){return{loading:!1}},template:'\x3cq-input :dense\x3d"dense" :outlined\x3d"outlined" :bg-color\x3d"bgColor" stack-label :label\x3d"label" :prefix\x3d"prefix" v-bind:value\x3d"value" v-on:input\x3d"$emit(\'input\', $event)" :type\x3d"type" :id\x3d"id" :name\x3d"name" :size\x3d"size" :loading\x3d"loading" :rules\x3d"rules" :disable\x3d"disable" :mask\x3d"mask" :fill-mask\x3d"fillMask" :reverse-fill-mask\x3d"reverseFillMask" autocapitalize\x3d"off" autocomplete\x3d"off"\x3e\x3cq-tooltip v-if\x3d"tooltip"\x3e{{tooltip}}\x3c/q-tooltip\x3e\x3c/q-input\x3e',
methods:{serverData:function(){var a=!0,b=this.dependsOn,c=this.defaultParameters,d={moquiSessionToken:this.$root.moquiSessionToken};for(f in c)c.hasOwnProperty(f)&&(d[f]=c[f]);for(var e in b)if(b.hasOwnProperty(e)){if(this.fields)c=this.fields[b[e]];else{var f=$("#"+b[e]);(c=f.val())||(c=f.find("select").val())}c?d[e]=c:a=!1}d.hasAllParms=a;return d},populateFromUrl:function(a){a=this.serverData(a);if(this.defaultUrl&&this.defaultUrl.length)if(a.hasAllParms||this.dependsOptional){var b=this;this.loading=
!0;$.ajax({type:"POST",url:this.defaultUrl,data:a,dataType:"text",error:function(c,d,e){b.loading=!1;moqui.handleAjaxError(c,d,e)},success:function(c){b.loading=!1;c&&c.length&&b.$emit("input",c)}})}else console.warn("In m-text-line tried to populateFromUrl but not hasAllParms and not dependsOptional");else console.warn("In m-text-line tried to populateFromUrl but no defaultUrl")}},mounted:function(){if(this.defaultUrl&&this.defaultUrl.length){var a=this.dependsOn,b;for(b in a)a.hasOwnProperty(b)&&
...
console.error("Error loading m-chart script: "+b):a.instance=new Chart(a.$refs.canvas,a.config)},function(){return!!window.Chart})},watch:{config:function(a){this.instance&&(a.type&&(this.instance.type=a.type),a.labels&&(this.instance.labels=a.labels),a.data&&(this.instance.data=a.data),a.options&&(this.instance.options=a.options),this.instance.update())}}});
Vue.component("m-mermaid",{name:"mMermaid",props:{config:{type:Object,"default":function(){return{startOnLoad:!0,securityLevel:"loose"}}},height:{type:String,"default":"400px"},width:{type:String,"default":"100%"}},template:'\x3cpre ref\x3d"mermaid" class\x3d"mermaid" :style\x3d"{height:height,width:width}"\x3e\x3cslot\x3e\x3c/slot\x3e\x3c/pre\x3e',mounted:function(){var a=this;moqui.loadScript("https://cdnjs.cloudflare.com/ajax/libs/mermaid/9.3.0/mermaid.min.js",function(b){b||mermaid.init(a.config,
a.$refs.mermaid)},function(){return!!window.mermaid})}});
...
!1;var c=a.ckeditor=CKEDITOR[a.useInline?"inline":"replace"](a.$refs.area,b);c.on("instanceReady",function(){var d=a.value;c.fire("lockSnapshot");c.setData(d,{callback:function(){c.on("change",function(f){var g=c.getData();a.value!==g&&a.$emit("input",g,f,c)});c.on("focus",function(f){a.$emit("focus",f,c)});c.on("blur",function(f){a.$emit("blur",f,c)});var e=c.getData();d!==e?(a.$once("input",function(){a.$emit("ready",c)}),a.$emit("input",e)):a.$emit("ready",c);c.fire("unlockSnapshot")}})})}},function(){return!!window.CKEDITOR})},
...
initialValue:a.value},a.config);var c=a.simplemde=new SimpleMDE(b);c.codemirror.on("change",function(d,e){"setValue"!==e.origin&&(d=c.value(),a.$emit("input",d))});c.codemirror.on("blur",function(){var d=c.value();a.$emit("blur",d)});a.$nextTick(function(){a.$emit("initialized",c)})}},function(){return!!window.SimpleMDE})},watch:{value:function(a){this.simplemde&&this.simplemde.value()!==a&&this.simplemde.value(a)}}});
...
methods:{goTo:function(a){this.$root.setUrl(this.$root.getLinkPath(a))}},computed:{subscreens:function(){if(!this.pathIndex||0>this.pathIndex)return[];var a=this.$root.navMenuList[this.pathIndex];return a&&a.subscreens?a.subscreens:[]},activeTab:function(){if(!this.pathIndex||0>this.pathIndex)return null;var a=this.$root.navMenuList[this.pathIndex];if(!a||!a.subscreens)return null;var b=null;$.each(a.subscreens,function(c,d){d.active&&(b=d.name)});return b}},mounted:function(){this.pathIndex=this.$root.activeSubscreens.length}});
Vue.component("m-subscreens-active",{name:"mSubscreensActive",data:function(){return{activeComponent:moqui.EmptyComponent,pathIndex:-1,pathName:null}},template:'\x3ccomponent :is\x3d"activeComponent" style\x3d"height:100%;width:100%;"\x3e\x3c/component\x3e',methods:{loadActive:function(){var a=this,b=a.$root,c=a.pathIndex,d=b.currentPathList,e=d[c],f=this.pathName!==e;this.pathName=e;if(!e||0===e.length)return console.info("in m-subscreens-active newPath is empty, loading EmptyComponent and returning true"),
this.activeComponent=moqui.EmptyComponent,!0;e=b.basePath+"/"+d.slice(0,c+1).join("/");if(!f&&moqui.componentCache.containsKey(e))return!1;f={path:e};c===d.length-1&&(d=b.extraPathList)&&0<d.length&&(f.extraPath=d.join("/"));(d=b.currentSearch)&&0<d.length&&(f.search=d);f.bodyParameters=b.bodyParameters;(d=b.navMenuList[c+b.basePathSize])&&d.renderModes&&(f.renderModes=d.renderModes);console.info("m-subscreens-active loadActive pathIndex "+c+" pathName "+a.pathName+" urlInfo "+JSON.stringify(f));
b.loading++;b.currentLoadRequest=moqui.loadComponent(f,function(g){b.currentLoadRequest=null;a.activeComponent=g;b.loading--});return!0}},mounted:function(){this.$root.addSubscreen(this)}});
...
moqui.webrootVue=new Vue({el:"#apps-root",data:{basePath:"",linkBasePath:"",currentPathList:[],extraPathList:[],currentParameters:{},bodyParameters:null,activeSubscreens:[],navMenuList:[],navHistoryList:[],navPlugins:[],accountPlugins:[],notifyHistoryList:[],lastNavTime:Date.now(),loading:0,currentLoadRequest:null,activeContainers:{},urlListeners:[],moquiSessionToken:"",appHost:"",appRootPath:"",userId:"",username:"",locale:"en",reLoginShow:!1,reLoginPassword:null,reLoginMfaData:null,reLoginOtp:null,
notificationClient:null,sessionTokenBc:null,qzVue:null,leftOpen:!1,moqui},methods:{setUrl:function(a,b,c,d=!0){this.currentLoadRequest&&(console.log("Aborting current page load currentLinkUrl "+this.currentLinkUrl+" url "+a),this.currentLoadRequest.abort(),this.currentLoadRequest=null,this.loading=0);this.bodyParameters=b;a=this.getLinkPath(a);if(this.currentLinkUrl===a&&a!==this.linkBasePath)this.reloadSubscreens(),c&&this.callOnComplete(c,this.currentPath);else{var e=this.currentPath;b=moqui.parseHref(a);
this.extraPathList=[];this.currentSearch=b.search;this.currentPath=b.path;b=this.currentSearch;if((b=this.currentPath+(0<b.length?"?"+b:""))&&0!==b.length){console.info("Current URL changing to "+b);this.lastNavTime=Date.now();this.activeContainers={};var f=this;b=this.appRootPath&&this.appRootPath.length&&0===b.indexOf(this.appRootPath)?this.appRootPath+"/menuData"+b.slice(this.appRootPath.length):"/menuData"+b;$.ajax({type:"GET",url:b,dataType:"text",error:moqui.handleAjaxError,success:function(g){var h=
null;try{h=JSON.parse(g)}catch(l){console.info("Error parson menu list JSON: "+l)}h&&moqui.isArray(h)&&(f.navMenuList=h,c&&f.callOnComplete(c,e))}});d&&window.history.pushState(null,this.ScreenTitle,a);this.urlListeners.forEach(function(g){g(a,this)},this);document.documentElement.scrollTop=0;document.body.scrollTop=0}}},callOnComplete:function(a,b){if(a){var c=this.getRoute();b&&(c.redirectedFrom=b);a(c)}},getRoute:function(){return{name:this.currentPathList[this.currentPathList.length-1],meta:{},
path:this.currentPath,hash:"",query:this.currentParameters,params:this.bodyParameters||{},fullPath:this.currentLinkUrl,matched:[]}},setParameters:function(a){if(a){this.$root.currentParameters=$.extend({},this.$root.currentParameters,a);a=this.currentLinkUrl;var b=this.navHistoryList[0];b.pathWithParams=a;window.history.pushState(null,b.title||"",a)}this.$root.reloadSubscreens()},addSubscreen:function(a){a.pathIndex=this.activeSubscreens.length;a.loadActive();this.activeSubscreens.push(a)},reloadSubscreens:function(){var a=
this.currentPathList,b=this.activeSubscreens;console.info("reloadSubscreens currentPathList "+JSON.stringify(this.currentPathList));if(0===a.length&&0<b.length)b.splice(1),b[0].loadActive();else for(var c=0;c<b.length&&!(c>=a.length);c++)b[c].loadActive()&&b.splice(c+1)},goPreviousScreen:function(){for(var a=this.currentPath,b=this.navHistoryList,c,d=0;d<b.length;d++)if(0>b[d].pathWithParams.indexOf(a)){c=b[d];break}c&&c.pathWithParams&&c.pathWithParams.length&&this.setUrl(c.pathWithParams)},addContainer:function(a,
b){this.activeContainers[a]=b},reloadContainer:function(a){var b=this.activeContainers[a];b?b.reload():console.error("Container with ID "+a+" not found, not reloading")},loadContainer:function(a,b){var c=this.activeContainers[a];c?c.load(b):console.error("Container with ID "+a+" not found, not loading url "+b)},hideContainer:function(a){var b=this.activeContainers[a];b?b.hide():console.error("Container with ID "+a+" not found, not hidding")},addNavPlugin:function(a){var b=this;moqui.loadComponent(this.appRootPath+
a,function(c){b.navPlugins.push(c)})},addNavPluginsWait:function(a,b){if(a&&a.length>b){this.addNavPlugin(a[b]);var c=this;a.length>b+1&&setTimeout(function(){c.addNavPluginsWait(a,b+1)},500)}},addAccountPlugin:function(a){var b=this;moqui.loadComponent(this.appRootPath+a,function(c){b.accountPlugins.push(c)})},addAccountPluginsWait:function(a,b){if(a&&a.length>b){this.addAccountPlugin(a[b]);var c=this;a.length>b+1&&setTimeout(function(){c.addAccountPluginsWait(a,b+1)},500)}},addUrlListener:function(a){0<=
this.urlListeners.indexOf(a)||this.urlListeners.push(a)},addNotify:function(a,b,c,d){var e=this.notifyHistoryList.slice(0),f=new Date,g=f.getHours();10>g&&(g="0"+g);f=f.getMinutes();10>f&&(f="0"+f);for(e.unshift({message:a,type:b,time:g+":"+f,link:c,icon:d});25<e.length;)e.pop();this.notifyHistoryList=e},switchDarkLight:function(){this.$q.dark.toggle();$.ajax({type:"POST",url:this.appRootPath+"/apps/setPreference",error:moqui.handleAjaxError,data:{moquiSessionToken:this.moquiSessionToken,preferenceKey:"QUASAR_DARK",
preferenceValue:this.$q.dark.isActive?"true":"false"}})},toggleLeftOpen:function(){this.leftOpen=!this.leftOpen;$.ajax({type:"POST",url:this.appRootPath+"/apps/setPreference",error:moqui.handleAjaxError,data:{moquiSessionToken:this.moquiSessionToken,preferenceKey:"QUASAR_LEFT_OPEN",preferenceValue:this.leftOpen?"true":"false"}})},stopProp:function(a){a.stopPropagation()},getNavHref:function(a){a||=this.navMenuList.length-1;a=this.navMenuList[a];if(a.extraPathList&&a.extraPathList.length){var b=a.path+
"/"+a.extraPathList.join("/"),c=a.pathWithParams.indexOf("?");0<c&&(b+=a.pathWithParams.slice(c));return b}return a.pathWithParams||a.path},getLinkPath:function(a){moqui.isPlainObject(a)&&(a=moqui.makeHref(a));this.appRootPath&&this.appRootPath.length&&0!==a.indexOf(this.appRootPath)&&(a=this.appRootPath+a);a=a.split("/");var b=this.appRootPath.split("/").length;a[b]=this.linkBasePath.split("/").slice(-1);return a=a.join("/")},getQuasarColor:function(a){return moqui.getQuasarColor(a)},getCsrfToken:function(a){(a=
a.getResponseHeader("X-CSRF-Token"))&&a.length&&a!==this.moquiSessionToken&&(console.log("Updating session token from jqXHR, sending to BroadcastChannel"),this.moquiSessionToken=a,this.sessionTokenBc.postMessage(a))},receiveBcCsrfToken:function(a){(a=a.data)&&a.length&&this.moquiSessionToken!==a&&(console.log("Updating session token from BroadcastChannel"),this.moquiSessionToken=a)},reLoginCheckShow:function(){this.reLoginShowDialog()},reLoginShowDialog:function(){this.reLoginOtp=this.reLoginMfaData=
null;this.reLoginShow=!0},reLoginPostLogin:function(){this.reLoginShow=!1;this.reLoginMfaData=this.reLoginOtp=this.reLoginPassword=null;this.$q.notify({timeout:12E3,type:"positive",message:"Background login successful"});this.addNotify("Background login successful","positive")},reLoginSubmit:function(){$.ajax({type:"POST",url:this.appRootPath+"/rest/login",error:moqui.handleAjaxError,success:this.reLoginHandleResponse,dataType:"json",headers:{Accept:"application/json"},xhrFields:{withCredentials:!0},
data:{username:this.username,password:this.reLoginPassword}})},reLoginHandleResponse:function(a,b,c){this.getCsrfToken(c);a.secondFactorRequired?this.reLoginMfaData=a:a.loggedIn&&this.reLoginPostLogin()},reLoginReload:function(){confirm("Reload page? All changes will be lost.")&&(window.location.href=this.currentLinkUrl)},reLoginSendOtp:function(a){$.ajax({type:"POST",url:this.appRootPath+"/rest/sendOtp",error:moqui.handleAjaxError,success:this.reLoginSendOtpResponse,dataType:"json",headers:{Accept:"application/json"},
xhrFields:{withCredentials:!0},data:{moquiSessionToken:this.moquiSessionToken,factorId:a}})},reLoginSendOtpResponse:function(a,b,c){a&&moqui.notifyMessages(a.messages,a.errors,a.validationErrors)},reLoginVerifyOtp:function(){$.ajax({type:"POST",url:this.appRootPath+"/rest/verifyOtp",error:moqui.handleAjaxError,success:this.reLoginVerifyOtpResponse,dataType:"json",headers:{Accept:"application/json"},xhrFields:{withCredentials:!0},data:{moquiSessionToken:this.moquiSessionToken,code:this.reLoginOtp}})},
reLoginVerifyOtpResponse:function(a,b,c){this.getCsrfToken(c);a.loggedIn&&this.reLoginPostLogin()},qLayoutMinHeight:function(a){return{minHeight:a?`calc(100vh - ${a}px)`:"100vh"}}},watch:{navMenuList:function(a){if(0<a.length){var b=a[a.length-1];a=1<a.length?a[a.length-2]:null;b.extraPathList&&(this.extraPathList=b.extraPathList);var c=this.basePathSize,d=b.path.split("/").slice(c+1);console.info("nav updated fullPath "+JSON.stringify(d)+" currentPathList "+JSON.stringify(this.currentPathList)+" cur.path "+
b.path+" basePathSize "+c);this.currentPathList=d;this.reloadSubscreens();a=(a?a.title+" - ":"")+b.title;c=b.pathWithParams;var e=c.indexOf("?");if(0<e){d=["pageIndex","orderBySelect","orderByField","moquiSessionToken"];var f=c.substring(e+1).split("\x26");c=c.substring(0,e);e=0;for(var g="",h=0;h<f.length;h++){var l=f[h];c=-1===c.indexOf("?")?c+"?":c+"\x26";c+=l;if(!(3<e)){var n=l.indexOf("\x3d");if(0<n){var m=l.substring(0,n);l=l.substring(n+1);0<m.indexOf("_op")||0<m.indexOf("_not")||0<m.indexOf("_ic")||
0<=d.indexOf(m)||m===l||(0<g.length&&(g+=", "),g+=decodeURIComponent(l),e++)}}}0<g.length&&(70<g.length&&(g=g.substring(0,70)+"..."),a=a+" ("+g+")")}d=this.navHistoryList;for(f=0;f<d.length;)d[f].pathWithParams===c?d.splice(f,1):f++;for(d.unshift({title:a,pathWithParams:c,image:b.image,imageType:b.imageType});25<d.length;)d.pop();document.title=a}},currentPathList:function(a){(a=a[a.length-1])&&$(this.$el).removeClass().addClass(a)}},computed:{currentPath:{get:function(){var a=this.currentPathList,
b=this.extraPathList;return this.basePath+(a&&0<a.length?"/"+a.join("/"):"")+(b&&0<b.length?"/"+b.join("/"):"")},set:function(a){a&&0!==a.length?("/"===a.slice(a.length-1)&&(a=a.slice(0,a.length-1)),0===a.indexOf(this.linkBasePath)?a=a.slice(this.linkBasePath.length+1):0===a.indexOf(this.basePath)&&(a=a.slice(this.basePath.length+1)),this.currentPathList=a.split("/")):this.currentPathList=[]}},currentLinkPath:function(){var a=this.currentPathList,b=this.extraPathList;return this.linkBasePath+(a&&
0<a.length?"/"+a.join("/"):"")+(b&&0<b.length?"/"+b.join("/"):"")},currentSearch:{get:function(){return moqui.objToSearch(this.currentParameters)},set:function(a){this.currentParameters=moqui.searchToObj(a)}},currentLinkUrl:function(){var a=this.currentSearch;return this.currentLinkPath+(0<a.length?"?"+a:"")},basePathSize:function(){return this.basePath.split("/").length-this.appRootPath.split("/").length},ScreenTitle:function(){return 0<this.navMenuList.length?this.navMenuList[this.navMenuList.length-
1].title:""},documentMenuList:function(){for(var a=[],b=0;b<this.navMenuList.length;b++){var c=this.navMenuList[b].screenDocList;c&&c.length&&c.forEach(function(d){a.push(d)})}return a}},created:function(){this.moquiSessionToken=$("#confMoquiSessionToken").val();this.appHost=$("#confAppHost").val();this.appRootPath=$("#confAppRootPath").val();this.basePath=$("#confBasePath").val();this.linkBasePath=$("#confLinkBasePath").val();this.userId=$("#confUserId").val();this.username=$("#confUsername").val();
this.locale=$("#confLocale").val();moqui.localeMap[this.locale]&&(this.locale=moqui.localeMap[this.locale]);this.leftOpen="true"===$("#confLeftOpen").val();var a=$("#confDarkMode").val();this.$q.dark.set("true"===a);this.notificationClient=new moqui.NotificationClient(("https:"===location.protocol?"wss://":"ws://")+this.appHost+this.appRootPath+"/notws");this.sessionTokenBc=new BroadcastChannel("SessionToken");this.sessionTokenBc.onmessage=this.receiveBcCsrfToken;var b=[];$(".confNavPluginUrl").each(function(d,
e){b.push($(e).val())});this.addNavPluginsWait(b,0);var c=[];$(".confAccountPluginUrl").each(function(d,e){c.push($(e).val())});this.addAccountPluginsWait(c,0)},mounted:function(){$(this.$el).css("display","initial");this.setUrl(window.location.pathname+window.location.search);this.notificationClient.registerListener("ALL");window.Notification&&"granted"!==Notification.permission&&"denied"!==Notification.permission&&Notification.requestPermission(function(a){"granted"===a?moqui.notifyMessages("Browser notifications enabled, if you don't want them use browser notification settings to block"):
"denied"===a&&moqui.notifyMessages("Browser notifications disabled, if you want them use browser notification settings to allow")})},beforeDestroy:function(){this.sessionTokenBc.close()}});window.addEventListener("popstate",function(){moqui.webrootVue.setUrl(window.location.pathname+window.location.search,null,null,!1)});

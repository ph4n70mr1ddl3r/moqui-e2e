...
public interface ServiceFacade {
	 ...
	 *
	 * @param jobName
	 *            The name of the job. There must be a moqui.service.job.ServiceJob
	 *            record for this jobName.
	 */
	ServiceCallJob job(String jobName);

	/**
	 * Get a service caller for special service calls such as on commit and on
	 * rollback of current transaction.
	 */
	ServiceCallSpecial special();

	/**
	 * Call a JSON remote service. For Moqui services the location will be something
	 * like "http://hostname/rpc/json".
	 */
	 ...
	 *            "${path}.${verb}${noun}". To explicitly separate the verb and noun

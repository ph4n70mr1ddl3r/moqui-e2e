...
public interface ArtifactExecutionInfo {
	enum ArtifactType {
		AT_XML_SCREEN, AT_XML_SCREEN_TRANS, AT_XML_SCREEN_CONTENT, AT_SERVICE, AT_ENTITY, AT_REST_PATH, AT_OTHER
	}
	enum AuthzAction {
		AUTHZA_VIEW, AUTHZA_CREATE, AUTHZA_UPDATE, AUTHZA_DELETE, AUTHZA_ALL
	}
	enum AuthzType {
		AUTHZT_ALLOW, AUTHZT_DENY, AUTHZT_ALWAYS
	}
	...
	Map<String, AuthzAction> authzActionByName = Collections.unmodifiableMap(Stream
			.of(new SimpleEntry<>("view", AUTHZA_VIEW), new SimpleEntry<>("create", AUTHZA_CREATE),
					new SimpleEntry<>("update", AUTHZA_UPDATE), new SimpleEntry<>("delete", AUTHZA_DELETE),
					new SimpleEntry<>("all", AUTHZA_ALL))

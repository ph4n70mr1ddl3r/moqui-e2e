...
public interface NotificationMessage extends java.io.Serializable {
	...
	/**
	 * Get userId for all users associated with this notification, directly or
	 * through the UserGroup, and who have
	 * NotificationTopicUser.receiveNotifications=Y, which if not set (or there is
	 * no NotificationTopicUser record) defaults to
	 * NotificationTopic.receiveNotifications (if not set defaults to Y)
	 */
	...
	/**
	 * Set the message as a JSON String. The top-level should be a Map (JSON
	 * Object).
	 * 
	 * @param messageJson
	 *            The message as a JSON string containing a Map (JSON Object)
	 ...
	 * 
	 * @param message
	 *            The message as a Map (JSON Object), must be convertible to JSON
	 *            String
	 * @return Self-reference for convenience
	 */
	...
	/**
	 * Set the title to display, a GString (${} syntax) that will be expanded using
	 * the message Map; may be a localization template name
	 */
	NotificationMessage title(String title);
	/**
	 * Get the title, expanded using the message Map; if not set and topic has a
	 * NotificationTopic record will default to value there
	 */
	String getTitle();
	/**
	 * Set the link to get more detail about the notification or go to its source, a
	 * GString (${} syntax) expanded using the message Map
	 */
	NotificationMessage link(String link);
	/**
	 * Get the link to detail/source, expanded using the message Map; if not set and
	 * topic has a NotificationTopic record will default to value there
	 */
	String getLink();

	NotificationMessage type(NotificationType type);
	/**
	 * Must be a String for a valid NotificationType (ie info, success, warning, or
	 * danger)
	 */
	NotificationMessage type(String type);
	/**
	 * Get the type as a String; if not set and topic has a NotificationTopic record
	 * will default to value there
	 */
	String getType();

	NotificationMessage showAlert(boolean show);
	/**
	 * Show an alert for this notification? If not set and topic has a
	 * NotificationTopic record will default to value there
	 */
	boolean isShowAlert();
	 ...
	 * 
	 * @param persist
	 *            If true this is persisted and message received is tracked. If
	 *            false this is sent to active topic listeners only.
	 * @return Self-reference for convenience
	 */
	NotificationMessage send(boolean persist);
	/**
	 * Send this Notification Message using persistOnSend setting (defaults to
	 * false).
	 */
	 ...
	 * type, and showAlert using the get method for each
	 */

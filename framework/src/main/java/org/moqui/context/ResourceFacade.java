...
public interface ResourceFacade {
	 ...
	 *
	 * @param location
	 *            A URL-style location string. In addition to the standard URL
	 *            protocols (http, https, ftp, jar, and file) can also have the
	 *            special Moqui protocols of "component://" for a resource location
	 *            relative to a component base location, "content://" for a resource
	 *            in the content repository, and "classpath://" to get a resource
	 ...
	 *
	 * @param location
	 *            A URL-style location string that also support the Moqui-specific
	 *            component and content protocols.
	 */
	InputStream getLocationStream(String location);

	/**
	 * Get the text at the given location, optionally from the cache
	...
	/**
	 * Render a template at the given location using the current context and write
	...
	/**
	 * Run a script at the given location (optionally with the given method, like in
	 * a groovy class) using the current context for its variable space.
	 *
	...
	/**
	 * Evaluate a Groovy expression as a context field, or more generally as an
	 * expression that evaluates to an Object reference. This can be used to get a
	 * value from an expression or to run any general expression or script.
	 *
	...
	/**
	 * Evaluate a Groovy expression as a GString to be expanded/interpolated into a
	 * simple String.
	 *
	 * NOTE: the inputString is always run through the L10nFacade.localize() method
	 * before evaluating the expression in order to implicitly internationalize
	 * string expansion.
	 *

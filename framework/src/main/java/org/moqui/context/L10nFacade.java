...
public interface L10nFacade {
	...
	/**
	 * Use the current locale (see ec.user.getLocale() method) to localize the
	 * message based on data in the moqui.basic.LocalizedMessage entity. The
	 * localized message may have variables inserted using the ${} syntax that when
	 * this is called through ec.resource.expand().
	 *
	 * The approach here is that original messages are actual messages in the
	 * primary language of the application. This reduces issues with duplicated
	 * messages compared to the approach of explicit/artificial property keys.
	 * Longer messages (over 255 characters) should use an artificial message key
	 * with the actual value always coming from the database.
	 */
	 ...
	 * 
	 * @param amount
	 *            An object representing the amount, should be a subclass of Number.
	 * @param uomId
	 *            The uomId (ISO currency code), required.
	 * @param fractionDigits
	 *            Number of digits after the decimal point to display. If null
	 *            defaults to number defined by
	 *            java.util.Currency.defaultFractionDigits() for the specified
	 *            currency in uomId.
	 * @param locale
	 *            Locale to use for formatting.
	 * @param hideSymbol
	 *            option to hide the Symbol of the currency and only display the
	 *            number formatted according to locale.
	...
	/**
	 * Round currency according to the currency's specified amount of digits and
	 * rounding method.
	 * 
	 * @param amount
	 *            The amount in BigDecimal to be rounded.
	 * @param uomId
	 *            The currency uomId (ISO currency code), required
	 * @param precise
	 *            A boolean indicating whether the currency should be treated with
	 *            an additional digit
	 * @param roundingMode
	...
	/**
	 * Format a Number, Timestamp, Date, Time, or Calendar object using the given
	 * format string. If no format string is specified the default for the user's
	 * locale and time zone will be used.
	 *
	 * @param value
	 *            The value to format. Must be a Number, Timestamp, Date, Time, or
	 *            Calendar object.
	 * @param format
	 *            The format string used to specify how to format the value.

...
/**
 * Light weight String Keyed Map optimized for memory usage and garbage
 * collection overhead. Uses parallel key and value arrays internally and does
 * not create an object for each Map.Entry unless entrySet() is used. This is
 * generally slower than HashMap unless key String objects are already interned.
 * With '*IString' variations of methods a call with a known already interned
 * String can operate as fast, and for smaller Maps faster, than HashMap (such
 * as in the EntityFacade where field names come from an interned String in a
 * FieldInfo object). This is most certainly not thread-safe.
 */
public class LiteStringMap<V>
		implements
			Map<String, V>,
			Externalizable,
			Comparable<Map<String, ? extends V>>,
			Cloneable {
	// NOTE: for over the wire compatibility do not change this unless
	// writeExternal() and readExternal() are changed OR the non-transient fields
	// change from only keyArray, valueArray, and lastIndex
	private static final long serialVersionUID = 688763341199951234L;
	private static final int DEFAULT_CAPACITY = 8;

	// NOTE: from basic profiling HashMap.get() runs in just over half the time
	// (0.13 microseconds) of String.intern() (0.24 microseconds) over ~500k runs
	// with OpenJDK 8
	...
	// NOTE: key design point is to use parallel arrays with simple values in each
	// so that no Object need be created per entry (minimize GC overhead, etc)
	...
	public LiteStringMap(Map<String, V> cloneMap) {
		init(cloneMap.size());
		if (cloneMap instanceof LiteStringMap && ((LiteStringMap<V>) cloneMap).useManualIndex)
			useManualIndex = true;
		putAll(cloneMap);
	}
	...
	public int findIndex(String keyOrig) {
		...
		/*
		 * safer but slower approach, needed? by String.intern() JavaDoc no, consistency
		 * guaranteed int keyLength = keyString.length(); int keyHashCode =
		 * keyString.hashCode(); // NOTE: can't use Arrays.binarySearch() as we want to
		 * maintain the insertion order and not use natural order for array elements for
		 * (int i = 0; i <= lastIndex; i++) { // all strings in keyArray should be
		 * interned, only added via put() String curKey = keyArray[i]; // first
		 * optimization is using interned String with identity compare, but don't always
		 * rely on this // next optimization comparing length() and hashCode() first to
		 * eliminate mismatches more quickly (by far the most common case) // basic
		 * premise is that key Strings will be reused frequently and will already have a
		 * hashCode calculated if (curKey == keyString || (curKey.length() == keyLength
		 * && curKey.hashCode() == keyHashCode && keyString.equals(curKey))) return i; }
		 * return -1;
		 */
	}
	/**
	 * For this method the String key must be non-null and interned (returned value
	 * from String.intern())
	 */
	...
	public boolean containsKeyIString(String key, int index) {
		...
		if (idxKey != key)
			throw new IllegalArgumentException(
					"Index " + index + " has key " + keyArray[index] + ", cannot check contains with key " + key);
		return true;
	}
	...
	public V getByIString(String key, int index) {
		if (index >= keyArray.length)
			throw new ArrayIndexOutOfBoundsException(
					"Index " + index + " invalid, internal array length " + keyArray.length + "; for key: " + key);
		String idxKey = keyArray[index];
		if (idxKey == null)
			return null;
		if (idxKey != key)
			throw new IllegalArgumentException(
					"Index " + index + " has key " + keyArray[index] + ", cannot get with key " + key);
	...
	public V put(String keyOrig, V value) {
		if (keyOrig == null)
			throw new IllegalArgumentException("LiteStringMap Key may not be null");
	...
	public V putByIString(String key, V value) {
		// if ("pseudoId".equals(key)) { System.out.println("========= put no index " +
		// key + ": " + value); new Exception("location").printStackTrace(); }
		int keyIndex = findIndexIString(key);
		if (keyIndex == -1) {
			lastIndex++;
			if (lastIndex >= keyArray.length)
				growArrays(null);
	...
	public V putByIString(String key, V value, int index) {
		// if ("pseudoId".equals(key)) { System.out.println("========= put index " +
		// index + " key " + key + ": " + value); new
		// Exception("location").printStackTrace(); }
		useManualIndex = true;
		if (index >= keyArray.length)
			growArrays(index + 1);
		if (index > lastIndex)
		...
		if (keyArray[index] == null) {
			...
			if (key != keyArray[index])
				throw new IllegalArgumentException(
						"Index " + index + " already has key " + keyArray[index] + ", cannot use with key " + key);
	...
	private V removeByIndex(int keyIndex) {
		if (keyIndex == -1) {
			...
			V oldValue = valueArray[keyIndex];
			if (useManualIndex) {
				// with manual indexes don't shift entries, will cause manually specified
				// indexes to be wrong
	...
	public boolean removeAllValues(Collection<?> collection) {
		if (collection == null)
			return false;
		boolean removedAny = false;
		// NOTE: could iterate over valueArray outer and collection inner but value
		// array has no Iterator overhead so do that inner (and nice to reuse
		// removeValue())
		for (Object obj : collection) {
			if (removeValue(obj))
				removedAny = true;
	...
	public void putAll(Map<? extends String, ? extends V> map, Set<String> skipKeys) {
		...
		if (map instanceof LiteStringMap) {
			...
			if (useManualIndex) {
				...
				if (keyArray.length <= lsm.lastIndex)
					growArrays(lsm.lastIndex);
			}
			for (int i = 0; i <= lsm.lastIndex; i++) {
				if (skipKeys != null && skipKeys.contains(lsm.keyArray[i]))
					continue;
				if (useManualIndex) {
	...
	public int hashCode() {
		if (mapHash == 0) {
			// NOTE: this mimics the HashMap implementation from AbstractMap.java for the
			// outer (add entry hash codes) and HashMap.java for the Map.Entry impl
			for (int i = 0; i <= lastIndex; i++) {
				mapHash += (keyArray[i] == null ? 0 : keyArray[i].hashCode())
						^ (valueArray[i] == null ? 0 : valueArray[i].hashCode());
			}
		}
		return mapHash;
	}
	...
	public boolean equals(Object o) {
		if (o instanceof LiteStringMap) {
			...
			if (lastIndex != lsm.lastIndex)
				return false;
			for (int i = 0; i <= lastIndex; i++) {
				// identity compare of interned String keys, if equal the value in the other LSM
				// is conveniently at the same index
				if (keyArray[i] == lsm.keyArray[i]) {
					if (!Objects.equals(valueArray[i], lsm.valueArray[i]))
						return false;
				} else {
					Object value = lsm.getByIString(keyArray[i]);
					if (!Objects.equals(valueArray[i], value))
						return false;
				}
			}
			return true;
	...
	protected Object clone() {
		return new LiteStringMap<V>(this);
	}
	public LiteStringMap<V> cloneLite() {
		return new LiteStringMap<V>(this);
	}
	...
	public int compareTo(Map<String, ? extends V> that) {
		...
		if (that instanceof LiteStringMap) {
			...
			for (int i = 0; i <= lastIndex; i++) {
				...
				// identity compare of interned String keys, if equal the value in the other LSM
				// is conveniently at the same index
				Comparable thatVal = keyArray[i] == lsm.keyArray[i]
						? (Comparable) lsm.valueArray[i]
						: (Comparable) lsm.getByIString(keyArray[i]);
	...
	public static class KeySetWrapper implements Set<String> {
		...
		public boolean containsAll(Collection<?> collection) {
			if (collection == null)
				return false;
			for (Object obj : collection)
				if (obj == null || lsm.findIndex(obj.toString()) == -1)
					return false;
			return true;
		}
	...
	public static class EntryWrapper<V> implements Entry<String, V> {
		...
		public V getValue() {
			...
			if (!Objects.equals(key, keyCheck))
				curIndex = lsm.findIndex(key);
			if (curIndex == -1)
				return null;
			return lsm.valueArray[curIndex];
		}

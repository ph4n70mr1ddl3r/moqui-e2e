...
public class ContextStack implements Map<String, Object> {
	...
	private static class ContextInfo {
		...
		int stackIndex;
		ContextInfo(Map<String, Object>[] stackArray, int stackIndex) {
	...
	/**
	 * Push (save) the entire context, ie the whole Map stack, to create an isolated
	 * empty context.
	 */
	...
	/**
	 * Pop (restore) the entire context, ie the whole Map stack, undo isolated empty
	 * context and get the original one.
	 */
	public ContextStack popContext() {
		if (contextStack == null || contextStack.size() == 0)
			throw new IllegalStateException("Cannot pop context, no context pushed");
	...
	/**
	 * Puts an existing Map on the top of the stack (top meaning will override lower
	 * layers on the stack)
	 * 
	 * @param existingMap
	 *            An existing Map
	...
	public ContextStack push(Map<String, Object> existingMap) {
		if (existingMap == null)
			throw new IllegalArgumentException("Cannot push null as an existing Map");
	...
	/**
	 * Remove and returns the Map from the top of the stack (the local context). If
	 * there is only one Map on the stack it returns null and does not remove it.
	 *
	...
	/**
	 * Add an existing Map as the Root Map, ie on the BOTTOM of the stack meaning it
	 * will be overridden by other Maps on the stack
	 * 
	 * @param existingMap
	 *            An existing Map
	 */
	public void addRootMap(Map<String, Object> existingMap) {
		if (existingMap == null)
			throw new IllegalArgumentException("Cannot add null as an existing Map");
		if (includeContext && existingMap.containsKey("context"))
			throw new IllegalArgumentException("Cannot push existing Map containing key 'context', reserved key");
	...
	/**
	 * Creates a ContextStack object that has the same Map objects on its stack (a
	 * shallow clone). Meant to be used to enable a situation where a parent and
	 * child context are operating simultaneously using two different ContextStack
	 * objects, but sharing the Maps between them.
	 *
	...
	public ContextStack clone() throws CloneNotSupportedException {
		...
		if (contextStack != null) {
			...
			for (ContextInfo ci : contextStack)
				newStack.contextStack.add(ci.cloneInfo());
		}
	...
	@Override
	public int size() {
		// use the keySet since this gets a set of all unique keys for all Maps in the
		// stack
		Set keys = keySet();
		return keys.size();
	}
	...
	public boolean isEmpty() {
		for (int i = stackIndex; i >= 0; i--) {
			if (stackArray[i] != null && !stackArray[i].isEmpty())
				return false;
		}
		return true;
	}
	...
	public boolean containsKey(Object key) {
		for (int i = stackIndex; i >= 0; i--) {
			if (stackArray[i] != null && stackArray[i].containsKey(key))
				return true;
		}
		return false;
	}
	...
	public boolean containsValue(Object value) {
		// this keeps track of keys looked at for values at each level of the stack so
		// that the same key is not
		...
		// maybe do simpler but not as correct? for (int i = stackIndex; i >= 0; i--) {
		// if (stackArray[i] != null && stackArray[i].containsValue(value)) return true;
		// }
	}

	/**
	 * For faster access to multiple entries; do not write to this Map or use when
	 * any changes to ContextStack are possible
	 */
	public Map<String, Object> getCombinedMap() {
		...
		for (int i = 0; i <= stackIndex; i++) {
			if (stackArray[i] != null)
				combinedMap.putAll(stackArray[i]);
		}
		return combinedMap;
	}
	...
	public Object getByString(String key) {
		...
		if (includeContext && "context".equals(key))
			return this;
	...
	public Object put(String key, Object value) {
		if (includeContext && "context".equals(key))
			throw new IllegalArgumentException("Cannot put with key 'context', reserved key");

		if (stackArray[stackIndex] == null)
			stackArray[stackIndex] = new HashMap<>();
	...
	public @Nonnull Set<String> keySet() {
		...
		for (int i = stackIndex; i >= 0; i--)
			if (stackArray[i] != null)
				resultSet.addAll(stackArray[i].keySet());
	...
	@Override
	public int hashCode() {
		return Arrays.deepHashCode(stackArray);
	}
	@Override
	public boolean equals(Object o) {
		return !(o == null || o.getClass() != this.getClass())
				&& Arrays.deepEquals(stackArray, ((ContextStack) o).stackArray);
	}
}

...
public class StringUtilities {
	...
	private static final String[] SCALES = new String[]{"", "thousand", "million", "billion", "trillion", "quadrillion",
			"quintillion", "sextillion"};
	private static final String[] SUBTWENTY = new String[]{"", "one", "two", "three", "four", "five", "six", "seven",
	...
	public static String elementValue(Element element) {
		...
		if (textNode.getNodeType() == org.w3c.dom.Node.CDATA_SECTION_NODE
				|| textNode.getNodeType() == org.w3c.dom.Node.TEXT_NODE)
		...
		for (int i = 0; i < newValue.length(); i++) {
			...
			switch (curChar) {
				...
				case 0x19 :
					...
					break;
				default :
					if (DefaultGroovyMethods.compareTo(curChar, 0x20) < 0 && curChar != 0x9 && curChar != 0xA
							&& curChar != 0xD) {
						// the only valid values < 0x20 are 0x9 (tab), 0xA (newline), 0xD (carriage
						// return)
	...
	/**
	 * See if contains only characters allowed by URLDecoder, if so doesn't need to
	 * be encoded or is already encoded
	 */
	public static boolean isUrlDecoderSafe(String text) {
		...
		// '+' is treated as space, so allow but means we can't detect if already
		// encoded vs doesn't need to be encoded
		if (text == null)
			return true;
		// NOTE: expect mostly shorter strings to charAt() faster than
		// text.toCharArray() and chars[i]; more memory efficient too
	...
	public static ArrayList<String> pathStringToList(String path, int skipSegments) {
		...
		for (int i = skipSegments; i < pathArray.length; i++) {
			...
			if (pathSegment == null || pathSegment.isEmpty())
				continue;
			try {
				pathSegment = URLDecoder.decode(pathSegment, "UTF-8");
			} catch (Exception e) {
				if (logger.isTraceEnabled())
					logger.trace("Error decoding screen path segment ${pathSegment}", e);
			}
	...
	public static String camelCaseToPretty(String camelCase) {
		...
		for (String part : camelCase.split("(?=[A-Z0-9\\.#])")) {
			...
			if (Character.isLowerCase(firstChar))
				part = Character.toUpperCase(firstChar) + part.substring(1);
	...
	public static boolean isAlphaNumeric(String str, String allowedChars) {
		...
		for (int i = 0; i < strChars.length; i++) {
			char c = strChars[i];
			if (!Character.isLetterOrDigit(c) && (allowedChars == null || allowedChars.indexOf(c) == -1))
				return false;
		}
		return true;
	}
	...
	public static String decodeFromXml(String original) {
		...
		while (pos < newValue.length() && pos >= 0) {
			...
			// logger.warn("========= pos=${pos}, entityName=${entityName},
			// replaceChar=${replaceChar}")
			if (replaceChar != null)
				newValue.replace(pos, scIndex + 1, replaceChar);
	...
	public static String cleanStringForJavaName(String original) {
		...
		for (int i = 0; i < origChars.length; i++) {
			...
			// remove dots too, get down to simple class name to work best with Groovy class
			// compiling and loading
			// if (curChar == '.') { cleanChars[i] = '.'; isIdentifierStart = true;
			// continue; }
			// also don't allow $ as groovy blows up on it with class compile/load
			if (curChar != '$' && (isIdentifierStart
					? Character.isJavaIdentifierStart(curChar)
	...
	public static String getExpressionClassName(String expression) {
		...
		for (int i = 0; i < hashLength; i++)
			cnChars[i + 1] = hashCode.charAt(i);
		for (int i = 0; i < copyChars; i++) {
			char exprChar = expression.charAt(i);
			if (exprChar == '$' || !Character.isJavaIdentifierPart(exprChar))
				exprChar = '_';
	...
	public static String elasticQueryAutoWildcard(CharSequence query, boolean allFieldPrefix) {
		// TODO: would be nice to somehow parse the query string, matching parentheses
		// and quotes, and add *: for the field if none for each term
		if (query == null)
			return "*";
		...
		for (int i = 0; i < querySplit.length; i++) {
			...
			if (sb.length() > 0)
				sb.append(' ');
			if (!isEsWord && allFieldPrefix && noEsChars)
				sb.append("*:");
			sb.append(term);
			if (!isEsWord && noEsChars)
				sb.append('*');
		}
		return sb.toString();

		/*
		 * based on old code: if (term) { termSb.append((term.split(' ') as
		 * List).collect({ it.matches(/[^*:\\?_~\/\.\[\]\{\}+?*><="^-]* /) ? (it + '*')
		 * : it }).join(' ')) } else { termSb.append('*') }
		 * 
		 * <if condition="queryString &amp;&amp; isAlphaNumeric(queryString, ' *?')">
		 * <set field="queryString"
		 * from="queryString.split(' ').collect({ (!it || it in ['AND', 'OR', 'NOT']) ? it : '*:' + (it.contains('*') || it.contains('?') ? it : it + '*') }).join(' ')"
		 * /></if>
		 */
	}
	...
	public static String paddedNumber(long number, Integer desiredLength) {
		...
		while (desiredLength > outStrBfr.length())
	...
	public static String paddedString(String input, Integer desiredLength, Character padChar, boolean rightPad) {
		if (!DefaultGroovyMethods.asBoolean(padChar))
			padChar = ' ';
		if (input == null)
			input = "";
		...
		while (desiredLength > outStrBfr.length())
			if (rightPad)
				outStrBfr.append(padChar);
			else
				outStrBfr.insert(0, padChar);
		return outStrBfr.toString();
	}
	...
	public static MaskFormatter masker(String mask, String placeholder) throws ParseException {
		...
		if (placeholder != null && !placeholder.isEmpty()) {
			if (placeholder.length() == 1)
				formatter.setPlaceholderCharacter(placeholder.charAt(0));
			else
	...
	public static String getRandomString(int length, BaseNCodec baseNCodec) {
		if (baseNCodec == null)
			baseNCodec = org.apache.commons.codec.binary.Base64.builder().setUrlSafe(true).get();
	 ...
	 * @param useAnd
	 *            true if you want to use the word 'and' in the text (eleven
	...
	public static String removeChar(String orig, char ch) {
		...
		// NOTE: this seems to run pretty slow, plain replace might be faster, but
		// avoiding its use anyway (in ServiceFacadeImpl for SECA rules)

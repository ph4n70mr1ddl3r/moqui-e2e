...
public class ObjectUtilities {
	...
	/**
	 * Populate a Map with public fields and Java Bean style properties (using
	 * java.beans.BeanInfo)
	 */
	public static Map<String, Object> objectToMap(Object bean) {
		...
		for (int fi = 0; fi < fields.length; fi++) {
			...
			try {
				...
				// do nothing, maybe log at some point if we care enough and are okay with the
				// potential performance hit
			}
		}

		/*
		 * commenting for now, seems to call a bunch of undesired methods, will need
		 * some work to filter them out: try { BeanInfo beanInfo =
		 * Introspector.getBeanInfo(clazz); PropertyDescriptor[] propertyDescriptors =
		 * beanInfo.getPropertyDescriptors(); for (int pi = 0; pi <
		 * propertyDescriptors.length; pi++) { PropertyDescriptor propertyDescriptor =
		 * propertyDescriptors[pi]; Method readMethod =
		 * propertyDescriptor.getReadMethod(); if (readMethod != null) { try {
		 * map.put(propertyDescriptor.getName(), readMethod.invoke(bean)); } catch
		 * (IllegalAccessException | InvocationTargetException e) { // nothing again } }
		 * } } catch (IntrospectionException e) { // nothing again }
		 */

		// this gets picked up automatically, just remove at the end, faster than
	...
	public static Object basicConvert(Object value, final String javaType) {
		...
		if (origClass.equals(theClass))
			return value;
		try {
			if (origString) {
				if (theClass == Boolean.class) {
					// for non-empty String to Boolean don't use normal not-empty rules, look for
					// "true", "false", etc
					String valStr = value.toString();
					return "true".equalsIgnoreCase(valStr) || "y".equalsIgnoreCase(valStr);
				} else if (theClass == Integer.class) {
					// groovy does funny things with single character strings, ie gets the int value
					// of the single char, so do it ourselves
			...
			if (theClass == Date.class && value instanceof Timestamp) {
				...
				// logger.warn("Converted " + value + " of type " + origClass.getName() + " to "
				// + DefaultGroovyMethods.asType(value, theClass) + " for class " +
				// theClass.getName());
	...
	public static boolean compareLike(Object value1, Object value2) {
		...
		if (value1 instanceof CharSequence && value2 instanceof CharSequence) {
			// first escape the characters that would be interpreted as part of the regular
			// expression
	...
	@SuppressWarnings("unchecked")
	public static boolean compare(Object field, final String operator, final String value, Object toField,
			String format, final String type) {
		if (isEmpty(toField))
			toField = value;
		...
		if (logger.isTraceEnabled())
			logger.trace("Compare result [" + result + "] for field [" + field + "] operator [" + operator + "] value ["
					+ value + "] toField [" + toField + "] type [" + type + "]");
		return result;
	}
	...
	public static Comparable makeComparable(final Object obj) {
		...
		if (obj instanceof Comparable)
			...
			throw new IllegalArgumentException(
					"Object of type " + obj.getClass().getName() + " is not Comparable, cannot compare");
	}

	public static int countChars(String s, boolean countDigits, boolean countLetters, boolean countOthers) {
		// this seems like it should be part of some standard Java API, but I haven't
		// found it
	...
	public static String toPlainString(Object obj) {
		...
		// BigDecimal toString() uses scientific notation, annoying, so use
		// toPlainString()
		if (objClass == BigDecimal.class)
			return ((BigDecimal) obj).toPlainString();
		// handle the special case of timestamps used for primary keys, make sure we
		// avoid TZ, etc problems
	...
	/**
	 * Like the Groovy empty except doesn't consider empty 0 value numbers, false
	 * Boolean, etc; only null values, 0 length String (actually CharSequence to
	 * include GString, etc), and 0 size Collection/Map are considered empty.
	 */
	public static boolean isEmpty(Object obj) {
		if (obj == null)
			return true;
		/*
		 * faster not to do this Class objClass = obj.getClass(); // some common direct
		 * classes if (objClass == String.class) return ((String) obj).length() == 0; if
		 * (objClass == GString.class) return ((GString) obj).length() == 0; if
		 * (objClass == ArrayList.class) return ((ArrayList) obj).size() == 0; if
		 * (objClass == HashMap.class) return ((HashMap) obj).size() == 0; if (objClass
		 * == LinkedHashMap.class) return ((HashMap) obj).size() == 0; // hopefully less
		 * common sub-classes
		 */
		if (obj instanceof CharSequence)
	...
	public static boolean isInstanceOf(Object theObjectInQuestion, String javaType) {
		...
		if (theClass == null)
			throw new IllegalArgumentException("Cannot find class for type: " + javaType);
	...
	public static Number addNumbers(Number a, Number b) {
		...
		if (bClass == BigDecimal.class) {
			if (aClass == BigDecimal.class)
				return ((BigDecimal) b).add((BigDecimal) a);
			else
				return ((BigDecimal) b).add(new BigDecimal(a.toString()));
		}

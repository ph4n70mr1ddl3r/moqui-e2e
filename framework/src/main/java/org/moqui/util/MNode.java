...
/**
 * An alternative to groovy.util.Node with methods more type safe and generally
 * useful in Moqui.
 */
...
public class MNode
	...
	public static MNode parse(File fl) throws BaseException {
		...
		MNode cached = parsedNodeCache.get(location);
		if (cached != null && cached.lastModified >= fl.lastModified())
			return cached;
		...
		try {
			...
			try {
				...
				logger.error("Error closing XML file at " + fl.getPath(), e);
			}
		}
	}
	...
	// NOTE: start with small capacity, optimize for memory use vs put overhead to
	// grow as mostly used for config kept long term
		...
		for (Object entryObj : attrEntries)
			if (entryObj instanceof Map.Entry) {
				...
				if (entry.getKey() != null)
					attributeMap.put(entry.getKey().toString(),
	...
	public MNode setFileLocation(String location) {
	...
	/**
	 * If name starts with an ampersand (@) then get an attribute, otherwise get a
	 * list of child nodes with the given name.
	 */
	...
	public String attribute(String attrName) {
		...
		if (systemExpandAttributes && attrValue != null && attrValue.contains("${")) {
			...
			// system properties and environment variables don't generally change once
			// initial init is done, so save expanded value
	...
	public boolean hasAncestor(String nodeName) {
		if (parentNode == null)
			return false;
		if (nodeName == null || nodeName.isEmpty() || nodeName.equals(parentNode.nodeName))
			return true;
	...
	public ArrayList<MNode> children(String name) {
		if (childList == null)
			childList = new ArrayList<>(4);
		if (childrenByName == null)
			childrenByName = new HashMap<>(4);
		if (name == null)
			return childList;
		...
		for (int i = 0; i < childListSize; i++) {
			...
			if (name.equals(curChild.nodeName))
	...
	public ArrayList<MNode> children(Closure<Boolean> condition) {
		...
		for (int i = 0; i < childListSize; i++) {
			MNode curChild = childList.get(i);
			if (condition == null || condition.call(curChild))
				curList.add(curChild);
		}
		return curList;
	}
	...
	public Map<String, ArrayList<MNode>> getChildrenByName() {
		...
		for (int i = 0; i < childListSize; i++) {
			...
			if (existingList != null) {
				if (existingList.size() > 0 && !allByName.containsKey(name))
					allByName.put(name, existingList);
				continue;
			}
	...
	/**
	 * Search all descendants for nodes matching any of the names, return a Map with
	 * a List for each name with nodes found or empty List if no nodes found
	 */
	...
	public MNode first(String name) {
		...
		if (nameChildren.size() > 0)
			return nameChildren.get(0);
		return null;

		/*
		 * with cache in children(name) that is faster than searching every time here:
		 * int childListSize = childList.size(); for (int i = 0; i < childListSize; i++)
		 * { MNode curChild = childList.get(i); if (name.equals(curChild.nodeName))
		 * return curChild; } return null;
		 */
	}
	...
	public int firstIndex(String name) {
		...
		for (int i = 0; i < childListSize; i++) {
			MNode curChild = childList.get(i);
			if (name.equals(curChild.getName()))
				return i;
		}
		return -1;
	}
	...
	public int firstIndex(MNode child) {
		...
		for (int i = 0; i < childListSize; i++) {
			MNode curChild = childList.get(i);
			if (!child.getName().equals(curChild.getName()))
				continue;
			if (child.getAttributes().equals(curChild.getAttributes()))
				return i;
		}
		return -1;
	}
	...
	public MNode deepCopy(MNode parent) {
		...
		if (fileLocation != null)
			newNode.fileLocation = fileLocation;
		if (childList != null) {
			int childListSize = childList.size();
			if (childListSize > 0) {
	...
	public MNode replace(int index, MNode child) {
		if (childList == null || childList.size() < index)
			throw new IllegalArgumentException(
					"Index " + index + " not valid, size is " + (childList == null ? 0 : childList.size()));
	...
	/**
	 * Merge a single child node with the given name from overrideNode if it has a
	 * child with that name.
	 *
	 * If this node has a child with the same name copies/overwrites attributes from
	 * the overrideNode's child and if overrideNode's child has children the
	 * children of this node's child will be replaced by them.
	 *
	 * Otherwise appends a copy of the override child as a child of the current
	 * node.
	 */
		...
		if (childBaseNode != null) {
			...
			if (childOverrideNode.childList != null && childOverrideNode.childList.size() > 0) {
				if (childBaseNode.childList != null) {
					if (childBaseNode.childrenByName != null)
						childBaseNode.childrenByName.clear();
			...
			if (childrenByName != null)
				childrenByName.remove(childOverrideNode.nodeName);
			if (childList == null)
				childList = new ArrayList<>();
	...
	public void mergeChildWithChildKey(MNode overrideNode, String childName, String grandchildName,
			String keyAttributeName, Closure grandchildMerger) {
		MNode overrideChildNode = overrideNode.first(childName);
		if (overrideChildNode == null)
			return;
		...
		if (baseChildNode != null) {
			...
			if (childrenByName != null)
				childrenByName.remove(overrideChildNode.nodeName);
			if (childList == null)
				childList = new ArrayList<>();
	...
	/**
	 * Merge attributes and child nodes from overrideNode into this node, matching
	 * on childNodesName and optionally the value of the attribute in each named by
	 * keyAttributeName.
	 *
	 * Always copies/overwrites attributes from override child node, and merges
	 * their child nodes using childMerger or if null the default merge of removing
	 * all children under the child of this node and appending copies of the
	 * children of the override child node.
	 */
	public void mergeNodeWithChildKey(MNode overrideNode, String childNodesName, String keyAttributeName,
			Closure childMerger) {
	...
	public void mergeChildrenByKey(MNode overrideNode, String childNodesName, String keyAttributeName,
			Closure childMerger) {
	...
	public void mergeChildrenByKeys(MNode overrideNode, String childNodesName, Closure childMerger,
		...
		for (int curOc = 0; curOc < overrideChildrenSize; curOc++) {
			...
			for (int i = 0; i < childListSize; i++) {
				MNode curChild = childList.get(i);
				if (!curChild.getName().equals(childNodesName))
					continue;
				if (keyAttributeNames == null || keyAttributeNames.length == 0) {
				...
				for (int ai = 0; ai < keyAttributeNames.length; ai++) {
					...
					if (!keyValue.equals(curChild.attribute(keyAttributeNames[ai])))
						allMatch = false;
				}
				if (allMatch) {
					childBaseNode = curChild;
			...
			if (childBaseNode != null) {
				...
				if (childMerger != null) {
					...
					// do the default child merge: remove current nodes children and replace with a
					// copy of the override node's children
					if (childBaseNode.childList != null) {
						if (childBaseNode.childrenByName != null)
							childBaseNode.childrenByName.clear();
						childBaseNode.childList.clear();
	...
	private void addToSb(StringBuilder sb, int level) {
		...
		if ((childText != null && childText.length() > 0) || (childList != null && childList.size() > 0)) {
			sb.append(">");
			if (childText != null)
				sb.append("<![CDATA[").append(childText).append("]]>");
			if (childList != null && childList.size() > 0) {
	...
	private static class MNodeXmlHandler extends DefaultHandler {
		...
		MNodeXmlHandler(boolean rootOnly, String fileLocation) {
		...
		public void startElement(String ns, String localName, String qName, Attributes attributes) {
			...
			// logger.info("startElement ns [${ns}], localName [${localName}] qName
			// [${qName}]")
			if (curNode == null) {
			...
			if (fileLocation != null)
				curNode.setFileLocation(fileLocation);
		...
		public void endElement(String ns, String localName, String qName) {
			...
			if (!qName.equals(curNode.nodeName))
				throw new IllegalStateException(
						"Invalid close element " + qName + ", was expecting " + curNode.nodeName);
			if (curText != null) {
				String curString = curText.toString().trim();
				if (curString.length() > 0)
					curNode.childText = curString;
			}
	...
	public TemplateModel get(String s) {
		if (s == null)
			return null;
		// first try the attribute and children caches, then if not found in either pick
		// it apart and create what is needed
		ConcurrentHashMap<String, TemplateModel> localAttrAndChildren = ftlAttrAndChildren != null
				? ftlAttrAndChildren
				: makeAttrAndChildrenByName();
		TemplateModel attrOrChildWrapper = localAttrAndChildren.get(s);
		if (attrOrChildWrapper != null)
			return attrOrChildWrapper;
		if (knownNullAttributes != null && knownNullAttributes.containsKey(s))
			return null;

		// at this point we got a null value but attributes and child nodes were
		// pre-loaded so return null or empty list
		if (s.startsWith("@")) {
			if ("@@text".equals(s)) {
				// if we got this once will get it again so add @@text always, always want
				// wrapper though may be null
				...
				// TODO: handle other special hashes? (see
				// http://www.freemarker.org/docs/xgui_imperative_formal.html)
			} else {
				String attrName = s.substring(1, s.length());
				String attrValue = attributeMap.get(attrName);
				if (attrValue == null) {
					if (knownNullAttributes == null)
						knownNullAttributes = new ConcurrentHashMap<>();
	...
	private synchronized ConcurrentHashMap<String, TemplateModel> makeAttrAndChildrenByName() {
		if (ftlAttrAndChildren == null)
			ftlAttrAndChildren = new ConcurrentHashMap<>();
		return ftlAttrAndChildren;
	}
	@Override
	public boolean isEmpty() {
		return attributeMap.isEmpty() && (childList == null || childList.isEmpty())
				&& (childText == null || childText.length() == 0);
	}

	// TemplateHashModelEx methods
	@Override
	public TemplateCollectionModel keys() throws TemplateModelException {
		return new SimpleCollection(attributeMap.keySet(), wrapper);
	}
	@Override
	public TemplateCollectionModel values() throws TemplateModelException {
		return new SimpleCollection(attributeMap.values(), wrapper);
	}
	...
	private FtlNodeListWrapper getSequenceList() {
		// Looks like attributes should NOT go in the FTL children list, so just use the
		// node.children()
		if (allChildren == null)
			allChildren = (childText != null && childText.length() > 0)
					? new FtlNodeListWrapper(childText, this)
					: new FtlNodeListWrapper(childList, this);
		return allChildren;
	}
	...
	private static class FtlAttributeWrapper
			implements
				TemplateNodeModel,
				TemplateSequenceModel,
				AdapterTemplateModel,
		...
		public TemplateModel get(int i) {
			...
			throw new IndexOutOfBoundsException(
					"Attribute node only has 1 value. Tried to get index [${i}] for attribute [${key}]");
		}
		@Override
		public int size() {
			return 1;
	...
	private static class FtlTextWrapper
			implements
				TemplateNodeModel,
				TemplateSequenceModel,
				AdapterTemplateModel,

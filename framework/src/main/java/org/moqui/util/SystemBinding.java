...
/**
 * Simple class for evaluating expressions to get System properties and
 * environment variables by string expansion
 */
public class SystemBinding extends Binding {
	...
	public static String getPropOrEnv(String name) {
		...
		// no luck? try replacing underscores with dots (dots used for map access in
		// Groovy so need workaround)
		String dotName = null;
		if (name.contains("_")) {
		...
		if (isTraceEnabled)
			logger.trace("No '" + name + (dotName != null ? "' (or '" + dotName + "')" : "'")
	...
	public static String expand(String value) {
		...
		if (result == null)
			return ""; // should never happen, always at least empty String

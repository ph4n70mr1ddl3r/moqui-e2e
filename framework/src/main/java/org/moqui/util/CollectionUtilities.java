...
/**
 * These are utilities that should exist elsewhere, but I can't find a good
 * simple library for them, and they are stupid but necessary for certain
 * things.
 */
...
public class CollectionUtilities {
	...
	public static HashMap<String, Object> toHashMap(Object... keyValues) {
		if (keyValues.length % 2 != 0)
			throw new IllegalArgumentException("Must have even number of arguments in name, value pairs");
	...
	public static ArrayList<Object> getMapArrayListValues(ArrayList<Map<Object, Object>> mapList, Object key,
		...
		for (int i = 0; i < mapListSize; i++) {
			...
			if (excludeNullValues && curVal == null)
				continue;
			valList.add(curVal);
		}
		return valList;
	...
	/**
	 * Filter theList (of Map) using fieldValues; if exclude=true remove matching
	 * items, else keep only matching items
	 */
	public static void filterMapList(List<Map> theList, Map<String, Object> fieldValues, boolean exclude) {
		if (theList == null || fieldValues == null)
			return;
	...
	public static List<Map> filterMapListByDate(List<Map> theList, String fromDateName, String thruDateName,
			Timestamp compareStamp) {
		if (theList == null || theList.size() == 0)
			return theList;

		if (fromDateName == null || fromDateName.isEmpty())
			fromDateName = "fromDate";
		if (thruDateName == null || thruDateName.isEmpty())
			thruDateName = "thruDate";
		// no access to ec.user here, so this should always be passed in, but just in
		// case
		if (compareStamp == null)
			compareStamp = new Timestamp(System.currentTimeMillis());
		...
		while (theIterator.hasNext()) {
			...
			if (thruDate != null && compareStamp.compareTo(thruDate) >= 0)
				theIterator.remove();
		}
		return theList;
	}

	public static void filterMapListByDate(List<Map> theList, String fromDateName, String thruDateName,
			Timestamp compareStamp, boolean ignoreIfEmpty) {
		if (ignoreIfEmpty && compareStamp == null)
			return;
		filterMapListByDate(theList, fromDateName, thruDateName, compareStamp);
	}

	/**
	 * Order list elements in place (modifies the list passed in), returns the list
	 * for convenience
	 */
	public static List<Map<String, Object>> orderMapList(List<Map<String, Object>> theList,
			List<? extends CharSequence> fieldNames) {
		return orderMapList(theList, fieldNames, null);
	}
	public static List<Map<String, Object>> orderMapList(List<Map<String, Object>> theList,
			List<? extends CharSequence> fieldNames, Boolean nullsLast) {
		if (fieldNames == null)
		...
		if (theList != null && fieldNames.size() > 0)
			theList.sort(new MapOrderByComparator(fieldNames).nullsLast(nullsLast));
		return theList;
	}
	...
	public static class MapOrderByComparator implements Comparator<Map> {
		...
		public int compare(Map map1, Map map2) {
			...
			for (int i = 0; i < fieldNameArray.length; i++) {
				...
				// NOTE: nulls go earlier in the list for ascending, later in the list for
				// !ascending
				if (value1 == null) {
					if (value2 != null)
		...
		public boolean equals(Object obj) {
			return obj instanceof MapOrderByComparator
					&& Arrays.equals(fieldNameArray, ((MapOrderByComparator) obj).fieldNameArray);
		}
	...
	/**
	 * For a list of Map find the entry that best matches the fieldsByPriority
	 * Ordered Map; null field values in a Map in mapList match against any value
	 * but do not contribute to maximal match score, otherwise value for each field
	...
	public static Map<String, Object> findMaximalMatch(List<Map<String, Object>> mapList,
		...
		for (Map<String, Object> curMap : mapList) {
			...
			for (int i = 0; i < numFields; i++) {
				...
				// if curMap value is null skip field (null value in Map means allow any match
				// value
				...
				// add to score based on index (lower index higher score), also add numFields so
				// more fields matched weights higher
	...
	public static void addToBigDecimalInMap(Object key, BigDecimal value, Map theMap) {
		...
		if (curObj == null) {
			...
			BigDecimal curVal;
			if (curObj instanceof BigDecimal)
				curVal = (BigDecimal) curObj;
			else
				curVal = new BigDecimal(curObj.toString());
	...
	/**
	 * Returns Map with total, squaredTotal, count, average, stdDev, maximum;
	 * fieldName field in Maps must have type BigDecimal; if count of non-null
	 * fields is less than 2 returns null as cannot calculate a standard deviation
	 */
	public static Map<String, BigDecimal> stdDevMaxFromMapField(List<Map<String, Object>> dataList, String fieldName,
		...
		BigDecimal stdDev = new BigDecimal(
				Math.sqrt(Math.abs(squaredTotal.doubleValue() - ((totalDouble * totalDouble) / count)) / (count - 1)));
	...
	/**
	 * Find a field value in a nested Map containing fields, Maps, and Collections
	 * of Maps (Lists, etc)
	 */
	...
	/**
	 * Find all values of a named field in a nested Map containing fields, Maps, and
	 * Collections of Maps (Lists, etc)
	 */
	public static void findAllFieldsNestedMap(String key, Map theMap, Set<Object> valueSet) {
		if (theMap instanceof LiteStringMap) {
			...
			for (int i = 0; i < lsmSize; i++) {
				...
				if (!foundKey && keyLength == curKey.length() && keyHashCode == curKey.hashCode()
						&& curKey.equals(key)) {
					foundKey = true;
					if (curValue != null)
						valueSet.add(curValue);
				}
	...
	/**
	 * Creates a single Map with fields from the passed in Map and all nested Maps
	 * (for Map and Collection of Map entry values)
	 */
	@SuppressWarnings("unchecked")
		...
		for (Map.Entry<String, Object> entry : theMap.entrySet()) {
			...
			if (value instanceof Map) {
				...
				for (Object colValue : (Collection<?>) value) {
					if (colValue instanceof Map) {
						output.putAll(
								flattenNestedMapWithKeys((Map<String, Object>) colValue, newKey + "[" + index + "]"));
					} else {
					...
					if (value instanceof CharSequence) {
						...
						if (baseValue != null && baseValue instanceof Collection) {
							...
							for (Object overrideObj : overrideCol) {
								// NOTE: if we have a Collection of Map we have no way to merge the Maps without
								// knowing the 'key' entries to use to match them
	...
	/**
	 * Removes entries with a null value from the Map, returns the passed in Map for
	 * convenience (does not clone before removes!).
	 */
	@SuppressWarnings("unchecked")
	...
	public static String nodeText(Object nodeObj) {
		if (!DefaultGroovyMethods.asBoolean(nodeObj))
			return "";
		Node theNode = null;
	...
	public static void paginateList(String listName, String pageListName, Map<String, Object> context) {
		if (pageListName == null || pageListName.isEmpty())
			pageListName = listName;
	...
	public static List paginateList(List theList, String pageListName, Map<String, Object> context) {
		...
		if (context.containsKey(pageListName + "AlreadyPaginated"))
			return theList;
	...
	public static Map paginateParameters(int listSize, String pageListName, Map<String, Object> context) {
		...
		if (!ObjectUtilities.isEmpty(pageSizeObj)) {
			try {
				...
				/* just use the default above */
			}
		}
		if (pageSize < 0)
			pageSize = 20;

		// NOTE: if context has a *Count field don't calc and set values, assume are
		// already in place
		if (context.get(pageListName + "Count") == null) {
			int count = listSize;
			// calculate the pagination values
			int maxIndex = (new BigDecimal(count - 1)).divide(new BigDecimal(pageSize), 0, RoundingMode.DOWN)
					.intValue();
			int pageRangeLow = (pageIndex * pageSize) + 1;
			if (pageRangeLow > count)
				pageRangeLow = count + 1;

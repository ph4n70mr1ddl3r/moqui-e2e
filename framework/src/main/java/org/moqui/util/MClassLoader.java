...
/**
 * A caching ClassLoader that allows addition of JAR files and class directories
 * to the classpath at runtime.
 *
 * This loads resources from its class directories and JAR files first, then
 * tries the parent. This is not the standard approach, but needed for
 * configuration in moqui/runtime and components to override other classpath
 * resources.
 *
 * This loads classes from the parent first, then its class directories and JAR
 * files.
 */
public class MClassLoader extends ClassLoader {
	private static final boolean checkJars = false;
	// rememberClassNotFound causes problems with Groovy that tries to load
	// variations on class names, then creates them, then tries again
	private static final boolean rememberClassNotFound = false;
	private static final boolean rememberResourceNotFound = true;
	// don't track known: with a few tool components in place uses 20MB memory and
	// really doesn't help start/etc time much:
	...
	// This Map contains either a Class or a ClassNotFoundException, cached for fast
	// access because Groovy hits a LOT of
	// weird invalid class names resulting in expensive new ClassNotFoundException
	// instances
	...
	public void addJarFile(JarFile jf, URL jarLocation) {
		...
		while (jeEnum.hasMoreElements()) {
			...
			/*
			 * NOTE: can't do this as classes are defined out of order, end up with
			 * NoClassDefFoundError for dependencies: Class<?> cls = makeClass(className,
			 * jf, je); if (cls != null) classCache.put(className, cls);
			 */
			...
			if (checkJars) {
				try {
					...
					System.out.println(
							"Class " + className + " in jar " + jfName + " already loaded from parent ClassLoader");
	...
	private void findClassFiles(String pathSoFar, File dir) {
		...
		for (int i = 0; i < children.length; i++) {
			...
			if (child.isDirectory()) {
				 ...
				 * NOTE: can't do this as classes are defined out of order, end up with
				 * NoClassDefFoundError for dependencies: Class<?> cls = makeClass(className,
				 * child); if (cls != null) classCache.put(className, cls);
				 */
			}
		}
	}
	...
	protected URL findResource(String resourceName) {
		...
		if (cachedUrl != null)
			return cachedUrl;
		if (rememberResourceNotFound && resourcesNotFound.contains(resourceName))
			return null;

		// Groovy looks for BeanInfo and Customizer groovy resources, even for anonymous
		// scripts and they will never exist
		if (rememberResourceNotFound) {
			if ((resourceName.endsWith("BeanInfo.groovy") || resourceName.endsWith("Customizer.groovy"))
					&& (resourceName.startsWith("script") || resourceName.contains("_actions")
							|| resourceName.contains("_condition"))) {
		...
		for (int i = 0; i < classesDirectoryListSize; i++) {
			...
			try {
				if (testFile.exists() && testFile.isFile())
					resourceUrl = testFile.toURI().toURL();
			} catch (MalformedURLException e) {
				System.out.println("Error making URL for [" + resourceName + "] in classes directory [" + classesDir
						+ "]: " + e.toString());
			}
		}
		...
		if (resourceUrl == null) {
			// NOTE: it is weird for any ClassLoader to throw exceptions for valid resource
			// names, but
			...
			// As a workaround catch that exception, try the grand-parent classloader, and
			// move on...
			try {
				resourceUrl = getParent().getResource(resourceName);
			} catch (Throwable t) {
				System.out.println(
						"Error in findResource() in parent classloader " + getParent().getClass().getCanonicalName()
								+ " for name [" + resourceName + "]: " + t.toString());
				// t.printStackTrace();
				...
				if (grandParent != null) {
					try {
						...
						if (resourceUrl != null)
							System.out.println("Found " + resourceName + " in grand-parent ClassLoader "
									+ grandParent.getClass().getCanonicalName());
					} catch (Throwable t2) {
						System.out.println("Error in findResource() in grand-parent classloader "
								+ grandParent.getClass().getCanonicalName() + " for name [" + resourceName + "]: "
								+ t2.toString());
					}
				}
			}
		...
		if (resourceUrl != null) {
			// System.out.println("finding resource " + resourceName + " got " +
			...
			// for testing to see if resource not found cache is working, should see this
			// once for each not found resource
	...
	public Enumeration<URL> findResources(String resourceName) throws IOException {
		...
		if (cachedUrls != null)
			return Collections.enumeration(cachedUrls);
		...
		for (int i = 0; i < classesDirectoryListSize; i++) {
			...
			try {
				if (testFile.exists() && testFile.isFile())
					urlList.add(testFile.toURI().toURL());
			} catch (MalformedURLException e) {
				System.out.println("Error making URL for [" + resourceName + "] in classes directory [" + classesDir
						+ "]: " + e.toString());
			}
		...
		for (int i = 0; i < jarFileListSize; i++) {
			...
			if (jarEntry != null) {
				try {
					...
					System.out.println(
							"Error making URL for [" + resourceName + "] in jar [" + jarFile + "]: " + e.toString());
				}
			}
		}
		...
		try {
			...
			System.out.println("Error in findResources() in parent classloader "
					+ getParent().getClass().getCanonicalName() + " for name [" + resourceName + "]: " + t.toString());
			// t.printStackTrace();
			...
			if (grandParent != null) {
				try {
					...
					System.out.println("Error in findResources() in grand-parent classloader "
							+ grandParent.getClass().getCanonicalName() + " for name [" + resourceName + "]: "
							+ t2.toString());
				}
			}
		...
		// System.out.println("finding all resources with name " + resourceName + " got
		// " + urlList);
	...
	private Class<?> loadClassInternal(String className, boolean resolve) throws ClassNotFoundException {
		/*
		 * This may not be a good idea, Groovy looks for all sorts of bogus class name
		 * but there may be a reason so not doing this or looking for other patterns:
		 * for (int i = 0; i < ignoreSuffixesSize; i++) { String ignoreSuffix =
		 * ignoreSuffixes.get(i); if (className.endsWith(ignoreSuffix)) {
		 * ClassNotFoundException cfne = new
		 * ClassNotFoundException("Ignoring Groovy style bogus class name " +
		 * className); classCache.put(className, cfne); throw cfne; } }
		 */

		Class<?> c = null;
		try {
			// classes handled opposite of resources, try parent chain first (avoid
			// java.lang.LinkageError)
			ClassLoader cl = getParent();
			int depth = 0;
			/*
			 * When in jetty embedded mode (MoquiStart) first try
			 * org.eclipse.jetty.ee.webapp.WebAppClassLoader then
			 * MoquiStart.StartClassLoader. If in a servlet container, then use the
			 * classloader parents provided by that container.
			 */
				...
				try {
					if (trackKnown) {
						...
						if (c == null) {
							...
							if (jei != null)
								c = makeClass(className, jei.file, jei.entry, jei.jarLocation);
						}
					}
			...
			if (c == null) {
				...
				if (rememberClassNotFound) {
					...
					// Groovy does similar with *Customizer and *BeanInfo; so just don't remember
					// any of these
					// In general it seems that anything with a '$' needs to be excluded
					if (!className.contains("$") && !className.endsWith("Customizer")
	...
	private Class<?> makeClass(String className, JarFile file, JarEntry entry, URL jarLocation) {
		try {
			...
			if (jeBytes == null) {
				...
				// System.out.println("Loading class " + className + " from " + entry.getName()
				// + " in " + file.getName());
				return defineClass(className, jeBytes, 0, jeBytes.length,
						jarLocation != null ? getProtectionDomain(jarLocation) : pd);
			}
		} catch (Throwable t) {
			System.out.println(
					"Error reading class file " + entry.getName() + " in jar" + file.getName() + ": " + t.toString());
			return null;
		}
	}

...
/**
 * This is a base class that implements a simple way to access the Moqui
 * framework for use in simple deployments where there is nothing available like
 * a webapp or an OSGi component.
 *
 * In deployments where a static reference to the ExecutionContextFactory is not
 * helpful, or not possible, this does not need to be used and the
 * ExecutionContextFactory instance should be referenced and used from somewhere
 * else.
 */
...
public class Moqui {
	...
	private static final ServiceLoader<ExecutionContextFactory> executionContextFactoryLoader = ServiceLoader
			.load(ExecutionContextFactory.class);
	static {
		// only do this if the moqui.init.static System property is true
		if ("true".equals(System.getProperty("moqui.init.static"))) {
			// initialize the activeExecutionContextFactory from configuration using
			// java.util.ServiceLoader
			// the implementation class name should be in:
			// "META-INF/services/org.moqui.context.ExecutionContextFactory"
	...
	public static void dynamicInit(ExecutionContextFactory executionContextFactory) {
		if (activeExecutionContextFactory != null && !activeExecutionContextFactory.isDestroyed())
			throw new IllegalStateException(
					"Active ExecutionContextFactory already in place, cannot set one dynamically.");
	...
	public static <K extends ExecutionContextFactory> K dynamicReInit(Class<K> ecfClass, ServletContext sc)
		...
		// handle Servlet pause then resume taking requests after by removing
		// executionContextFactory attribute
		if (sc.getAttribute("executionContextFactory") != null)
	...
	public static ExecutionContextFactory getExecutionContextFactory() {
		return activeExecutionContextFactory;
	}
	...
	/**
	 * This should be called when it is known a context won't be used any more, such
	 * as at the end of a web request or service execution.
	 */
	...
	/**
	 * This should be called when the process is terminating to clean up framework
	 * and tool operations and resources.
	 */
	...
	/**
	 * This method is meant to be run from a command-line interface and handle data
	 * loading in a generic way.
	 * 
	 * @param argMap
	 *            Arguments, generally from command line, to configure this data
	 *            load.
	 */
	public static void loadData(Map<String, String> argMap) {
		...
		ec.getArtifactExecution().push("loadData", ArtifactExecutionInfo.AT_OTHER, ArtifactExecutionInfo.AUTHZA_ALL,
				false);
		...
		if (argMap.containsKey("location"))
			edl.location(argMap.get("location"));
		if (argMap.containsKey("timeout"))
			edl.transactionTimeout(Integer.valueOf(argMap.get("timeout")));
		if (argMap.containsKey("dummy-fks"))
			edl.dummyFks(true);
		if (argMap.containsKey("raw") || argMap.containsKey("no-fk-create"))
			edl.disableFkCreate(true);
		if (argMap.containsKey("raw") || argMap.containsKey("use-try-insert"))
			edl.useTryInsert(true);
		if (argMap.containsKey("raw") || argMap.containsKey("disable-eeca"))
			edl.disableEntityEca(true);
		if (argMap.containsKey("raw") || argMap.containsKey("disable-audit-log"))
			edl.disableAuditLog(true);
		if (argMap.containsKey("raw") || argMap.containsKey("disable-data-feed"))
			edl.disableDataFeed(true);

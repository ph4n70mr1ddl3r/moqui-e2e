...
public class UrlResourceReference extends ResourceReference {
	...
	public UrlResourceReference(File file) {
		...
		try {
			...
			throw new BaseException("Error creating URL for file " + file.getAbsolutePath(), e);
		}
	}
	...
	public ResourceReference init(String location) {
		if (location == null || location.isEmpty())
			throw new BaseException("Cannot create URL Resource Reference with empty location");

		if (location.startsWith(runtimePrefix))
			location = location.substring(runtimePrefix.length());

		if (location.startsWith("/") || !location.contains(":")) {
			// no prefix, local file: if starts with '/' is absolute, otherwise is relative
			// to runtime path
			...
			try {
				locationUrl = URI.create("file:" + location).toURL();
			} catch (MalformedURLException | IllegalArgumentException e) {
				throw new BaseException("Invalid file url for location " + location, e);
			}
			isFileProtocol = true;
		} else {
			try {
				...
				try {
					locationUrl = URI.create("file:/" + location).toURL();
				} catch (MalformedURLException | IllegalArgumentException se) {
					throw new BaseException("Invalid url for location " + location, e);
				}
			}
	...
	public File getFile() {
		if (!isFileProtocol)
			throw new IllegalArgumentException(
					"File not supported for resource with protocol [" + locationUrl.getProtocol() + "]");
		if (localFile != null)
			return localFile;
		// NOTE: using toExternalForm().substring(5) instead of toURI because URI does
		// not allow spaces in a filename
	...
	public OutputStream getOutputStream() {
		if (!isFileProtocol) {
			...
			throw new IllegalArgumentException("Write not supported for resource [" + url.toString()
	...
	public String getText() {
		return ObjectUtilities.getStreamText(openStream());
	}
	@Override
	...
	public boolean isFile() {
		if (isFileProtocol) {
			...
			throw new IllegalArgumentException(
					"Is file not supported for resource with protocol [" + locationUrl.getProtocol() + "]");
		}
	}
	...
	public boolean isDirectory() {
		if (isFileProtocol) {
			...
			throw new IllegalArgumentException(
					"Is directory not supported for resource with protocol [" + locationUrl.getProtocol() + "]");
		}
	}
		...
		if (isFileProtocol) {
			...
			if (listFiles != null)
				for (File dirFile : listFiles)
					fileNameSet.add(dirFile.getName());
			for (String filename : fileNameSet)
				children.add(new UrlResourceReference().init(baseLocation + "/" + filename));
			return children;
		} else {
			throw new IllegalArgumentException(
					"Children not supported for resource with protocol [" + locationUrl.getProtocol() + "]");
		}
	}
	...
	public boolean getExists() {
		...
		if (isFileProtocol) {
			...
			final URL url = locationUrl;
			throw new IllegalArgumentException("Exists not supported for resource with protocol ["
	...
	public void putText(String text) {
		if (!isFileProtocol) {
			final URL url = locationUrl;
			throw new IllegalArgumentException("Write not supported for resource [" + getLocation()
					+ "] with protocol [" + (url == null ? null : getUrl().getProtocol()) + "]");
		}
	...
	public void putStream(InputStream stream) {
		if (!isFileProtocol) {
			throw new IllegalArgumentException("Write not supported for resource [" + locationUrl + "] with protocol ["
					+ (locationUrl == null ? null : locationUrl.getProtocol()) + "]");
		}
	...
	public void move(final String newLocation) {
		...
		if (!newRr.getUrl().getProtocol().equals("file"))
			throw new IllegalArgumentException(
					"Location [" + newLocation + "] is not a file location, not moving resource at " + getLocation());
		if (!isFileProtocol)
			throw new IllegalArgumentException("Move not supported for resource [" + locationUrl + "] with protocol ["
					+ (locationUrl == null ? null : locationUrl.getProtocol()) + "]");

		File curFile = getFile();
		if (!curFile.exists())
			throw new IllegalArgumentException(
					"File at " + getLocation() + " [" + curFile.getAbsolutePath() + "] does not exist, cannot move");
	...
	public ResourceReference makeDirectory(final String name) {
		if (!isFileProtocol) {
			final URL url = locationUrl;
			throw new IllegalArgumentException("Write not supported for resource [" + getLocation()
					+ "] with protocol [" + (url == null ? null : url.getProtocol()) + "]");
		}

		UrlResourceReference newRef = (UrlResourceReference) new UrlResourceReference()
				.init(getLocation() + "/" + name);

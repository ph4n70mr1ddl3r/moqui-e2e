...
public abstract class ResourceReference implements Serializable {
	...
	public void putBytes(byte[] bytes) {
		putStream(new ByteArrayInputStream(bytes));
	}
	...
	public URI getUri() {
		try {
			if (supportsUrl()) {
				...
				if (locUrl == null)
					return null;
				// use the multi-argument constructor to have it do character encoding and avoid
				// an exception
				// WARNING: a String from this URI may not equal the String from the URL (ie if
				// characters are encoded)
				// NOTE: this doesn't seem to work on Windows for local files: when protocol is
				// plain "file" and path starts
				// with a drive letter like "C:\moqui\..." it produces a parse error showing the
				// URI as "file://C:/..."
				if (logger.isTraceEnabled())
					logger.trace("Getting URI for URL " + locUrl.toExternalForm());
				...
				return new URI(locUrl.getProtocol(), locUrl.getUserInfo(), locUrl.getHost(), locUrl.getPort(), path,
						locUrl.getQuery(), locUrl.getRef());
			} else {
	...
	/**
	 * One part of the URI not easy to get from the URI object, basically the last
	 * part of the path.
	 */
	...
	public boolean isBinary() {
		return isBinaryContentType(getContentType());
	}
	public boolean isText() {
		return isTextContentType(getContentType());
	}
	...
	public ResourceReference getParent() {
		...
		if (curLocation.endsWith("/"))
			curLocation = curLocation.substring(0, curLocation.length() - 1);
		String strippedLocation = stripLocationPrefix(curLocation);
		if (strippedLocation.isEmpty())
			return null;
		if (strippedLocation.startsWith("/"))
		...
		if (strippedLocation.contains("/")) {
			...
			if (prefix != null && !prefix.isEmpty())
				return createNew(prefix);
			return null;
		}
	}

	/**
	 * Find the directory with a name that matches the current filename (minus the
	 * extension)
	 */
	public ResourceReference findMatchingDirectory() {
		...
		while (!(directoryRef.getExists() && directoryRef.isDirectory()) && dirLoc.lastIndexOf(".") > 0) {
			// get rid of one suffix at a time (for screens probably .xml but use .* for
			// other files, etc)
	...
	/**
	 * Get a reference to the child of this directory or this file in the matching
	 * directory
	 */
	public ResourceReference getChild(String childName) {
		...
		if (fileLoc.charAt(fileLoc.length() - 1) == '/')
			fileLoc.deleteCharAt(fileLoc.length() - 1);
	...
	/**
	 * Get a list of references to all files in this directory or for a file in the
	 * matching directory
	 */
	public List<ResourceReference> getChildren() {
		List<ResourceReference> children = new LinkedList<>();
		ResourceReference directoryRef = findMatchingDirectory();
		if (directoryRef == null || !directoryRef.getExists())
			return null;
	...
	/**
	 * Find a file by path (can be single name) in the matching directory and child
	...
	public ResourceReference findChildFile(String relativePath) {
		...
		// logger.warn("============= finding child resource of [${toString()}] path
		// [${relativePath}]")

		// check the cache first
		ResourceReference childRef = getSubContentRefByPath().get(relativePath);
		if (childRef != null && childRef.getExists())
			return childRef;

		// this finds a file in a directory with the same name as this resource, unless
		// this resource is a directory
		ResourceReference directoryRef = findMatchingDirectory();

		// logger.warn("============= finding child resource path [${relativePath}]
		...
		if (directoryRef.getExists()) {
			...
			if (theFile.getExists() && theFile.isFile())
				childRef = theFile;
			...
			if (childRef == null) {
				...
				// logger.warn("============= finding child resource path [${relativePath}]
				// directoryRef [${directoryRef}] childFilename [${childFilename}] childRef
				// [${childRef}]")
			}
		...
		if (childRef == null) {
			// still nothing? treat the path to the file as a literal and return it (exists
			// will be false)
			if (directoryRef.getExists()) {
		...
		// logger.warn("============= finding child resource of [${toString()}] path
		// [${relativePath}] got [${childRef}]")
		return childRef;
	}

	/**
	 * Find a directory by path (can be single name) in the matching directory and
	 * child matching directories
	 */
	public ResourceReference findChildDirectory(String relativePath) {
		...
		if (this.isFile())
			childDirectoryRef = this.findMatchingDirectory();

		// search remaining relativePathNameList, ie partial directories leading up to
		// filename
	...
	private ResourceReference internalFindChildDir(ResourceReference directoryRef, String childDirName,
			Set<String> parentLocSet) {
		if (directoryRef == null || !directoryRef.getExists())
			return null;
		// no child dir name, means this/current dir
		if (childDirName == null || childDirName.isEmpty())
			return directoryRef;

		// try a direct sub-directory, if it is there it's more efficient than a
		// recursive search
		StringBuilder dirLocation = new StringBuilder(directoryRef.getLocation());
		if (dirLocation.charAt(dirLocation.length() - 1) == '/')
			dirLocation.deleteCharAt(dirLocation.length() - 1);
		if (childDirName.charAt(0) != '/')
			dirLocation.append('/');
		dirLocation.append(childDirName);
		ResourceReference directRef = createNew(dirLocation.toString());
		if (directRef != null && directRef.getExists())
			return directRef;

		// if no direct reference is found, try the more flexible search
		for (ResourceReference childRef : directoryRef.getDirectoryEntries()) {
			if (childRef.isDirectory() && (childRef.getFileName().equals(childDirName)
					|| childRef.getFileName().contains(childDirName + "."))) {
				...
				if (parentLocSet != null) {
					if (parentLocSet.contains(childRef.getLocation())) {
						logger.error("In internalFindChildDir found loop in directory tree, " + childRef.getLocation()
								+ " already visited, parent locations: " + parentLocSet);
						continue;
					}
	...
	private ResourceReference internalFindChildFile(ResourceReference directoryRef, String childFilename,
			Set<String> parentLocSet) {
		// logger.warn("internalFindChildFile " + directoryRef + " [" + childFilename +
		// "] " + parentLocSet);
		if (directoryRef == null || !directoryRef.getExists())
			return null;
		...
		if (exactMatchRef.isFile() && exactMatchRef.getExists())
			return exactMatchRef;
		...
		for (ResourceReference childRef : childEntries) {
			if (childRef.isFile() && (childRef.getFileName().equals(childFilename)
					|| childRef.getFileName().startsWith(childFilename + "."))) {
				return childRef;
			}
		}
		...
		for (ResourceReference childRef : childEntries) {
			if (childRef.isDirectory()) {
				...
				if (parentLocSet != null) {
					if (parentLocSet.contains(childRef.getLocation())) {
						logger.error("In internalFindChildFile found loop in directory tree, " + childRef.getLocation()
								+ " already visited, parent locations: " + parentLocSet);
						continue;
					}
					recurseParentLocSet.addAll(parentLocSet);
	...
	public void walkChildTree(List<Map> allChildFileFlatList, List<Map> childResourceList) {
		...
		if (this.isDirectory())
			for (ResourceReference childRef : getDirectoryEntries()) {
				childRef.walkChildFileTree(this, "", allChildFileFlatList, childResourceList);
			}
	}
	private void walkChildFileTree(ResourceReference rootResource, String pathFromRoot, List<Map> allChildFileFlatList,
			List<Map> childResourceList) {
		// logger.warn("================ walkChildFileTree rootResource=${rootResource}
		// pathFromRoot=${pathFromRoot} curLocation=${getLocation()}")
		...
		if (this.isFile()) {
			...
			if (allChildFileFlatList != null) {
				...
				infoMap.put("path", curPath);
				infoMap.put("name", curFileName);
		...
		// TODO: walk child directories somehow or just stick with files with matching
		// directories?
	}
	...
	private Map<String, ResourceReference> getSubContentRefByPath() {
		if (subContentRefByPath == null)
			subContentRefByPath = new HashMap<>();
	...
	public static String getContentType(String filename) {
		// need to check this, or type mapper handles it fine? ||
		// !filename.contains(".")
	...
	public static boolean isTextContentType(String contentType) {
		...
		if (contentType.endsWith("+json"))
			return true;
		if ("application/rtf".equals(contentType))
			return true;
	...
	public static String stripLocationPrefix(String location) {
		...
		while (strippedLocation.length() > 0 && strippedLocation.charAt(0) == '/')
			strippedLocation.deleteCharAt(0);
	...
	public ArrayList<Version> getNextVersions(String versionName) {
		return new ArrayList<>();
	}
	public InputStream openStream(String versionName) {
		return openStream();
	}
	public String getText(String versionName) {
		return getText();
	}
	...
	public static class Version {
		...
		public Version(ResourceReference ref, String versionName, String previousVersionName, String userId,
		...
		public Version getPreviousVersion() {
			return ref.getVersion(previousVersionName);
		}
		public ArrayList<Version> getNextVersions() {
			return ref.getNextVersions(versionName);
		}
		...
		public InputStream openStream() {
			return ref.openStream(versionName);
		}
		public String getText() {
			return ref.getText(versionName);
		}
		public Map<String, Object> getMap() {
			Map<String, Object> map = new LinkedHashMap<>();
			map.put("versionName", versionName);
			map.put("previousVersionName", previousVersionName);

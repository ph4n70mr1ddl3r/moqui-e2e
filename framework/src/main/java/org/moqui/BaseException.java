...
/**
 * BaseException - the base/root exception for all exception classes in Moqui
 * Framework.
 */
	...
	public static StackTraceElement[] filterStackTrace(StackTraceElement[] orig) {
		...
		for (StackTraceElement ste : orig) {
			...
			if (cn.startsWith("freemarker.core.") || cn.startsWith("freemarker.ext.beans.")
					|| cn.startsWith("org.eclipse.jetty.") || cn.startsWith("java.lang.reflect.")
					|| cn.startsWith("sun.reflect.") || cn.startsWith("org.codehaus.groovy.")
					|| cn.startsWith("groovy.lang.")) {
				continue;
			}
			// if ("renderSingle".equals(ste.getMethodName()) &&
			// cn.startsWith("org.moqui.impl.screen.ScreenSection")) continue;
			// if (("internalRender".equals(ste.getMethodName()) ||
			// "doActualRender".equals(ste.getMethodName())) &&
			// cn.startsWith("org.moqui.impl.screen.ScreenRenderImpl")) continue;
			if (("call".equals(ste.getMethodName()) || "callCurrent".equals(ste.getMethodName()))
					&& ste.getLineNumber() == -1)
				continue;
			// System.out.println("Adding className: " + cn + ", line: " +
			// ste.getLineNumber());
			newList.add(ste);
		}
		// System.out.println("Called getFilteredStackTrace, orig.length=" + orig.length

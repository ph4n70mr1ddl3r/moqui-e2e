...
public class BaseArtifactException extends BaseException {
	...
	public BaseArtifactException(String message) {
		super(message);
		populateArtifactStack();
	}
	public BaseArtifactException(String message, Deque<ArtifactExecutionInfo> curStack) {
		super(message);
		artifactStack = curStack;
	}
	public BaseArtifactException(String message, Throwable nested) {
	...
	public BaseArtifactException(Throwable nested) {
		super(nested);
		populateArtifactStack();
	}
	...
	private void populateArtifactStack() {
		...
		if (ecf != null)
			artifactStack = ecf.getExecutionContext().getArtifactExecution().getStack();
	}

	public Deque<ArtifactExecutionInfo> getArtifactStack() {
	...
	public void printStackTrace(PrintStream printStream) {
		if (artifactStack != null && artifactStack.size() > 0)
			for (ArtifactExecutionInfo aei : artifactStack)
				printStream.println(aei.toBasicString());
	...
	public void printStackTrace(PrintWriter printWriter) {
		if (artifactStack != null && artifactStack.size() > 0)
			for (ArtifactExecutionInfo aei : artifactStack)
				printWriter.println(aei.toBasicString());
		filterStackTrace(this);

...
/**
 * A test harness for screen rendering. Does internal rendering without HTTP
 * request/response
 */
...
public interface ScreenTest {
	...
	/**
	 * A screen path prepended to the screenPath used for all subsequent render()
	 * calls
	 */
	...
	/**
	 * Calls to WebFacade.sendJsonResponse will not be serialized, use along with
	 * ScreenTestRender.getJsonObject()
	 */
	ScreenTest skipJsonSerialize(boolean skip);

	/**
	 * Get screen name paths to all screens with no required parameters under the
	 * rootScreen and (if specified) baseScreenPath
	 */
	 ...
	 * 
	 * @param screenPath
	 *            Path from rootScreen in the sub-screen hierarchy
	 * @param parameters
	 *            Map with name/value pairs to use as if they were URL or body
	 *            parameters
	 * @param requestMethod
	 *            The HTTP request method to use when selecting a transition
	 *            (defaults to get)

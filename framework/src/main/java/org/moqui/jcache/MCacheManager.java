...
/**
 * This class does not completely support the javax.cache.CacheManager spec, it
 * is just enough to use as a factory for MCache instances.
 */
public class MCacheManager implements CacheManager {
	...
	public static MCacheManager getMCacheManager() {
		return singleCacheManager;
	}
	...
	private MCacheManager() {
		try {
			cmUri = new URI("MCacheManager");
		} catch (URISyntaxException e) {
			logger.error("URI Syntax error initializing MCacheManager", e);
		}
	...
	public synchronized <K, V, C extends Configuration<K, V>> Cache<K, V> createCache(String cacheName, C configuration)
		...
		if (isClosed)
			throw new IllegalStateException("MCacheManager is closed");
		if (cacheMap.containsKey(cacheName)) {
			// not per spec, but be more friendly and just return the existing cache: throw
			// new CacheException("Cache with name " + cacheName + " already exists");
	...
	public void enableStatistics(String cacheName, boolean enabled) {
		throw new UnsupportedOperationException(
				"MCacheManager does not support registered statistics; use the MCache.getStats() or getMStats() methods");
	}

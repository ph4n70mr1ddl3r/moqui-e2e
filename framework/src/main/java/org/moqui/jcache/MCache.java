...
/**
 * A simple implementation of the javax.cache.Cache interface. Basically a
 * wrapper around a Map with stats and expiry.
 */
...
public class MCache<K, V> implements Cache<K, V> {
	...
	// NOTE: use ConcurrentHashMap for write locks and such even if can't so easily
	// use putIfAbsent/etc
	...
	private static ScheduledThreadPoolExecutor workerPool = new ScheduledThreadPoolExecutor(1,
		...
		if (configuration != null) {
			...
			if (configuration instanceof MCacheConfiguration) {
				...
				if (mCacheConf.maxEntries > 0) {
					...
					evictFuture = workerPool.scheduleWithFixedDelay(evictRunnable, 30, mCacheConf.maxCheckSeconds,
							TimeUnit.SECONDS);
				}
			}
		}
	...
	public synchronized void setMaxEntries(int elements) {
		if (elements == 0) {
			...
			if (evictRunnable != null) {
				...
				if (configuration instanceof MCacheConfiguration)
					maxCheckSeconds = ((MCacheConfiguration) configuration).maxCheckSeconds;
	...
	/**
	 * Get with expire if the entry's last updated time is before the
	 * expireBeforeTime. Useful when last updated time of a resource is known to see
	 * if the cached entry is out of date.
	 */
	 ...
	 * The policy can be null to use cache's policy.
	 */
	...
	public MEntry<K, V> getEntryNoCheck(K key) {
		...
		if (entry != null) {
			...
			entry.accessCount++;
			if (accessTime > entry.lastAccessTime)
	...
	private MEntry<K, V> getEntryInternal(final K key, final ExpiryPolicy policy, final Long expireBeforeTime,
			long currentTime) {
		if (isClosed)
			throw new IllegalStateException("Cache " + name + " is closed");
		if (key == null)
		...
		if (entry != null) {
			...
			if (entry != null) {
				...
				entry.accessCount++;
				// at this point if an ad-hoc policy is used or hasExpiry == true currentTime
				// will be set, otherwise will be 0
				// meaning we don't need to track the lastAccessTime (only thing we need
	...
	private MEntry<K, V> getCheckExpired(K key, long currentTime) {
		...
		if (key == null)
			throw new IllegalArgumentException("Cache key cannot be null");
		MEntry<K, V> entry = entryStore.get(key);
		if (hasExpiry && entry != null
				&& entry.isExpired(currentTime, accessDuration, creationDuration, updateDuration)) {
			entryStore.remove(key);
			entry = null;
			if (statsEnabled)
				stats.countExpire();
	...
	public void putAll(Map<? extends K, ? extends V> map) {
		...
		for (Map.Entry<? extends K, ? extends V> me : map.entrySet())
			getAndPut(me.getKey(), me.getValue());
	}
	@Override
	...
	public <C extends Configuration<K, V>> C getConfiguration(Class<C> clazz) {
		...
		throw new IllegalArgumentException("Class " + clazz.getName() + " not compatible with configuration class "
	...
	public void close() {
		if (isClosed)
			throw new IllegalStateException("Cache " + name + " is already closed");
		isClosed = true;
		entryStore.clear();
	}
	@Override
		...
		public Entry<K, V> next() {
			...
			while (curIndex < maxIndex) {
				...
				if (curEntry.isExpired) {
					curEntry = null;
				} else if (mCache.hasExpiry && curEntry.isExpired(initialTime, mCache.accessDuration,
						mCache.creationDuration, mCache.updateDuration)) {
					mCache.entryStore.remove(curEntry.getKey());
					if (mCache.statsEnabled)
						mCache.stats.countExpire();
					curEntry = null;
				} else {
					if (mCache.statsEnabled) {
						mCache.stats.gets++;
	...
	private static class EvictRunnable<K, V> implements Runnable {
		...
		MCache cache;
		int maxEntries;
		EvictRunnable(MCache mc, int entries) {
			cache = mc;
			maxEntries = entries;
		}
		@Override
	...
	private static class AccessComparator implements Comparator<MEntry> {
		...
		public int compare(MEntry e1, MEntry e2) {
			if (e1.accessCount == e2.accessCount) {
				if (e1.lastAccessTime == e2.lastAccessTime)
					return 0;
				else
					return e1.lastAccessTime > e2.lastAccessTime ? 1 : -1;
			} else {

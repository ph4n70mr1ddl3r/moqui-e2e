...
public class FlatXmlExtractor implements SimpleEtl.Extractor {
	...
	FlatXmlExtractor(ResourceReference xmlRef) {
		resourceRef = xmlRef;
	}
	...
	public void extract(SimpleEtl etl) throws Exception {
		...
		if (resourceRef == null || !resourceRef.getExists()) {
			logger.warn("Resource does not exist, not extracting data from "
					+ (resourceRef != null ? resourceRef.getLocation() : "[null ResourceReference]"));
			return;
		}
		...
		try {
			...
			try {
				...
				logger.error("Error closing XML stream from " + resourceRef.getLocation(), e);
			}
		}
	...
	private static class FlatXmlHandler extends DefaultHandler {
		...
		FlatXmlHandler(FlatXmlExtractor fxe) {
			extractor = fxe;
		}
		...
		public void endElement(String ns, String localName, String qName) {
			...
			if (curEntry == null) {
				...
				if (rootName != null && rootName.equals(qName))
					rootName = null;
				return;
			}
			...
			if (!qName.equals(curEntry.type))
				throw new IllegalStateException("Invalid close element " + qName + ", was expecting " + curEntry.type);

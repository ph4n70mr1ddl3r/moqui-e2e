...
public class SimpleEtl {
	...
	/**
	 * Call this to add a transformer to run for any type, will be run in order
	 * added
	 */
	public SimpleEtl addTransformer(@Nonnull Transformer transformer) {
		if (internalConfig == null)
			internalConfig = new TransformConfiguration();
		internalConfig.addTransformer(transformer);
		return this;
	}
	/**
	 * Call this to add a transformer to run a particular type, which will be run in
	 * order for entries of the type
	 */
	...
	/**
	 * Add from an external TransformConfiguration, copies configuration to avoid
	 * modification
	 */
	...
	/**
	 * Use an external configuration as-is. Overrides any previous addTransformer()
	 * and addConfiguration() calls. Any calls to addTransformer() and
	 * addConfiguration() will modify this configuration.
	 */
	...
	public SimpleEtl stopOnError() {
		this.stopOnError = true;
		return this;
	}
	/** Set timeout in seconds; passed to Loader.init() for transactions, etc */
	public SimpleEtl setTimeout(Integer timeout) {
	...
	public SimpleEtl addMessage(String msg) {
	...
	public Exception getExtractException() {
		return extractException;
	}
	public List<EtlError> getTransformErrors() {
		return Collections.unmodifiableList(transformErrors);
	}
	public List<EtlError> getLoadErrors() {
		return Collections.unmodifiableList(loadErrors);
	}
	public boolean hasError() {
		return extractException != null || transformErrors.size() > 0 || loadErrors.size() > 0;
	}
	public Throwable getSingleErrorCause() {
		if (extractException != null)
			return extractException;
		if (transformErrors.size() > 0)
			return transformErrors.get(0).error;
		if (loadErrors.size() > 0)
			return loadErrors.get(0).error;
		return null;
	}
	 ...
	 *             if thrown extraction should stop and return
	 */
	public boolean processEntry(Entry extractEntry) throws StopException {
		...
		if (internalConfig != null && internalConfig.hasTransformers) {
			...
			if (entryTransform.loadCurrent != null
					? entryTransform.loadCurrent
	...
	public static class TransformConfiguration {
		...
		/**
		 * Call this to add a transformer to run for any type, which will be run in
		 * order
		 */
		...
		void runTransformers(SimpleEtl etl, EntryTransform entryTransform, ArrayList<Entry> loadEntries)
				throws StopException {
			...
			for (int i = 0; i < newEntriesSize; i++) {
				...
				if (newTransform.loadCurrent != null
						? newTransform.loadCurrent
						: newTransform.newEntries == null || newTransform.newEntries.size() == 0) {
					loadEntries.add(newEntry);
				}
			}
		}
		// internal method, returns true to skip entry (or remove from load list)
		void transformEntry(SimpleEtl etl, Transformer transformer, EntryTransform entryTransform)
				throws StopException {
			try {
	...
	public static class EtlError {
		...
		EtlError(Entry entry, Throwable t) {
			this.entry = entry;
			this.error = t;
		}
	}
	...
	public static class SimpleEntry implements Entry {
		...
		public SimpleEntry(String type, Map<String, Object> values) {
			this.type = type;
			this.values = values;
		}
		@Override
	...
	public static class EntryTransform {
		...
		/**
		 * By default the current entry is loaded only if no new entries are added; set
		 * to false to not load even if no entries are added (filter); set to true to
		 * load even if no new entries are added
		 */
		public EntryTransform loadCurrent(boolean load) {
			loadCurrent = load;
			return this;
		}
	...
	public interface Extractor {
		/**
		 * Called once to start processing, should call etl.processEntry() for each
		 * entry and close itself once finished
		 */
	...
	public interface Transformer {
		/**
		 * Call methods on EntryTransform to add new entries (generally with different
		 * types), modify the current entry's values, or filter the entry.
		 */
	...
	public interface Loader {
		...
		/**
		 * Called after all entries processed to close files, commit/rollback
		 * transactions, etc;
		 */

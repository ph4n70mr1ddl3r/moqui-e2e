...
public class XmlAction {
	...
	/**
	 * The Groovy class compiled from the script transformed from the XML actions
	 * text using the FTL template.
	 */
	...
	public Object run(ExecutionContextImpl eci) {
		...
		if (curClass == null)
			throw new IllegalStateException(
					"No Groovy class in place for XML actions, look earlier in log for the error in init");
		if (isDebugEnabled)
			logger.debug("Running groovy script: \n" + writeGroovyWithLines() + "\n");

		Script script = InvokerHelper.createScript(curClass, eci.contextBindingInternal);
		try {
			return script.run();
		} catch (Throwable t) {
			// NOTE: not logging full stack trace, only needed when lots of threads are
			// running to pin down error (always logged later)
	...
	protected synchronized Class makeGroovyClass() {
		...
		// if (logger.isTraceEnabled()) logger.trace("Xml Action [${location}]
		// groovyString: ${curGroovy}")
		try {
	...
	public String getGroovyString() {
		...
		try {
			...
			Environment env = ecfi.resourceFacade.getXmlActionsScriptRunner().getXmlActionsTemplate()
					.createProcessingEnvironment(root, outWriter);
			env.process();
		...
		if (logger.isTraceEnabled())
			logger.trace("XML actions at [" + location + "] produced groovy script:\n" + groovyString

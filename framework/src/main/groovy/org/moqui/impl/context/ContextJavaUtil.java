...
public class ContextJavaUtil {
	...
	/**
	 * the Groovy JsonBuilder doesn't handle various Moqui objects very well, ends
	 * up trying to access all properties and results in infinite recursion, so need
	 * to unwrap and exclude some
	 */
	public static Map<String, Object> unwrapMap(Map<String, Object> sourceMap) {
		...
		for (Map.Entry<String, Object> entry : sourceMap.entrySet()) {
			...
			if (value == null)
				continue;
			// logger.warn("======== actionsResult - ${entry.key}
			// (${entry.value?.getClass()?.getName()}): ${entry.value}")
			Object unwrapped = unwrap(key, value);
			if (unwrapped != null)
				targetMap.put(key, unwrapped);
		}
		return targetMap;
	}
	...
	public static Object unwrap(String key, Object value) {
		...
		if (value instanceof CharSequence || value instanceof Number || value instanceof java.util.Date) {
			...
			for (Object entry : valCol)
				newList.add(unwrap(key, entry));
			return newList;
			...
			for (Map.Entry entry : valMap.entrySet())
				newMap.put(entry.getKey(), unwrap(key, entry.getValue()));
			return newMap;
		} else {
			logger.info("In screen actions skipping value from actions block that is not supported; key=" + key
	...
	public static class ArtifactStatsInfo {
		...
		ArtifactStatsInfo(ArtifactExecutionInfo.ArtifactType artifactTypeEnum, String artifactSubType,
				String artifactName) {
		...
		double getAverage() {
			return hitCount > 0 ? totalTimeMillis / hitCount : 0;
		}
		double getStdDev() {
			if (hitCount < 2)
				return 0;
			return Math.sqrt(
					Math.abs(totalSquaredTime - ((totalTimeMillis * totalTimeMillis) / hitCount)) / (hitCount - 1L));
		}
		public boolean countHit(long startTime, double runningTime) {
			hitCount++;
			boolean isSlow = isHitSlow(runningTime);
			// if (isSlow) slowHitCount++;
			// do something funny with these so we get a better avg and std dev, leave out
			// the first result (count 2nd
			// twice) if first hit is more than 2x the second because the first hit is
			// almost always MUCH slower
			...
			if (curHitBin == null)
				curHitBin = new ArtifactBinInfo(this, startTime);
		...
		boolean isHitSlow(double runningTime) {
			...
			// if runningTime is more than 2.6 std devs from the avg, count it and possibly
			// log it
			// using 2.6 standard deviations because 2 would give us around 5% of hits
			// (normal distro), shooting for more like 1%
			double slowTime = average + (stdDev * 2.6);
			if (slowTime != 0 && runningTime > slowTime) {
				if (runningTime > userImpactMinMillis)
					logger.warn("Slow hit to " + artifactTypeEnum + ":" + artifactSubType + ":" + artifactName
							+ " running time " + runningTime + " is greater than average " + average
	...
	public static class ArtifactBinInfo {
		...
		void countHit(double runningTime, boolean isSlow) {
			...
			if (runningTime < minTimeMillis)
				minTimeMillis = runningTime;
			if (runningTime > maxTimeMillis)
				maxTimeMillis = runningTime;
		}
		...
		EntityValue makeAhbValue(ExecutionContextFactoryImpl ecfi, Timestamp binEndDateTime) {
			...
			ahb.put("serverIpAddress",
					ecfi.localhostAddress != null ? ecfi.localhostAddress.getHostAddress() : "127.0.0.1");
			ahb.put("serverHostName",
					ecfi.localhostAddress != null ? ecfi.localhostAddress.getHostName() : "localhost");
			return ahb;
	...
	public static class ArtifactHitInfo {
		...
		ArtifactHitInfo(ExecutionContextImpl eci, boolean isSlowHit,
				ArtifactExecutionInfo.ArtifactType artifactTypeEnum, String artifactSubType, String artifactName,
				long startTime, double runningTimeMillis, Map<String, Object> parameters, Long outputSize) {
			...
			if (eci.getMessage().hasError()) {
				...
				for (String curErr : eci.getMessage().getErrors())
					errorMessage.append(curErr).append(";");
				if (errorMessage.length() > 255)
					errorMessage.delete(255, errorMessage.length());
		...
		EntityValue makeAhiValue(ExecutionContextFactoryImpl ecfi) {
			...
			if (parameters != null && parameters.size() > 0) {
				...
				for (Map.Entry<String, Object> pme : parameters.entrySet()) {
					...
					if (value == null || ObjectUtilities.isEmpty(value) || value instanceof Map
					...
					if (valString.length() > 80)
						valString = valString.substring(0, 80);
					ps.append(key).append("=").append(valString);
				}
				// is text-long, could be up to 4000, probably don't want that much for data
				// size
				if (ps.length() > 1000)
					ps.delete(1000, ps.length());
				ahp.put("parameterString", ps.toString());
			}
			if (outputSize != null)
				ahp.put("outputSize", outputSize);
			...
			if (requestUrl != null && requestUrl.length() > 0)
				ahp.put("requestUrl", requestUrl);
			if (referrerUrl != null && referrerUrl.length() > 0)
				ahp.put("referrerUrl", referrerUrl);

			ahp.put("serverIpAddress",
					ecfi.localhostAddress != null ? ecfi.localhostAddress.getHostAddress() : "127.0.0.1");
			ahp.put("serverHostName",
					ecfi.localhostAddress != null ? ecfi.localhostAddress.getHostName() : "localhost");
	...
	static class RollbackInfo {
		...
		/**
		 * A rollback is often done because of another error, this represents that
		 * error.
		 */
		public Throwable causeThrowable;
		/**
		 * This is for a stack trace for where the rollback was actually called to help
		 * track it down more easily.
		 */
	...
	static class TxStackInfo {
		...
		public TxStackInfo(TransactionFacadeImpl tfi) {
			transactionFacade = tfi;
		}
		...
		public void clearCurrent() {
			...
			// lock track: remove all EntityRecordLock in recordLockList from
			// TransactionFacadeImpl.recordLockByEntityPk
			int recordLockListSize = recordLockList.size();
			// if (recordLockListSize > 0) logger.warn("TOREMOVE TxStackInfo
			// EntityRecordLock clearing " + recordLockListSize + " locks");
	...
	public static class EntityRecordLock {
		...
		public EntityRecordLock(String entityName, String pkString, ArrayList<ArtifactExecutionInfo> artifactStack) {
			...
			// NOTE: used primary as a key, for efficiency don't use separator between
			// entityName and pkString
		...
		void register(ConcurrentHashMap<String, ArrayList<EntityRecordLock>> recordLockByEntityPk,
				TxStackInfo txStackInfo) {
			if (txStackInfo != null) {
				moquiTxId = txStackInfo.moquiTxId;
				txBeginTime = txStackInfo.transactionBeginStartTime != null
						? txStackInfo.transactionBeginStartTime
						: -1;
			}

			ArrayList<EntityRecordLock> curErlList = recordLockByEntityPk.computeIfAbsent(entityPlusPk,
					k -> new ArrayList<>());
			synchronized (curErlList) {
				...
				// check for existing locks in this.recordLockByEntityPk, log warning if others
				// found
				if (curErlList.size() > 0) {
					StringBuilder msgBuilder = new StringBuilder().append("Potential lock conflict entity ")
							.append(entityName).append(" pk ").append(pkString).append(" thread ").append(threadName)
							.append(" TX ").append(moquiTxId).append(" began ").append(new Timestamp(txBeginTime));
					if (mutateEntityName != null)
						msgBuilder.append(" from mutate of entity ").append(mutateEntityName).append(" pk ")
								.append(mutatePkString);
					msgBuilder.append(" at: ");
					if (artifactStack != null)
						for (int mi = 0; mi < artifactStack.size(); mi++) {
							msgBuilder.append("\n")
					...
					for (int i = 0; i < curErlList.size(); i++) {
						...
						msgBuilder.append("\n== OTHER LOCK ").append(i).append(" thread ").append(otherErl.threadName)
								.append(" TX ").append(otherErl.moquiTxId).append(" began ")
								.append(new Timestamp(otherErl.txBeginTime)).append(" at: ");
						if (otherErl.artifactStack != null)
							for (int mi = 0; mi < otherErl.artifactStack.size(); mi++) {
								msgBuilder.append("\n")
										.append(StringGroovyMethods.padLeft((CharSequence) Integer.toString(mi), 2,
												"0"))
				...
				if (txStackInfo != null) {
					...
					logger.warn(
							"In EntityRecordLock register no TxStackInfo so not registering lock because won't be able to clear for entity "
									+ entityName + " pk " + pkString + " thread " + threadName);
				}
			}
		}
	 ...
	 *
	 * The close() method does nothing, only closed when closeInternal() called by
	 * TransactionFacade on commit, rollback, or destroy (when transactions are also
	 * cleaned up as a last resort).
	 *
	...
	public static class ConnectionWrapper implements Connection {
		...
		/*
		 * these don't work, don't think we need them anyway: protected Object clone()
		 * throws CloneNotSupportedException { return new ConnectionWrapper((Connection)
		 * con.clone(), tfi, groupName) } protected void finalize() throws Throwable {
		 * con.finalize() }
		 */
	}
	...
	static class TimestampNoNegativeJsonSerializer extends StdSerializer<Timestamp> {
		TimestampNoNegativeJsonSerializer() {
		...
		public void serialize(Timestamp value, JsonGenerator gen, SerializerProvider serializers)
			...
			if (value != null) {
				...
				if (time < 0) {
					String isoUtc = value.toInstant().atZone(ZoneOffset.UTC.normalized())
	...
	static class LiteStringMapJsonSerializer extends StdSerializer<LiteStringMap> {
		LiteStringMapJsonSerializer() {
			super(LiteStringMap.class);
		}
		@Override
		public void serialize(LiteStringMap lsm, JsonGenerator gen, SerializerProvider serializers)
				throws IOException, JsonProcessingException {
	...
	static class ResourceReferenceJsonSerializer extends StdSerializer<ResourceReference> {
		ResourceReferenceJsonSerializer() {
			super(ResourceReference.class);
		}
		@Override
		public void serialize(ResourceReference resourceRef, JsonGenerator gen, SerializerProvider serializers)
				throws IOException, JsonProcessingException {
			if (resourceRef == null) {
				gen.writeNull();
				return;
			...
			if (currentVersion != null)
				gen.writeObjectField("currentVersionName", currentVersion.getVersionName());
			gen.writeEndObject();
		}
	}

	// NOTE: using unbound LinkedBlockingQueue, so max pool size in
	// ThreadPoolExecutor has no effect
	...
	public static class JobThreadFactory implements ThreadFactory {
		...
		private final AtomicInteger threadNumber = new AtomicInteger(1);
		public Thread newThread(Runnable r) {
			return new Thread(workerGroup, r, "MoquiJob-" + threadNumber.getAndIncrement());
		}
	}
	public static class WorkerThreadPoolExecutor extends ThreadPoolExecutor {
		...
		protected void afterExecute(Runnable runnable, Throwable throwable) {
			...
			if (activeEc != null) {
				logger.warn(
						"In WorkerThreadPoolExecutor.afterExecute() there is still an ExecutionContext for runnable "
								+ runnable.getClass().getName() + " in thread (" + Thread.currentThread().threadId()
								+ ":" + Thread.currentThread().getName() + "), destroying");
				try {
					activeEc.destroy();
				} catch (Throwable t) {
				...
				if (ecfi.transactionFacade.isTransactionInPlace()) {
					logger.error("In WorkerThreadPoolExecutor a transaction is in place for thread "
							+ Thread.currentThread().getName() + ", trying to commit");
					try {
						ecfi.transactionFacade.destroyAllInThread();
					} catch (Exception e) {
						logger.error("WorkerThreadPoolExecutor commit in place transaction failed in thread "
	...
	static class ScheduledThreadFactory implements ThreadFactory {
		...
		private final AtomicInteger threadNumber = new AtomicInteger(1);
		public Thread newThread(Runnable r) {
			return new Thread(workerGroup, r, "MoquiScheduled-" + threadNumber.getAndIncrement());
		}
	}
	public static class CustomScheduledTask<V> implements RunnableScheduledFuture<V> {
		...
		@Override
		public String toString() {
			return "CustomScheduledTask " + (runnable != null
					? runnable.getClass().getName()
					: (callable != null ? callable.getClass().getName() : "[no Runnable or Callable!]"));
		}
	}
	...
	static class ScheduledRunnableInfo {
		...
		// NOTE: tracking initial ScheduledFuture is useless as it gets replaced with
		// each run: public final ScheduledFuture scheduledFuture;

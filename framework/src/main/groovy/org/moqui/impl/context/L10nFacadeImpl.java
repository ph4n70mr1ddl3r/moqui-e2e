...
public class L10nFacadeImpl implements L10nFacade {
	...
	public L10nFacadeImpl(ExecutionContextImpl eci) {
		this.eci = eci;
	}

	protected Locale getLocale() {
		return eci.userFacade.getLocale();
	}
	protected TimeZone getTimeZone() {
		return eci.userFacade.getTimeZone();
	}

	@Override
	public String localize(String original) {
		return localize(original, getLocale());
	}
	@Override
	public String localize(String original, Locale locale) {
		...
		if (originalLength > 255) {
			throw new BaseArtifactException(
					"Original String cannot be more than 255 characters long, passed in string was " + originalLength
							+ " characters long");
		}
		...
		// if original has a hash and we still don't have a localizedMessage then use
		// what precedes the hash and try again
		if (localizedMessage == null) {
			...
			if (indexOfHash > 0 && indexOfHash > indexOfCloseCurly) {
				...
				if (localizedMessage == null && localeUnderscoreIndex > 0)
					localizedMessage = findHash.condition("locale", localeString.substring(0, localeUnderscoreIndex))
							.one();
				if (localizedMessage == null)
	...
	@Override
	public String formatCurrencyNoSymbol(Object amount, String uomId) {
		return formatCurrency(amount, uomId, null, getLocale(), true);
	}
	@Override
	public String formatCurrency(Object amount, String uomId) {
		return formatCurrency(amount, uomId, null, getLocale(), false);
	}
	@Override
	public String formatCurrency(Object amount, String uomId, Integer fractionDigits) {
		return formatCurrency(amount, uomId, fractionDigits, getLocale(), false);
	}
	@Override
		...
		if (uomId != null && uomId.length() > 0) {
			try {
				...
				if (logger.isTraceEnabled())
					logger.trace("Ignoring IllegalArgumentException for Currency parse: " + e.toString());
			}
		}
		if (currencySymbol == null)
			currencySymbol = "";
	...
	@Override
	public BigDecimal roundCurrency(BigDecimal amount, String uomId) {
		return roundCurrency(amount, uomId, false);
	}
	@Override
	public BigDecimal roundCurrency(BigDecimal amount, String uomId, boolean precise) {
		return roundCurrency(amount, uomId, false, RoundingMode.HALF_UP);
	}
	@Override
	...
	public BigDecimal roundCurrency(BigDecimal amount, String uomId, boolean precise, RoundingMode mode) {
		if (amount == null)
			return null;
		List<EntityValue> uomList = eci.getEntity().find("moqui.basic.Uom").condition("uomId", uomId)
				.condition("uomTypeEnumId", "UT_CURRENCY_MEASURE").list();
	...
	public Time parseTime(String input, String format) {
		...
		// also try the full ISO-8601, times may come in that way (even if funny with a
		// date of 1970-01-01)
		if (cal == null)
			cal = calendarValidator.validate(input, "yyyy-MM-dd'T'HH:mm:ssZ", curLocale, curTz);
		if (cal != null) {
			Time time = new Time(cal.getTimeInMillis());
			// logger.warn("============== parseTime input=${input} cal=${cal}
			// long=${cal.getTimeInMillis()} time=${time} time long=${time.getTime()} util
			// date=${new java.util.Date(cal.getTimeInMillis())} timestamp=${new
	...
	public String formatTime(Time input, String format, Locale locale, TimeZone tz) {
		...
		// logger.warn("============= formatTime input=${input} timeStr=${timeStr}
		// long=${input.getTime()}")
		return timeStr;
	}
	...
	public java.sql.Date parseDate(String input, String format) {
		...
		// NOTE DEJ 20150317 Date parsing in terms of time zone causes funny issues
		// because the time part of the long
		// since epoch representation is lost going to/from the DB, especially since the
		// time portion is set to 0 and
		// with time zone conversion when the system date is in an earlier time zone
		// than the user date it pushes the
		// Date to the previous day; what seems like the best solution is to parse and
		// save the Date in the
		// system/default time zone, and format it that way as well.
		// The BIG dilemma is there is no way to represent a Date (yyyy-MM-dd) in an
		// object that does not use the long
		// since epoch but rather is an absolute year, month, and day... which is really
		// what we want.
		/*
		 * TimeZone curTz = getTimeZone() Calendar cal =
		 * calendarValidator.validate(input, format, curLocale, curTz) if (cal == null)
		 * cal = calendarValidator.validate(input, "MM/dd/yyyy", curLocale, curTz) //
		 * also try the full ISO-8601, dates may come in that way if (cal == null) cal =
		 * calendarValidator.validate(input, "yyyy-MM-dd'T'HH:mm:ssZ", curLocale, curTz)
		 */

		Calendar cal = calendarValidator.validate(input, format, curLocale);
		if (cal == null)
			cal = calendarValidator.validate(input, "MM/dd/yyyy", curLocale);
		// also try the full ISO-8601, dates may come in that way
		if (cal == null)
			cal = calendarValidator.validate(input, "yyyy-MM-dd'T'HH:mm:ssZ", curLocale);
		if (cal != null) {
			java.sql.Date date = new java.sql.Date(cal.getTimeInMillis());
			// logger.warn("============== parseDate input=${input} cal=${cal}
			// long=${cal.getTimeInMillis()} date=${date} date long=${date.getTime()} util
			// date=${new java.util.Date(cal.getTimeInMillis())} timestamp=${new
			// java.sql.Timestamp(cal.getTimeInMillis())}")
			return date;
		}
		...
		try {
			...
			if (logger.isTraceEnabled())
				logger.trace("Ignoring NumberFormatException for Date parse: " + e.toString());
		}
	...
	public String formatDate(java.util.Date input, String format, Locale locale, TimeZone tz) {
		...
		// logger.warn("============= formatDate input=${input} dateStr=${dateStr}
		// long=${input.getTime()}")
		return dateStr;
	}
	...
	static {
		...
		timestampFormats.add("yyyy-MM-dd HH:mm");
		...
		timestampFormats.add("yyyy-MM-dd");
	...
	@Override
	public Timestamp parseTimestamp(final String input, final String format, final Locale locale,
		...
		Calendar cal = null;
		if (format != null && !format.isEmpty())
			cal = calendarValidator.validate(input, format, curLocale, curTz);

		// long values are pretty common, so if there are no special characters try that
		// first (fast to check)
		if (cal == null) {
			...
			if (nonDigits == 0 || (nonDigits == 1 && input.startsWith("-"))) {
				try {
					...
					if (logger.isTraceEnabled())
						logger.trace("Ignoring NumberFormatException for Timestamp parse: " + e.toString());
				}
			}
		}
		...
		// logger.warn("=========== input=${input}, cal=${cal},
		// long=${cal?.getTimeInMillis()}, locale=${curLocale}, timeZone=${curTz},
		// System=${System.currentTimeMillis()}")
		if (cal != null)
			return new Timestamp(cal.getTimeInMillis());

		try {
			// NOTE: do this AFTER the long parse because long numbers are interpreted
			// really weird by this
			// ISO 8601 parsing using JAXB DatatypeConverter.parseDateTime(); on Java 7 can
			// use "X" instead of "Z" in format string, but not in Java 6
			cal = DatatypeConverter.parseDateTime(input);
			if (cal != null)
				return new Timestamp(cal.getTimeInMillis());
		} catch (Exception e) {
			if (logger.isTraceEnabled())
				logger.trace("Ignoring Exception for DatatypeConverter Timestamp parse: " + e.toString());
		}
	...
	public String formatNumber(Number input, String format, Locale locale) {
		if (locale == null)
			locale = getLocale();
		if (format == null || format.isEmpty()) {
			// BigDecimalValidator defaults to 3 decimal digits, if no format specified we
			// don't want to truncate so small, use better defaults
	...
	public String format(Object value, String format, Locale locale, TimeZone tz) {
		...
		if (valueClass == Timestamp.class)
			return formatTimestamp((Timestamp) value, format, locale, tz);
		if (valueClass == java.util.Date.class)
			return formatTimestamp((java.util.Date) value, format, locale, tz);
		if (valueClass == java.sql.Date.class)
			return formatDate((Date) value, format, locale, tz);
		if (valueClass == Time.class)
			return formatTime((Time) value, format, locale, tz);
		// this one needs to be instanceof to include the many sub-classes of Number
		if (value instanceof Number)
			return formatNumber((Number) value, format, locale);
		// Calendar is an abstract class, so must use instanceof here as well
		if (value instanceof Calendar)
			return formatDateTime((Calendar) value, format, locale, tz);

...
public class FtlTemplateRenderer implements TemplateRenderer {
	...
	public void render(String location, Writer writer) {
		...
		try {
			...
			throw new BaseArtifactException("Error rendering template at " + location, e);
		}
	}
	...
	private Template getFtlTemplateByLocation(final String location) {
		...
		if (!hasVersion) {
			if (templateFtlLocationCache instanceof MCache) {
				...
				// if we have a rr and last modified is newer than the cache entry then throw it
				// out (expire when cached entry
		...
		if (theTemplate == null)
			theTemplate = makeTemplate(location, hasVersion);
		if (theTemplate == null)
			throw new BaseArtifactException("Could not find template at " + location);
		return theTemplate;
	}
		...
		try {
			...
			if (templateReader != null) {
				try {
					...
					logger.error("Error closing template reader", e);
				}
			}
		}
	...
	private static Configuration makeFtlConfiguration(ExecutionContextFactoryImpl ecfi) {
		...
		/*
		 * not needed, using getTemplate override instead: newConfig.setCacheStorage(new
		 * NullCacheStorage()) newConfig.setTemplateUpdateDelay(1)
		 * newConfig.setTemplateLoader(new MoquiTemplateLoader(ecfi))
		 * newConfig.setLocalizedLookup(false)
		 */
		/*
		 * String moquiRuntime = System.getProperty("moqui.runtime"); if (moquiRuntime
		 * != null && !moquiRuntime.isEmpty()) { File runtimeFile = new
		 * File(moquiRuntime); try {
		 * newConfig.setDirectoryForTemplateLoading(runtimeFile); } catch (Exception e)
		 * { logger.error("Error setting FTL template loading directory to " +
		 * moquiRuntime, e); } }
		 */
	...
	private static class MoquiConfiguration extends Configuration {
		...
		public Template getTemplate(String name, Locale locale, Object customLookupCondition, String encoding,
			...
			// NOTE: doing this because template loading behavior with cache/etc not desired
	 ...
	 * return ecfi.resourceFacade.getLocationReference(name); }
	 * 
	 * @Override public long getLastModified(Object templateSource) { if
	 * (templateSource instanceof ResourceReference) { return ((ResourceReference)
	 * templateSource).getLastModified(); } else { return 0; } }
	 * 
	 * @Override public Reader getReader(Object templateSource, String encoding)
	 * throws IOException { if (!(templateSource instanceof ResourceReference))
	 * throw new
	 * IllegalArgumentException("Cannot get Reader, templateSource is not a ResourceReference"
	 * ); ResourceReference rr = (ResourceReference) templateSource; InputStream is
	 * = rr.openStream(); if (is == null) throw new
	 * IOException("Template not found at " + rr.getLocation()); return new
	 * InputStreamReader(is); }
	 * 
	 * @Override public void closeTemplateSource(Object templateSource) throws
	 * IOException { } }
	 */

	private static class MoquiTemplateExceptionHandler implements TemplateExceptionHandler {
		public void handleTemplateException(final TemplateException te, Environment env, Writer out)
				throws TemplateException {
			try {
				// TODO: encode error, something like: StringUtil.SimpleEncoder simpleEncoder =
				...
				if (te.getCause() != null) {
					...
					if (causeMsg == null || causeMsg.isEmpty())
						causeMsg = te.getMessage();
					if (causeMsg == null || causeMsg.isEmpty())
						causeMsg = "no message available";
					out.write("[Template Error: ");
					out.write(causeMsg);
					out.write("]");
				} else {
					// NOTE: if there is not cause it is an exception generated by FreeMarker and
					// not some code called in the template
					if (te instanceof InvalidReferenceException) {
						// NOTE: ScreenTestImpl looks for this string, ie "[Template Error"
						logger.error("[Template Error: expression '" + te.getBlamedExpressionString()
								+ "' was null or not found (" + te.getTemplateSourceName() + ":" + te.getLineNumber()
								+ "," + te.getColumnNumber() + ")]");
						out.write("[Template Error]");
					} else {

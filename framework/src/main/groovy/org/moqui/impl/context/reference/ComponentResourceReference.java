...
public class ComponentResourceReference extends WrapperResourceReference {
	...
	public ComponentResourceReference() {
		super();
	}
	...
	public ResourceReference init(String location, ExecutionContextFactoryImpl ecf) {
		...
		// if there is a hash (used in resource locations for versions) strip the hash
	...
	public List<ResourceReference> getDirectoryEntries() {
		// a little extra work to keep the directory entries as component-based
		// locations
		...
		for (ResourceReference entryRr : nestedList) {
			...
			if (entryLoc.endsWith("/"))
				entryLoc = entryLoc.substring(0, entryLoc.length() - 1);

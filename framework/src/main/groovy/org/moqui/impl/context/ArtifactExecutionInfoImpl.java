...
public class ArtifactExecutionInfoImpl implements ArtifactExecutionInfo {
	...
	// NOTE: these need to be in a Map instead of the DB because Enumeration records
	...
	static {
		artifactTypeDescriptionMap.put(AT_XML_SCREEN, "Screen");
		artifactTypeDescriptionMap.put(AT_XML_SCREEN_TRANS, "Transition");
		artifactTypeDescriptionMap.put(AT_XML_SCREEN_CONTENT, "Screen Content");
		artifactTypeDescriptionMap.put(AT_SERVICE, "Service");
		artifactTypeDescriptionMap.put(AT_ENTITY, "Entity");
		artifactTypeDescriptionMap.put(AT_REST_PATH, "REST Path");
		artifactTypeDescriptionMap.put(AT_OTHER, "Other");

		artifactActionDescriptionMap.put(AUTHZA_VIEW, "View");
		artifactActionDescriptionMap.put(AUTHZA_CREATE, "Create");
		artifactActionDescriptionMap.put(AUTHZA_UPDATE, "Update");
		artifactActionDescriptionMap.put(AUTHZA_DELETE, "Delete");
	...
	public ArtifactExecutionInfoImpl(String name, ArtifactType typeEnum, AuthzAction actionEnum, String detail) {
		...
		actionDetail = detail;
		// createdLocation = new Exception("Create AEII location for ${name}, type
		// ${typeEnumId}, action ${actionEnumId}")
		startTimeMillis = System.currentTimeMillis();
		startTimeNanos = System.nanoTime();
	}

	public ArtifactExecutionInfoImpl setParameters(Map<String, Object> parameters) {
	...
	@Override
	public ArtifactType getTypeEnum() {
	...
	@Override
	public AuthzAction getActionEnum() {
	...
	public String getAuthorizedUserId() {
		return internalAuthorizedUserId;
	}
	void setAuthorizedUserId(String authorizedUserId) {
		this.internalAuthorizedUserId = authorizedUserId;
	}

	@Override
	public AuthzType getAuthorizedAuthzType() {
		return internalAuthorizedAuthzType;
	}
	void setAuthorizedAuthzType(AuthzType authorizedAuthzType) {
		this.internalAuthorizedAuthzType = authorizedAuthzType;
	}

	@Override
	public AuthzAction getAuthorizedActionEnum() {
		return internalAuthorizedActionEnum;
	}
	void setAuthorizedActionEnum(AuthzAction authorizedActionEnum) {
		this.internalAuthorizedActionEnum = authorizedActionEnum;
	}

	@Override
	public boolean isAuthorizationInheritable() {
		return internalAuthorizationInheritable;
	}
	void setAuthorizationInheritable(boolean isAuthorizationInheritable) {
		this.internalAuthorizationInheritable = isAuthorizationInheritable;
	}

	@Override
	public boolean getAuthorizationWasRequired() {
		return internalAuthzWasRequired;
	}
	...
	public ArtifactExecutionInfoImpl setTrackArtifactHit(boolean tah) {
		trackArtifactHit = tah;
		return this;
	}
	@Override
	public boolean getAuthorizationWasGranted() {
		return internalAuthzWasGranted;
	}
	void setAuthorizationWasGranted(boolean value) {
		internalAuthzWasGranted = value ? Boolean.TRUE : Boolean.FALSE;
	}

	public Long getMoquiTxId() {
		return moquiTxId;
	}
	void setMoquiTxId(Long txId) {
		moquiTxId = txId;
	}

	ArtifactAuthzCheck getAacv() {
		return internalAacv;
	}
	...
	public long getRunningTime() {
		return endTimeNanos != 0 ? endTimeNanos - startTimeNanos : 0;
	}
	public double getRunningTimeMillisDouble() {
		return (endTimeNanos != 0 ? endTimeNanos - startTimeNanos : 0) / 1000000.0;
	}
	public long getRunningTimeMillisLong() {
		return Math.round((endTimeNanos != 0 ? endTimeNanos - startTimeNanos : 0) / 1000000.0);
	}
	private void calcChildTime(boolean recurse) {
		...
		if (childList != null)
			for (ArtifactExecutionInfoImpl aeii : childList) {
				...
				if (recurse)
					aeii.calcChildTime(true);
			}
	}
	@Override
	public long getThisRunningTime() {
		return getRunningTime() - getChildrenRunningTime();
	}
	@Override
	public long getChildrenRunningTime() {
		if (childrenRunningTime == 0)
			calcChildTime(false);
		return childrenRunningTime;
	}

	public BigDecimal getRunningTimeMillis() {
		return new BigDecimal(getRunningTime()).movePointLeft(6).setScale(2, RoundingMode.HALF_UP);
	}
	public BigDecimal getThisRunningTimeMillis() {
		return new BigDecimal(getThisRunningTime()).movePointLeft(6).setScale(2, RoundingMode.HALF_UP);
	}
	public BigDecimal getChildrenRunningTimeMillis() {
		return new BigDecimal(getChildrenRunningTime()).movePointLeft(6).setScale(2, RoundingMode.HALF_UP);
	}

	void setParent(ArtifactExecutionInfoImpl parentAeii) {
		this.parentAeii = parentAeii;
	}
	@Override
	public ArtifactExecutionInfo getParent() {
		return parentAeii;
	}
	@Override
	public BigDecimal getPercentOfParentTime() {
		return parentAeii != null && endTimeNanos != 0
				? new BigDecimal((getRunningTime() / parentAeii.getRunningTime()) * 100).setScale(2,
	...
	public void print(Writer writer, int level, boolean children) {
		try {
			for (int i = 0; i < (level * 2); i++)
				writer.append(" ");
			writer.append("[")
					.append(parentAeii != null
							? StringUtilities.paddedString(getPercentOfParentTime().toPlainString(), 5, false)
							: "     ")
					.append("%]");
			writer.append("[").append(StringUtilities.paddedString(getRunningTimeMillis().toPlainString(), 5, false))
					.append("]");
			writer.append("[")
					.append(StringUtilities.paddedString(getThisRunningTimeMillis().toPlainString(), 3, false))
					.append("]");
			writer.append("[")
					.append(childList != null
							? StringUtilities.paddedString(getChildrenRunningTimeMillis().toPlainString(), 3, false)
							: "   ")
					.append("] ");
			...
			if (children && childList != null)
				for (ArtifactExecutionInfoImpl aeii : childList)
					aeii.print(writer, level + 1, true);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	private String getKeyString() {
		return nameInternal + ":" + internalTypeEnum.name() + ":" + internalActionEnum.name() + ":" + actionDetail;
	}
	private String getKeyStringNoName() {
		return internalTypeEnum.name() + ":" + internalActionEnum.name() + ":" + actionDetail;
	}

	public static class ArtifactTypeStats {

		public int screenCount = 0, screenTransCount = 0, screenContentCount = 0, restPathCount = 0,
				serviceViewCount = 0, serviceOtherCount = 0, entityFindOneCount = 0, entityFindListCount = 0,
				entityFindIteratorCount = 0, entityFindCountCount = 0, entityCreateCount = 0, entityUpdateCount = 0,
				entityDeleteCount = 0;
		public long screenTime = 0, screenTransTime = 0, screenContentTime = 0, restPathTime = 0, serviceViewTime = 0,
				serviceOtherTime = 0, entityFindOneTime = 0, entityFindListTime = 0, entityFindIteratorTime = 0,
				entityFindCountTime = 0, entityCreateTime = 0, entityUpdateTime = 0, entityDeleteTime = 0;
		public void add(ArtifactTypeStats that) {
			if (that == null)
				return;

			screenCount += that.screenCount;
			screenTransCount += that.screenTransCount;
			screenContentCount += that.screenContentCount;
			restPathCount += that.restPathCount;
			serviceViewCount += that.serviceViewCount;
			serviceOtherCount += that.serviceOtherCount;
			entityFindOneCount += that.entityFindOneCount;
			entityFindListCount += that.entityFindListCount;
			entityFindIteratorCount += that.entityFindIteratorCount;
			entityFindCountCount += that.entityFindCountCount;
			entityCreateCount += that.entityCreateCount;
			entityUpdateCount += that.entityUpdateCount;
			entityDeleteCount += that.entityDeleteCount;

			screenTime += that.screenTime;
			screenTransTime += that.screenTransTime;
			screenContentTime += that.screenContentTime;
			restPathTime += that.restPathTime;
			serviceViewTime += that.serviceViewTime;
			serviceOtherTime += that.serviceOtherTime;
			entityFindOneTime += that.entityFindOneTime;
			entityFindListTime += that.entityFindListTime;
			entityFindIteratorTime += that.entityFindIteratorTime;
			entityFindCountTime += that.entityFindCountTime;
			entityCreateTime += that.entityCreateTime;
			entityUpdateTime += that.entityUpdateTime;
	...
	static void addArtifactTypeStats(ArrayList<ArtifactExecutionInfoImpl> aeiiList, ArtifactTypeStats stats) {
		...
		for (int i = 0; i < aeiiListSize; i++) {
			...
			// tight loop, use switch instead of if on these enums for much better
			// performance; run fast for use in on the fly accumulators
	...
	static List<Map<String, Object>> hotSpotByTime(List<ArtifactExecutionInfoImpl> aeiiList, boolean ownTime,
			String orderBy) {
		Map<String, Map<String, Object>> timeByArtifact = new LinkedHashMap<>();
		for (ArtifactExecutionInfoImpl aeii : aeiiList)
			aeii.addToMapByTime(timeByArtifact, ownTime);
		List<Map<String, Object>> hotSpotList = new LinkedList<>();
		hotSpotList.addAll(timeByArtifact.values());

		// in some cases we get REALLY long times before the system is warmed, knock
		// those out
		for (Map<String, Object> val : hotSpotList) {
			...
			if (knockOutCount > 0 && newTimes.size() > 0) {
				// calc new average, add knockOutCount times to fill in gaps, calc new time
				// total
				BigDecimal newTotal = BigDecimal.ZERO;
				BigDecimal newMax = BigDecimal.ZERO;
				for (BigDecimal time : newTimes) {
					newTotal = newTotal.add(time);
					if (time.compareTo(newMax) > 0)
						newMax = time;
				}
	...
	private void addToMapByTime(Map<String, Map<String, Object>> timeByArtifact, boolean ownTime) {
		...
		if (val == null) {
			...
			newMap.put("times", timesList);
			newMap.put("time", curTime);
			newMap.put("timeMin", curTime);
			newMap.put("timeMax", curTime);
			newMap.put("timeAvg", curTime);
			newMap.put("count", BigDecimal.ONE);
			newMap.put("name", nameInternal);
			newMap.put("actionDetail", actionDetail);
			newMap.put("type", getTypeDescription());
			newMap.put("action", getActionDescription());
			...
			if (newCount.intValue() == 2 && ((List<BigDecimal>) val.get("times")).get(0)
					.compareTo(curTime.multiply(new BigDecimal(3))) > 0) {
				// if the first is much higher than the 2nd, use the 2nd for both
				List<BigDecimal> timesList = new LinkedList<>();
				timesList.add(curTime);
				timesList.add(curTime);
				val.put("times", timesList);
				val.put("time", curTime.add(curTime));
				val.put("timeMin", curTime);
				val.put("timeMax", curTime);
				val.put("timeAvg", curTime);
			} else {
				((List<BigDecimal>) val.get("times")).add(curTime);
				val.put("time", ((BigDecimal) val.get("time")).add(curTime));
				val.put("timeMin",
						((BigDecimal) val.get("timeMin")).compareTo(curTime) > 0
								? curTime
								: (BigDecimal) val.get("timeMin"));
				val.put("timeMax",
						((BigDecimal) val.get("timeMax")).compareTo(curTime) > 0
								? (BigDecimal) val.get("timeMax")
								: curTime);
				val.put("timeAvg",
						((BigDecimal) val.get("time")).divide((BigDecimal) val.get("count"), 2, RoundingMode.HALF_UP));
			}
		}
		if (childList != null)
			for (ArtifactExecutionInfoImpl aeii : childList)
	...
	static void printHotSpotList(Writer writer, List<Map> infoList) throws IOException {
		// "[${time}:${timeMin}:${timeAvg}:${timeMax}][${count}] ${type} ${action}
		// ${actionDetail} ${name}"
		for (Map info : infoList) {
			writer.append("[")
					.append(StringUtilities.paddedString(((BigDecimal) info.get("time")).toPlainString(), 8, false))
					.append(":");
			writer.append(StringUtilities.paddedString(((BigDecimal) info.get("timeMin")).toPlainString(), 7, false))
					.append(":");
			writer.append(StringUtilities.paddedString(((BigDecimal) info.get("timeAvg")).toPlainString(), 7, false))
					.append(":");
			writer.append(StringUtilities.paddedString(((BigDecimal) info.get("timeMax")).toPlainString(), 7, false))
					.append("]");
			writer.append("[")
					.append(StringUtilities.paddedString(((BigDecimal) info.get("count")).toPlainString(), 4, false))
					.append("] ");
	...
	static List<Map> consolidateArtifactInfo(List<ArtifactExecutionInfoImpl> aeiiList) {
		...
		for (ArtifactExecutionInfoImpl aeii : aeiiList)
			aeii.consolidateArtifactInfo(topLevelList, flatMap, null);
		return topLevelList;
	}
	@SuppressWarnings("unchecked")
	private void consolidateArtifactInfo(List<Map> topLevelList, Map<String, Map<String, Object>> flatMap,
			Map parentArtifactMap) {
		String key = getKeyString();
		Map<String, Object> artifactMap = flatMap.get(key);
		if (artifactMap == null) {
			artifactMap = new LinkedHashMap<>();
			artifactMap.put("time", getRunningTimeMillis());
			artifactMap.put("thisTime", getThisRunningTimeMillis());
			artifactMap.put("childrenTime", getChildrenRunningTimeMillis());
			artifactMap.put("count", BigDecimal.ONE);
			artifactMap.put("name", nameInternal);
			artifactMap.put("actionDetail", actionDetail);
			artifactMap.put("childInfoList", new LinkedList());
			artifactMap.put("key", key);
			artifactMap.put("type", getTypeDescription());
			artifactMap.put("action", getActionDescription());
			...
			artifactMap.put("childrenTime",
					((BigDecimal) artifactMap.get("childrenTime")).add(getChildrenRunningTimeMillis()));
			if (parentArtifactMap != null) {
				// is the current artifact in the current parent"s child list? if not add it (a
				// given artifact may be under multiple parents, normal)
				boolean foundMap = false;
				for (Map candidate : (List<Map>) parentArtifactMap.get("childInfoList"))
					if (key.equals(candidate.get("key"))) {
						foundMap = true;
						break;
					}
				if (!foundMap)
		...
		if (childList != null)
			for (ArtifactExecutionInfoImpl aeii : childList)
				aeii.consolidateArtifactInfo(topLevelList, flatMap, artifactMap);
	}
	...
	public static void printArtifactInfoList(Writer writer, List<Map> infoList, int level) throws IOException {
		// "[${time}:${thisTime}:${childrenTime}][${count}] ${type} ${action}
		// ${actionDetail} ${name}"
		for (Map info : infoList) {
			...
			writer.append("[")
					.append(StringUtilities.paddedString(((BigDecimal) info.get("time")).toPlainString(), 8, false))
					.append(":");
			writer.append(StringUtilities.paddedString(((BigDecimal) info.get("thisTime")).toPlainString(), 6, false))
					.append(":");
			writer.append(
					StringUtilities.paddedString(((BigDecimal) info.get("childrenTime")).toPlainString(), 6, false))
					.append("]");
			writer.append("[")
					.append(StringUtilities.paddedString(((BigDecimal) info.get("count")).toPlainString(), 4, false))
					.append("] ");
	...
	public String toString() {
		return "[name:'" + nameInternal + "', type:'" + internalTypeEnum + "', action:'" + internalActionEnum
				+ "', required: " + internalAuthzWasRequired + ", granted:" + internalAuthzWasGranted + ", user:'"
				+ internalAuthorizedUserId + "', authz:'" + internalAuthorizedAuthzType + "', authAction:'"
				+ internalAuthorizedActionEnum + "', inheritable:" + internalAuthorizationInheritable + ", runningTime:"
				+ getRunningTime() + "', txId:" + moquiTxId + "]";
	}
	@Override
	...
	public static class ArtifactAuthzCheck {
		...
		@Override
		public String toString() {
			return "[userGroupId:" + userGroupId + ", artifactAuthzId:" + artifactAuthzId + ", artifactGroupId:"
					+ artifactGroupId + ", artifactName:" + artifactName + ", artifactType:" + artifactType
					+ ", authzAction:" + authzAction + ", authzType:" + authzType + ", nameIsPattern:" + nameIsPattern
					+ ", inheritAuthz:" + inheritAuthz + "]";
		}
	}
}

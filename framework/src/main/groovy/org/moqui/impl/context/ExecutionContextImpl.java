...
public class ExecutionContextImpl implements ExecutionContext {
	...
	public ExecutionContextImpl(ExecutionContextFactoryImpl ecfi, Thread forThread) {
		...
		if (cacheFacade == null)
			throw new IllegalStateException("cacheFacade was null");
		if (loggerFacade == null)
			throw new IllegalStateException("loggerFacade was null");
		if (resourceFacade == null)
			throw new IllegalStateException("resourceFacade was null");
		if (screenFacade == null)
			throw new IllegalStateException("screenFacade was null");
		if (serviceFacade == null)
			throw new IllegalStateException("serviceFacade was null");
		if (transactionFacade == null)
			throw new IllegalStateException("transactionFacade was null");
	...
	Cache<String, String> getL10nMessageCache() {
		return l10nMessageCache;
	}
	public Cache<String, ArrayList> getTarpitHitCache() {
		return tarpitHitCache;
	}

	@Override
	public @Nonnull ExecutionContextFactory getFactory() {
		return ecfi;
	}

	@Override
	public @Nonnull ContextStack getContext() {
		return contextStack;
	}
	@Override
	public @Nonnull Map<String, Object> getContextRoot() {
		return contextStack.getRootMap();
	}
	@Override
	public @Nonnull ContextBinding getContextBinding() {
		return contextBindingInternal;
	}
	...
	@Override
	public @Nullable WebFacade getWeb() {
		return webFacade;
	}
	public @Nullable WebFacadeImpl getWebImpl() {
		return webFacadeImpl;
	}

	@Override
	public @Nonnull UserFacade getUser() {
		return userFacade;
	}
	@Override
	public @Nonnull MessageFacade getMessage() {
		return messageFacade;
	}
	@Override
	public @Nonnull ArtifactExecutionFacade getArtifactExecution() {
		return artifactExecutionFacade;
	}
	@Override
	public @Nonnull L10nFacade getL10n() {
		return l10nFacade;
	}
	@Override
	public @Nonnull ResourceFacade getResource() {
		return resourceFacade;
	}
	@Override
	public @Nonnull LoggerFacade getLogger() {
		return loggerFacade;
	}
	@Override
	public @Nonnull CacheFacade getCache() {
		return cacheFacade;
	}
	@Override
	public @Nonnull TransactionFacade getTransaction() {
		return transactionFacade;
	}

	@Override
	public @Nonnull EntityFacade getEntity() {
		return activeEntityFacade;
	}
	public @Nonnull EntityFacadeImpl getEntityFacade() {
		return activeEntityFacade;
	}

	@Override
	public @Nonnull ElasticFacade getElastic() {
		return ecfi.elasticFacade;
	}
	@Override
	public @Nonnull ServiceFacade getService() {
		return serviceFacade;
	}
	@Override
	public @Nonnull ScreenFacade getScreen() {
		return screenFacade;
	}
	...
	public @Nonnull List<NotificationMessage> getNotificationMessages(@Nullable String topic) {
		...
		try {
			EntityFind nmbuFind = activeEntityFacade.find("moqui.security.user.NotificationMessageByUser")
			...
			if (!alreadyDisabled)
				artifactExecutionFacade.enableAuthz();
		}
	...
	@Override
	public void initWebFacade(@Nonnull String webappMoquiName, @Nonnull HttpServletRequest request,
		...
		// for convenience (and more consistent code in screen actions, services, etc)
		// add all requestParameters to the context
	...
	public boolean getSkipStats() {
		...
		if (webFacade != null)
			skipParms.put("pathInfo", webFacade.getPathInfo());
		skipStats = (skipStatsCond != null && !skipStatsCond.isEmpty())
				&& ecfi.resourceFacade.condition(skipStatsCond, null, skipParms);
		return skipStats;
	}
	...
	/**
	 * Uses the ECFI constructor for ThreadPoolRunnable so does NOT use the current
	 * ECI in the separate thread
	 */
	...
	@Override
	public void destroy() {
		// if webFacade exists this is the end of a request, so trigger after-request
		// actions
		...
		if (loggerDirect.isTraceEnabled())
			loggerDirect.trace("ExecutionContextImpl destroyed");
	}
	...
	public static class ThreadPoolRunnable implements Runnable {
		...
		/**
		 * With this constructor (passing ECFI) a new ECI is created for the separate
		 * thread
		 */
		...
		public void run() {
			...
			try {
				...
				// now using separate ECI in thread so always destroy, ie don't do: if
				// (threadEci == null)

...
public class EntityJavaUtil {
	...
	static String enDeCrypt(String value, boolean encrypt, EntityFacadeImpl efi) {
		...
		if (encrypt) {
			...
			// decrypt a bit different, use entity-facade as config node and then
			// decrypt-alt until success, or fail with original error
			try {
	...
	static String enDeCrypt(String value, boolean encrypt, MNode configNode) {
		...
		if (algo == null || algo.length() == 0)
			algo = "PBEWithHmacSHA256AndAES_128";

		// logger.info("TOREMOVE salt [" + salt + "] count [" + count + "] pass
		// [${pass}] algo [" + algo + "][" + configNode.attribute("crypt-algo") + "]");
		...
		try {
			...
			if (encrypt) {
				...
				/*
				 * more secure for larger multi-block values, but makes find by encrypted value
				 * impossible, maybe optionally enable with another field.@encrypt attribute if
				 * ever needed initVectorBytes = new byte[pbeCipher.getBlockSize()]; new
				 * SecureRandom().nextBytes(initVectorBytes);
				 */
			} else {
				// if contains ':' is the new format: split IV and value then decode using
				// Base64, otherwise decode value as hex
			...
			PBEParameterSpec pbeParamSpec = initVectorBytes == null
			...
			// change to Base64 encode always (2/3 size with 6 bits/char base64 vs 4
			// bits/char hex), always include IV + ':' + encrypted value
			if (encrypt) {
				// old hex approach, now supported for decrypt only: return
				// DatatypeConverter.printHexBinary(outBytes);
				if (defaultInitVectorBytes == initVectorBytes) {
					return ":" + Base64.getUrlEncoder().encodeToString(outBytes);
				} else {
					return Base64.getUrlEncoder().encodeToString(initVectorBytes) + ':'
							+ Base64.getUrlEncoder().encodeToString(outBytes);
				}
			} else {
				return new String(outBytes);
			}
		} catch (Exception e) {
			// logger.warn("crypt-pass " + pwStr + " salt " + saltStr + " algo " + algo + "
			// count " + count);
	...
	public static FieldOrderOptions makeFieldOrderOptions(String orderByName) {
		return new FieldOrderOptions(orderByName);
	}
	public static class FieldOrderOptions {
		...
		Boolean getNullsFirstLast() {
			return nullsFirstLast;
		}
		public boolean getDescending() {
			return descending;
		}
		Boolean getCaseUpperLower() {
		...
		public FieldOrderOptions(String orderByName) {
			...
			for (int i = 0; i < obnLength; i++) {
				...
				if (curChar == spaceChar) {
					...
					// leading characters (-,+,^), don't consider them non-spaces so we'll remove
					// spaces after
			...
			if (containsSpace) {
				// trim ending spaces
				while (fnSb.charAt(fnSb.length() - 1) == spaceChar)
					fnSb.delete(fnSb.length() - 1, fnSb.length());
			...
			if (containsOpenParen) {
				...
				int fnSbLength = fnSb.length();
				if (fnSb.charAt(fnSbLength - 1) == closeParenChar)
					fnSb.delete(fnSbLength - 1, fnSbLength);
			}
	...
	public static class EntityInfo {
		...
		EntityInfo(EntityDefinition ed, boolean memberNeverCache) {
			...
			if (packageName == null || packageName.isEmpty())
				packageName = internalEntityNode.attribute("package-name");
			...
			isInvalidViewEntity = isView
					&& (!internalEntityNode.hasChild("member-entity") || !internalEntityNode.hasChild("alias"));
			...
			if (tableNameAttr == null || tableNameAttr.isEmpty())
				tableNameAttr = EntityJavaUtil.camelCaseToUnderscored(internalEntityName);
			...
			if (schemaNameAttr != null && schemaNameAttr.length() == 0)
				schemaNameAttr = null;
			...
			if (spsAttr != null && !spsAttr.isEmpty())
				sequencePrimaryStagger = Long.parseLong(spsAttr);
			else
				sequencePrimaryStagger = 1;

			String sbsAttr = internalEntityNode.attribute("sequence-bank-size");
			if (sbsAttr != null && !sbsAttr.isEmpty())
				sequenceBankSize = Long.parseLong(sbsAttr);
			else
				sequenceBankSize = EntityFacadeImpl.defaultBankSize;
			...
			authorizeSkipCreate = authorizeSkipTrue
					|| (authorizeSkipStr != null && authorizeSkipStr.contains("create"));
			authorizeSkipView = authorizeSkipTrue || (authorizeSkipStr != null && authorizeSkipStr.contains("view"));

			// NOTE: see code in initFields that may set this to never if any member-entity
			// is set to cache=never
			if (memberNeverCache) {
				useCache = "never";
				neverCache = true;
			} else {
				String cacheAttr = internalEntityNode.attribute("cache");
				if (cacheAttr == null || cacheAttr.isEmpty())
					cacheAttr = "false";
				useCache = cacheAttr;
			...
			for (int i = 0; i < allFieldInfoSize; i++) {
				...
				if (fi.createOnly)
					createOnlyFieldsTemp = true;
				if ("true".equals(fi.enableAuditLog) || "update".equals(fi.enableAuditLog))
					needsAuditLogTemp = true;
				if ("true".equals(fi.fieldNode.attribute("encrypt")))
					needsEncryptTemp = true;
				...
				if (defaultStr != null && !defaultStr.isEmpty()) {
					if (fi.isPk)
						pkFieldDefaultsTemp.put(fi.name, defaultStr);
					else
						nonPkFieldDefaultsTemp.put(fi.name, defaultStr);
				}
				if ("lastUpdatedStamp".equals(fi.name))
					lastUpdatedTemp = fi;
			}
		...
		void setFields(Map<String, Object> src, Map<String, Object> dest, boolean setIfEmpty, String namePrefix,
				Boolean pks) {
			...
			FieldInfo[] fieldInfoArray = pks == null
					? allFieldInfoArray
					: (pks == Boolean.TRUE ? pkFieldInfoArray : nonPkFieldInfoArray);
			// use integer iterator, saves quite a bit of time, improves time for this
			// method by about 20% with this alone
			...
			for (int i = 0; i < size; i++) {
				...
				if (srcIsEntityValueBase) {
					value = hasNamePrefix
							? srcEvb.valueMapInternal.get(srcName)
							: srcEvb.valueMapInternal.getByIString(fi.name, fi.index);
					srcContains = value != null || (hasNamePrefix
							? srcEvb.valueMapInternal.containsKey(srcName)
							: srcEvb.valueMapInternal.containsKeyIString(fi.name, fi.index));
				} else {
					...
					if (!isEmpty) {
						if (isCharSequence) {
							try {
								...
								if (destIsEntityValueBase)
									destEvb.putKnownField(fi, converted);
								else
									dest.put(fieldName, converted);
							} catch (BaseException be) {
								eci.messageFacade.addValidationError(null, fieldName, null, be.getMessage(), be);
							}
						} else {
							if (destIsEntityValueBase)
								destEvb.putKnownField(fi, value);
							else
								dest.put(fieldName, value);
						}
					} else if (setIfEmpty) {
						// treat empty String as null, otherwise set as whatever null or empty type it
						// is
						if (value != null && isCharSequence) {
							if (destIsEntityValueBase)
								destEvb.putKnownField(fi, null);
							else
		...
		public Map<String, Object> cloneMapRemoveFields(Map<String, Object> theMap, Boolean pks) {
			...
			// ArrayList<String> fieldNameList = (pks != null ? this.getFieldNames(pks,
			// !pks, !pks) : this.getAllFieldNames())
			FieldInfo[] fieldInfoArray = pks == null
					? allFieldInfoArray
					: (pks == Boolean.TRUE ? pkFieldInfoArray : nonPkFieldInfoArray);
			int size = fieldInfoArray.length;
	...
	public static class RelationshipInfo {
		...
		RelationshipInfo(MNode relNode, EntityDefinition fromEd, EntityFacadeImpl efi) {
			...
			if (relatedAttr == null || relatedAttr.isEmpty())
				relatedAttr = relNode.attribute("related-entity-name");
			relatedEd = efi.getEntityDefinition(relatedAttr);
			if (relatedEd == null)
				throw new EntityNotFoundException("Invalid entity relationship, " + relatedAttr
						+ " not found in definition for entity " + fromEd.getFullEntityName());
		...
		// some methods for FTL templates that don't access member fields, just call
		// getters; don't follow getter pattern so groovy code won't pick them up
		public String riPrettyName() {
			return prettyName;
		}
		public String riRelatedEntityName() {
		...
		private boolean hasReverse() {
			...
			for (int i = 0; i < relatedRelListSize; i++) {
				...
				if (relatedAttr == null || relatedAttr.isEmpty())
					relatedAttr = reverseRelNode.attribute("related-entity-name");
				...
				if ((fromEd.entityInfo.fullEntityName.equals(relatedAttr)
						|| fromEd.entityInfo.internalEntityName.equals(relatedAttr))
		...
		public RelationshipInfo findReverse() {
			...
			for (int i = 0; i < relInfoListSize; i++) {
				...
				// TODO: instead of checking title check reverse expanded key-map
				if (fromEd.fullEntityName.equals(relInfo.relatedEntityName) && ((title == null && relInfo.title == null)
						|| (title != null && title.equals(relInfo.title)))) {
					return relInfo;
				}
			}
			return null;
		...
		public Map<String, Object> getTargetParameterMap(Map valueSource) {
			...
			for (Map.Entry<String, String> keyEntry : keyMap.entrySet()) {
				...
				if (!ObjectUtilities.isEmpty(value))
					targetParameterMap.put(keyEntry.getValue(), value);
			}
			if (keyValueMap != null) {
		...
		public String toString() {
			return relationshipName + (shortAlias != null ? " (" + shortAlias + ")" : "") + ", type " + type + ", one? "
					+ isTypeOne + ", dependent? " + dependent;
		}
	}
	...
	public static String underscoredToCamelCase(String underscored, boolean firstUpper) {
		if (underscored == null || underscored.length() == 0)
			return "";

		StringBuilder camelCased = new StringBuilder();
		camelCased.append(firstUpper
				? Character.toUpperCase(underscored.charAt(0))
				: Character.toLowerCase(underscored.charAt(0)));
		int inPos = 1;
	...
	public static class QueryStatsInfo {
		...
		public void countHit(EntityFacadeImpl efi, long runTimeNanos, boolean isError) {
			...
			if (isError)
				errorCount++;
			if (runTimeNanos < minTimeNanos)
				minTimeNanos = runTimeNanos;
			if (runTimeNanos > maxTimeNanos)
		...
		double getAverage() {
			return hitCount > 0 ? totalTimeNanos / hitCount : 0;
		}
		double getStdDev() {
			if (hitCount < 2)
				return 0;
			return Math.sqrt(
					Math.abs(totalSquaredTime - ((totalTimeNanos * totalTimeNanos) / hitCount)) / (hitCount - 1L));
		}
		final static long nanosDivisor = 1000;
		public Map<String, Object> makeDisplayMap() {
			Map<String, Object> dm = new HashMap<>();
			dm.put("entityName", entityName);
			dm.put("sql", sql);
			dm.put("hitCount", hitCount);
			dm.put("errorCount", errorCount);
			dm.put("minTime", new BigDecimal(minTimeNanos / nanosDivisor));
			dm.put("maxTime", new BigDecimal(maxTimeNanos / nanosDivisor));
			dm.put("totalTime", new BigDecimal(totalTimeNanos / nanosDivisor));
			dm.put("totalSquaredTime", new BigDecimal(totalSquaredTime / nanosDivisor));
			dm.put("average", new BigDecimal(getAverage() / nanosDivisor));
			dm.put("stdDev", new BigDecimal(getStdDev() / nanosDivisor));
	...
	public static class EntityWriteInfo {
		...
		public EntityWriteInfo(EntityValueBase evb, WriteMode writeMode) {
			// clone value so that create/update/delete stays the same no matter what
			// happens after
	...
	public static class FindAugmentInfo {
		...
		public FindAugmentInfo(ArrayList<EntityValueBase> valueList, Set<Map<String, Object>> foundUpdated,
				EntityCondition econd) {
			this.valueList = valueList;
			valueListSize = valueList.size();
			this.foundUpdated = foundUpdated;
			this.econd = econd;
		}
	}

	/*
	 * added as a possibility for EntityValueBase.checkAgainstDatabaseInfo() but
	 * simpler for interfaces, sorting, etc to use a Map: public static class
	 * EntityValueDiffInfo { public String entityName, fieldName; public Map<String,
	 * Object> pkValues; public Object checkValue, dbValue; public boolean notFound;
	 * public EntityValueDiffInfo(String entityName, Map<String, Object> pkValues) {
	 * this.entityName = entityName; this.fieldName = null; this.pkValues =
	 * pkValues; this.checkValue = null; this.dbValue = null; this.notFound = true;
	 * } public EntityValueDiffInfo(String entityName, Map<String, Object> pkValues,
	 * String fieldName, Object checkValue, Object dbValue) { this.entityName =
	 * entityName; this.fieldName = fieldName; this.pkValues = pkValues;
	 * this.checkValue = checkValue; this.dbValue = dbValue; this.notFound = false;
	 * } }
	 */
}

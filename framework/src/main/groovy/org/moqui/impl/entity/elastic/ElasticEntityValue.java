...
public class ElasticEntityValue extends EntityValueBase {
	...
	public ElasticDatasourceFactory getEdf() {
		if (edfInternal == null) {
			...
			edfInternal = (ElasticDatasourceFactory) ef
					.getDatasourceFactory(ef.getEntityGroupName(resolveEntityName()));
		}
	...
	public EntityValue cloneValue() {
		...
		// don't set mutable (default to mutable even if original was not) or modified
	...
	public void createExtended(FieldInfo[] fieldInfoArray, Connection con) {
		...
		if (ed.isViewEntity)
			throw new EntityException("View entities are not supported, Elastic/OpenSearch does not support joins");
		...
		// logger.warn("create elastic combinedId " + combinedId + " valueMapInternal "
		// + valueMapInternal);
	...
	public void updateExtended(FieldInfo[] pkFieldArray, FieldInfo[] nonPkFieldArray, Connection con) {
		...
		String combinedId = getPrimaryKeysString();
		// use ElasticClient.update() method, supports partial doc update, see

...
public class EntityFindBuilder extends EntityQueryBuilder {
	...
	public void makeCountFunction(FieldOrderOptions[] fieldOptionsArray, boolean isDistinct, boolean isGroupBy) {
		...
		if (isGroupBy || (isDistinct && fiaLength > 0)) {
			...
			if (isDistinct)
				sqlTopLevel.append("DISTINCT ");
			// NOTE: regardless of DB configuration (database.@add-unique-as) it is always
			// needed across various DBs in this case, including MySQL
			...
			if (isDistinct) {
				...
				// NOTE: on H2 COUNT(*) is faster than COUNT(1) (and perhaps other databases?
				// docs hint may be faster in MySQL)
	...
	public void closeCountSubSelect(int fiaLength, boolean isDistinct, boolean isGroupBy) {
		if (isGroupBy || (isDistinct && fiaLength > 0))
			sqlTopLevel.append(") TEMP_NAME");
	}

	public void expandJoinFromAlias(final MNode entityNode, final String searchEntityAlias,
			Set<String> entityAliasUsedSet, Set<String> entityAliasesJoinedInSet) {
		...
		if (memberEntityNode == null)
			throw new EntityException("Could not find member-entity with entity-alias " + searchEntityAlias
					+ " in view-entity " + entityNode.attribute("entity-name"));
		String joinFromAlias = memberEntityNode.attribute("join-from-alias");
		if (joinFromAlias == null || joinFromAlias.length() == 0)
			throw new EntityException(
					"In view-entity " + entityNode.attribute("entity-name") + " the member-entity for entity-alias "
							+ searchEntityAlias + " has no join-from-alias and is not the first member-entity");
		if (entityAliasesJoinedInSet.contains(joinFromAlias)) {
			...
			// recurse to find member-entity with joinFromAlias, add in its joinFromAlias
			// until one is found that is already in the set
			expandJoinFromAlias(entityNode, joinFromAlias, entityAliasUsedSet, entityAliasesJoinedInSet);
			// if no exception from an alias not found or not joined in then we found a join
			// path back so add in the current search alias
			entityAliasesJoinedInSet.add(searchEntityAlias);
			entityAliasUsedSet.add(searchEntityAlias);
		}
	}
	...
	public EntityConditionImplBase makeSqlFromClause(final EntityDefinition localEntityDefinition,
			StringBuilder localBuilder, EntityConditionImplBase localWhereCondition,
			EntityConditionImplBase localHavingCondition, Set<String> additionalFieldsUsed) {
		localBuilder.append(" FROM ");

		EntityConditionImplBase outWhereCondition = localWhereCondition;

		// TODO: bug in Groovy 3.0.10 that somehow flips the isViewEntity boolean; can
		// remove this once resolved with a future version of Groovy
		// if (localEntityDefinition.fullEntityName.contains("ArtifactTarpitCheckView")
		// || localEntityDefinition.fullEntityName.contains("DataFeedDocumentDetail"))
		// logger.warn("===== TOREMOVE ===== localEntityDefinition " +
		// localEntityDefinition.fullEntityName + " isViewEntity " +
		// localEntityDefinition.isViewEntity + " " + localEntityDefinition);
		...
		if (localEntityDefinition.isViewEntity) {
			...
			// get a list of all aliased fields selected or ordered by and don't bother
			// joining in a member-entity
			...
			if (viewWhere != null)
				viewWhere.getAllAliases(entityAliasUsedSet, fieldUsedSet);
			if (localWhereCondition != null)
				localWhereCondition.getAllAliases(entityAliasUsedSet, fieldUsedSet);
			if (localHavingCondition != null)
				localHavingCondition.getAllAliases(entityAliasUsedSet, fieldUsedSet);

			// logger.warn("SQL from viewWhere " + viewWhere + " localWhereCondition " +
			// localWhereCondition + " localHavingCondition " + localHavingCondition);
			// logger.warn("SQL from fieldUsedSet " + fieldUsedSet + " additionalFieldsUsed
			...
			if (additionalFieldsUsed == null) {
				...
				// additional fields to look for, when this is a sub-select for a member-entity
				// that is a view-entity
			...
			for (String fieldName : fieldUsedSet) {
				...
				if (fi == null)
					throw new EntityException("Could not find field " + fieldName + " in entity "
							+ localEntityDefinition.getFullEntityName());
				entityAliasUsedSet.addAll(fi.entityAliasUsedSet);
			}

			// if (localEntityDefinition.getFullEntityName().contains("Example"))
			// logger.warn("============== entityAliasUsedSet=${entityAliasUsedSet} for
			// entity ${localEntityDefinition.entityName}\n fieldUsedSet=${fieldUsedSet}\n
			...
			ArrayList<EntityConditionImplBase> filterCondList = efi.ecfi.getEci().artifactExecutionFacade
					.filterFindForUser(localEntityDefinition, entityAliasUsedSet);
			outWhereCondition = EntityConditionFactoryImpl.addAndListToCondition(outWhereCondition, filterCondList);

			// keep a set of all aliases in the join so far and if the left entity alias
			// isn't there yet, and this
			// isn't the first one, throw an exception
			final Set<String> joinedAliasSet = new TreeSet<>();

			// on initial pass only add opening parenthesis since easier than going back and
			// inserting them, then insert the rest
			...
			for (int meInd = 0; meInd < memberEntityNodesSize; meInd++) {
				...
				// logger.warn("=================== joining member-entity
				// ${relatedMemberEntity}")

				// if this isn't joined in skip it (should be first one only); the first is
				// handled below
				if (joinFromAlias == null || joinFromAlias.length() == 0)
					continue;
				...
				if (!entityAliasUsedSet.contains(entityAlias))
					continue;
				if (!entityAliasUsedSet.contains(joinFromAlias))
					continue;

				if (isFirst && useParenthesis)
					localBuilder.append("(");
				...
				if (isFirst) {
					// first link, add link entity for this one only, for others add related link
					// entity
					outWhereCondition = makeSqlViewTableName(linkEntityDefinition, localBuilder, outWhereCondition,
							localHavingCondition);
					...
					if (!joinedAliasSet.contains(joinFromAlias)) {
						logger.error("For view-entity [" + localEntityDefinition.fullEntityName
								...
								+ "] that is not in the joinedAliasSet: " + joinedAliasSet + "; view-entity Node: "
								+ entityNode);
				...
				if ("true".equals(subSelectAttr)) {
					...
					if (fromLateralStyle == null || fromLateralStyle.isEmpty())
						fromLateralStyle = "none";
				}
				boolean isLateralStyle = "lateral".equals(fromLateralStyle);
				boolean isApplyStyle = "apply".equals(fromLateralStyle);
				if (isApplyStyle)
					logger.warn("from-lateral-style=apply not yet supported, using non-lateral join for sub-select in "
							+ localEntityDefinition.getFullEntityName());

				// TODO: for isApplyStyle need to use CROSS APPLY or OUTER APPLY for
				// join-optional=true INSTEAD of [INNER|OUTER LEFT] JOIN in calling code
				...
				if (subSelect) {
					makeSqlMemberSubSelect(entityAlias, relatedMemberEntityNode, relatedLinkEntityDefinition,
							linkEntityDefinition, localBuilder);
				} else {
					outWhereCondition = makeSqlViewTableName(relatedLinkEntityDefinition, localBuilder,
							outWhereCondition, localHavingCondition);
				}
				...
				if (isLateralStyle) {
					...
					appendJoinConditions(relatedMemberEntityNode, entityAlias, localEntityDefinition,
							linkEntityDefinition, relatedLinkEntityDefinition, localBuilder);
				}

				isFirst = false;
			}

			if (!fromEmpty && useParenthesis)
				localBuilder.append(")");

			// handle member-entities not referenced in any member-entity.@join-from-alias
			// attribute
			for (int meInd = 0; meInd < memberEntityNodesSize; meInd++) {
				...
				// if entity alias not used don't join it in
				if (!entityAliasUsedSet.contains(memberEntityAlias))
					continue;
				if (joinedAliasSet.contains(memberEntityAlias))
					continue;
				...
				if ("true".equals(subSelectAttr) || "non-lateral".equals(subSelectAttr)) {
					...
					outWhereCondition = makeSqlViewTableName(fromEntityDefinition, localBuilder, outWhereCondition,
							localHavingCondition);
				}
			...
			ArrayList<EntityConditionImplBase> filterCondList = efi.ecfi.getEci().artifactExecutionFacade
					.filterFindForUser(localEntityDefinition, null);
			outWhereCondition = EntityConditionFactoryImpl.addAndListToCondition(outWhereCondition, filterCondList);
	...
	public void appendJoinConditions(MNode relatedMemberEntityNode, String entityAlias,
			EntityDefinition localEntityDefinition, EntityDefinition linkEntityDefinition,
			EntityDefinition relatedLinkEntityDefinition, StringBuilder localBuilder) {
		final String joinFromAlias = relatedMemberEntityNode.attribute("join-from-alias");
		...
		if ((keyMaps == null || keyMaps.size() == 0)
				&& (entityConditionList == null || entityConditionList.size() == 0)) {
			throw new EntityException(
					"No member-entity/join key-maps found for the " + joinFromAlias + " and the " + entityAlias
							+ " member-entities of the " + localEntityDefinition.fullEntityName + " view-entity.");
		}
		...
		for (int i = 0; i < keyMapsSize; i++) {
			...
			for (int ai = 0; ai < aliasNodes.size(); ai++) {
				...
				if (joinFromAlias.equals(curAliasNode.attribute("entity-alias"))) {
					...
					if (curFieldName == null || curFieldName.isEmpty())
						curFieldName = curAliasNode.attribute("name");
			...
			String relatedFieldName = relatedAttr != null && !relatedAttr.isEmpty()
					? relatedAttr
					: keyMap.attribute("related-field-name");
			...
			if (!relatedLinkEntityDefinition.isField(relatedFieldName)
				...
				// if we don't match these constraints and get this default we'll get an error
				// later...
			}

			if (entityAlias != null && !entityAlias.isEmpty())
				localBuilder.append(entityAlias).append(".");
			FieldInfo relatedFieldInfo = relatedLinkEntityDefinition.getFieldInfo(relatedFieldName);
			if (relatedFieldInfo == null)
				throw new EntityException("Invalid field name " + relatedFieldName + " for entity "
						+ relatedLinkEntityDefinition.fullEntityName);
			...
			// NOTE: sanitizeColumnName here breaks the generated SQL, in the case of a view
			// within a view we want EAO.EAI.COL_NAME...
			// localBuilder.append(sanitizeColumnName(relatedLinkEntityDefinition.getColumnName(relatedFieldName,
			// false)))
		}
		...
		if (entityConditionList != null && entityConditionList.size() > 0) {
			...
			// logger.warn("======== appendJoinConditions() localEntityDefinition " +
			// localEntityDefinition.fullEntityName + " linkEntityDefinition " +
			// linkEntityDefinition.fullEntityName + " relatedLinkEntityDefinition " +
			// relatedLinkEntityDefinition.fullEntityName);
	...
	public EntityConditionImplBase makeSqlViewTableName(EntityDefinition localEntityDefinition,
			StringBuilder localBuilder, EntityConditionImplBase localWhereCondition,
			EntityConditionImplBase localHavingCondition) {
		...
		if (entityInfo.isView) {
			...
			for (int i = 0; i < aliasListSize; i++) {
				...
				if (aliasField == null || aliasField.length() == 0)
				...
				// TODO: are the next two lines really needed? have removed AS stuff elsewhere
				// since it is not commonly used and not needed
				// localBuilder.append(" AS ")
				// localBuilder.append(sanitizeColumnName(localEntityDefinition.getColumnName(aliasName),
				// false)))
			}

			// pass through localWhereCondition in case changed
			outWhereCondition = makeSqlFromClause(localEntityDefinition, localBuilder, localWhereCondition,
					localHavingCondition, additionalFieldsUsed);

			// TODO: refactor this like below to do in the main loop; this is currently
			// unused though (view-entity as member-entity for sub-select)
			StringBuilder gbClause = new StringBuilder();
			if (entityInfo.hasFunctionAlias) {
				// do a different approach to GROUP BY: add all fields that are selected and
				...
				for (int i = 0; i < aliasListSize; i++) {
					...
					boolean isAggFunction = isAggregateAttr != null
							? "true".equalsIgnoreCase(isAggregateAttr)
	...
	public void makeSqlMemberSubSelect(String entityAlias, MNode memberEntity, EntityDefinition localEntityDefinition,
		...
		if (isLateralStyle)
			localBuilder.append(" LATERAL ");
		localBuilder.append("(SELECT ");

		// add any fields needed to join this to another member-entity, even if not in
		// the main set of selected fields
		...
		for (int i = 0; i < keyMapList.size(); i++) {
			...
			if (relFn == null || relFn.isEmpty())
				relFn = keyMap.attribute("field-name");
		...
		if (entityConditionList != null && entityConditionList.size() > 0) {
			...
			for (int i = 0; i < econdNodes.size(); i++) {
				...
				if (entityAlias.equals(econd.attribute("entity-alias")))
					joinFields.add(econd.attribute("field-name"));
				if (entityAlias.equals(econd.attribute("to-entity-alias")))
					joinFields.add(econd.attribute("to-field-name"));
			}
		}
		...
		for (int i = 0; i < fieldInfoArray.length; i++) {
			...
			if (!aliasFi.entityAliasUsedSet.contains(entityAlias))
				continue;

			if (localEntityDefinition.isViewEntity) {
				// get the outer alias node
				String outerAliasField = aliasFi.aliasFieldName;
				// get the local entity (sub-select) field node (may be alias node if sub-select
				// on view-entity)
				...
				if (complexAliasNode != null) {
					...
					if (!foundOtherEntityAlias) {
						if (localFi == null)
							throw new EntityException("Could not find field " + outerAliasField + " on entity "
									+ entityAlias + ":" + localEntityDefinition.fullEntityName);
						...
						localBuilder.append(colName).append(" AS ")
								.append(EntityJavaUtil.camelCaseToUnderscored(localFi.name));
						...
						// } else {
						// if we found another entity alias not all on this sub-select entity (or
						// view-entity)
						// TODO only select part that is - IFF not already selected to make sure is
						// selected for outer select
					}
				} else {
					if (localFi == null)
						throw new EntityException("Could not find field " + outerAliasField + " on entity "
					...
					localBuilder.append(localFi.getFullColumnName()).append(" AS ")
							.append(EntityJavaUtil.camelCaseToUnderscored(localFi.name));
					if (localFi.hasAggregateFunction) {
						hasAggregateFunction = true;
					} else {
				...
				// NOTE: this doesn't support various things that
				// EntityDefinition.makeFullColumnName() does like case/when, complex-alias, etc
				// those are difficult to pick out in nested XML elements where the 'alias'
				// element has no entity-alias, and may not be needed at this level (try to
				// handle at top level)
				...
				if (complexAliasNode != null) {
					...
					localBuilder.append(colName).append(" AS ")
							.append(EntityJavaUtil.camelCaseToUnderscored(aliasName));
					if (!isAggFunction) {
						if (gbClause.length() > 0)
							gbClause.append(", ");
					...
					String colName = EntityDefinition.getFunctionPrefix(function)
							+ localEntityDefinition.getColumnName(aliasField) + ")";
					localBuilder.append(colName).append(" AS ")
							.append(EntityJavaUtil.camelCaseToUnderscored(aliasName));
					if (!isAggFunction) {
						if (gbClause.length() > 0)
		...
		if (!isLateralStyle) {
			for (String joinField : joinFields) {
				...
				if (localEntityDefinition.isViewEntity)
					additionalFieldsUsed.add(joinField);
			}
		}

		// where condition to use for FROM clause (field filtering) and for sub-select
		// WHERE clause
		EntityConditionImplBase condition = whereCondition != null
				? whereCondition.filter(entityAlias, mainEntityDefinition)
				: null;
		...
		// logger.warn("Calling makeSqlFromClause for " + entityAlias + ":" +
		// localEntityDefinition.getEntityName() + " condition " + condition);
		// logger.warn("Calling makeSqlFromClause for " + entityAlias + ":" +
		// localEntityDefinition.getEntityName() + " addtl fields " +
		// additionalFieldsUsed);
		...
		if (condition != null || isLateralStyle)
			localBuilder.append(" WHERE ");
		// TODO: for isApplyStyle also do this
		if (isLateralStyle) {
			// TODO how to get this... is per field on inner/local view entity? probably
			// should be otherwise all fields that join to outer view must be on first
			// member-entity
			String joinToIeThisAlias = localEntityDefinition.isViewEntity ? null : localEntityDefinition.getTableName();
			appendJoinConditions(memberEntity, joinToIeThisAlias, localEntityDefinition, linkEntityDefinition,
					localEntityDefinition, localBuilder);
			if (condition != null)
				localBuilder.append(" AND ");
		}
		if (condition != null) {
	...
	public void makeGroupByClause() {
		...
		if (entityInfo.hasFunctionAlias) {
			// do a different approach to GROUP BY: add all fields that are selected and
			// don't have a function or that are in a sub-select
			for (int j = 0; j < fieldInfoArray.length; j++) {
				...
				if (fi.hasAggregateFunction && fi.memberEntityNode != null) {
					...
					if ("true".equals(subSelectAttr) || "non-lateral".equals(subSelectAttr)) {
						// TODO we have a sub-select, if it is on a non-view entity we want to group by
						// (on a view-entity would be only if no aggregate in wrapping alias)
						EntityDefinition fromEntityDefinition = efi
								.getEntityDefinition(fi.memberEntityNode.attribute("entity-name"));
						if (!fromEntityDefinition.isViewEntity)
							doGroupBy = true;
					}
				}
				if (doGroupBy) {
	...
	public void makeOrderByClause(ArrayList<String> orderByFieldList, boolean hasLimitOffset) {
		...
		for (int i = 0; i < obflSize; i++) {
			...
			// Parse the fieldName (can have other stuff in it, need to tear down to just
			// the field name)
			...
			if (fieldInfo == null)
				throw new EntityException("Making ORDER BY clause, could not find field " + fieldName + " in entity "
						+ getMainEd().fullEntityName);
			int typeValue = fieldInfo.typeValue;

			// now that it's all torn down, build it back up using the column name
			if (foo.getCaseUpperLower() != null && typeValue == 1)
			...
			if (!"true".equals(databaseNode.attribute("never-nulls"))) {
				if (foo.getNullsFirstLast() != null)
					sqlTopLevel.append(foo.getNullsFirstLast() ? " NULLS FIRST" : " NULLS LAST");
				else
					sqlTopLevel.append(" NULLS LAST");
			}
		}
	}
	public void addLimitOffset(Integer limit, Integer offset) {
		...
		if (databaseNode != null) {
			...
			if ("limit".equals(offsetStyle)) {
				...
				if (limit != null)
					sqlTopLevel.append(" FETCH FIRST ").append(limit).append(" ROWS ONLY");
			}
	...
	public PreparedStatement makePreparedStatement() {
		...
		// if (this.mainEntityDefinition.getEntityName().contains("FooBar"))
		// logger.warn("========= making find PreparedStatement for SQL: " + finalSql +
		// "; parameters: " + parameters);
		if (isDebugEnabled)
			logger.debug("making find PreparedStatement for SQL: " + finalSql);
		try {
			ps = connection.prepareStatement(finalSql, entityFindBase.getResultSetType(),
					entityFindBase.getResultSetConcurrency());
			Integer maxRows = entityFindBase.getMaxRows();
			Integer fetchSize = entityFindBase.getFetchSize();
			if (maxRows != null && maxRows > 0)
				ps.setMaxRows(maxRows);
			// NOTE: always set a fetch size, without explicit fetch size some JDBC drivers
			// (like MySQL Connector/J) will try to fetch all rows
			// NOTE: the default here of 1000 is a balance between memory use and network
			// overhead, 100 rows generally being easy to accommodate

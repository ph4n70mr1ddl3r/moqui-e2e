...
public class FieldValueCondition implements EntityConditionImplBase, Externalizable {
	...
	public FieldValueCondition(ConditionField field, ComparisonOperator operator, Object value) {
		...
		// if EQUALS and we have a Collection value the IN operator is implied, similar
		// with NOT_EQUAL
		if (value instanceof Collection) {
			if (tempOp == EQUALS)
				tempOp = IN;
			else if (tempOp == NOT_EQUAL)
				tempOp = NOT_IN;
		}
	...
	public void makeSqlWhere(EntityQueryBuilder eqb, EntityDefinition subMemberEd) {
		...
		if (fi == null)
			throw new EntityException(
					"Could not find field " + field.fieldName + " in entity " + curEd.getFullEntityName());
		...
		if (value instanceof Collection && ((Collection) value).isEmpty()) {
			...
			if (value == null) {
				if (operator == EQUALS || operator == LIKE || operator == IN || operator == BETWEEN) {
					sql.append(" IS NULL");
					valueDone = true;
				} else if (operator == NOT_EQUAL || operator == NOT_LIKE || operator == NOT_IN
		...
		if (!valueDone) {
			...
			if (operator == IN || operator == NOT_IN) {
				...
				if (ignoreCase && (value1 instanceof CharSequence))
					value1 = value1.toString().toUpperCase();
				Object value2 = iterator.next();
				if (ignoreCase && (value2 instanceof CharSequence))
	...
	public void makeSearchFilter(List<Map<String, Object>> filterList) {
		...
		switch (operator) {
			...
			case EQUALS :
				...
				if (isNot) {
					filterList.add(
							CollectionUtilities.toHashMap("bool", CollectionUtilities.toHashMap("must_not", termMap)));
				} else {
			...
			case IN :
				...
				if (isNot) {
					filterList.add(
							CollectionUtilities.toHashMap("bool", CollectionUtilities.toHashMap("must_not", termsMap)));
				} else {
			...
			case LIKE :
				...
				Map<String, Object> wildcardMap = CollectionUtilities.toHashMap("wildcard",
						CollectionUtilities.toHashMap(field.fieldName, CollectionUtilities.toHashMap("value", value)));
				if (isNot) {
			...
			case BETWEEN :
				...
				if (value instanceof Collection && ((Collection) value).size() == 2) {
					...
					Map<String, Object> rangeMap = CollectionUtilities.toHashMap("range", CollectionUtilities
							.toHashMap(field.fieldName, CollectionUtilities.toHashMap("gte", value1, "lte", value2)));
					if (isNot) {
						filterList.add(CollectionUtilities.toHashMap("bool",
			...
			case IS_NULL :
				filterList.add(CollectionUtilities.toHashMap("bool",
						CollectionUtilities.toHashMap("must_not", CollectionUtilities.toHashMap("exists",
			...
			case GREATER_THAN_EQUAL_TO :
				filterList.add(CollectionUtilities.toHashMap("range", CollectionUtilities.toHashMap(field.fieldName,
	...
	public boolean populateMap(Map<String, Object> map) {
		if (operator != EQUALS || ignoreCase || field instanceof ConditionAlias)
			return false;
	...
	public void getAllAliases(Set<String> entityAliasSet, Set<String> fieldAliasSet) {
		// this will only be called for view-entity, so we'll either have a entityAlias
		// or an aliased fieldName
		...
		if (entityAlias == null) {
			...
			if (fieldMe != null && entityAlias.equals(fieldMe.attribute("entity-alias"))) {
				if (fi.aliasFieldName != null && !fi.aliasFieldName.equals(field.fieldName)) {
					FieldValueCondition newCond = new FieldValueCondition(new ConditionField(fi.aliasFieldName),
							operator, value);
					if (ignoreCase)
						newCond.ignoreCase();
					return newCond;
	...
	private int createHashCode() {
		return (field != null ? field.hashCode() : 0) + operator.hashCode() + (value != null ? value.hashCode() : 0)
				+ (ignoreCase ? 1 : 0);
	}

	@Override
	public boolean equals(Object o) {
		if (o == null || o.getClass() != thisClass)
			return false;
	...
	public void writeExternal(ObjectOutput out) throws IOException {
		...
		// NOTE: found that the serializer in Hazelcast is REALLY slow with writeUTF(),
		// uses String.chatAt() in a for loop, crazy

...
public class AggregationUtil {
	...
	public AggregationUtil(String listName, String listEntryName, AggregateField[] aggregateFields,
			String[] groupFields, XmlAction rowActions) {
		this.listName = listName;
		this.listEntryName = listEntryName;
		if (this.listEntryName != null && this.listEntryName.isEmpty())
			this.listEntryName = null;
		...
		for (int i = 0; i < aggregateFields.length; i++) {
			...
			if (aggField.fromExpr != null)
				hasFromExpr = true;
			if (aggField.subList && aggField.showTotal != null)
				hasSubListTotals = true;
		}
	}

	@SuppressWarnings("unchecked")
	public ArrayList<Map<String, Object>> aggregateList(Object listObj, Set<String> includeFields, boolean makeSubList,
		...
		// for (Object result : (Iterable) listObj) logger.warn("Aggregate Input: " +
		// result.toString());
		...
		if (listObj instanceof List) {
			...
			if (listObj instanceof RandomAccess) {
				for (int i = 0; i < listSize; i++) {
					...
					processAggregateOriginal(curObject, resultList, includeFields, groupRows, totalsMap, i,
							(i < (listSize - 1)), makeSubList, eci);
					originalCount++;
				}
			} else {
				int i = 0;
			...
			while (listIter.hasNext()) {
				...
				processAggregateOriginal(curObject, resultList, includeFields, groupRows, totalsMap, i,
						listIter.hasNext(), makeSubList, eci);
				i++;
				originalCount++;
			}
			...
			throw new BaseArtifactException("form-list list " + listName + " is a type we don't know how to iterate: "
		...
		if (hasSubListTotals) {
			...
			for (int i = 0; i < resultSize; i++) {
				...
				if (aggregateSubList != null) {
					...
					if (aggregateSubListTotals != null)
						aggregateSubList.add(aggregateSubListTotals);
				}
			}
		}
		if (totalsMap.size() > 0)
			resultList.add(new HashMap<>(totalsMap));
		...
		// for (Map<String, Object> result : resultList) logger.warn("Aggregate Result:
		// " + result.toString());
	...
	private void processAggregateOriginal(Object curObject, ArrayList<Map<String, Object>> resultList,
			Set<String> includeFields, Map<Map<String, Object>, Map<String, Object>> groupRows,
			Map<String, Object> totalsMap, int index, boolean hasNext, boolean makeSubList, ExecutionContextImpl eci) {
		Map curMap = null;
		...
		if (makeSubList) {
			...
			for (int i = 0; i < groupFields.length; i++) {
				...
				if (!includeFields.contains(groupBy))
					continue;
		...
		if (resultMap == null) {
			...
			for (int i = 0; i < aggregateFields.length; i++) {
				...
				// don't want to put null values, a waste of time/space; if count aggregate
				// continue so it isn't counted
				if (fieldValue == null)
					continue;

				// handle subList
				if (makeSubList && aggField.subList) {
					// NOTE: may have an issue here not using contextTopMap as starting point for
					// sub-list entry, ie row-actions values lost if not referenced in a field
					// name/from
					// NOTE2: if we start with contextTopMap should clone and perhaps remove
					// aggregateFields that are not sub-list
					if (subListMap == null)
			...
			if (subListMap != null) {
				ArrayList<Map<String, Object>> subList = (ArrayList<Map<String, Object>>) resultMap
						.get("aggregateSubList");
				if (subList != null)
					subList.add(subListMap);
			}
		}
	...
	private Object getField(String fieldName, ContextStack context, Object curObject, boolean curIsMap) {
		...
		if (curObject != null && !curIsMap && ObjectUtilities.isEmpty(value)) {
			...
			try {
				...
				if (isTraceEnabled)
					logger.trace("Field " + fieldName + " is not a property of list-entry " + listEntryName
							+ " in list " + listName + ": " + e.toString());
			}
		}
		return value;
	}
	...
	@SuppressWarnings("unchecked")
	private void doFunction(AggregateFunction function, Map<String, Object> resultMap, String fieldName,
			Object fieldValue) {
		switch (function) {
			case MIN :
			case MAX :
				...
				if (existingComp == null) {
					...
					if ((function == AggregateFunction.MIN && compResult > 0)
							|| (function == AggregateFunction.MAX && compResult < 0))
						resultMap.put(fieldName, newComp);
				}
				break;
			case SUM :
				if (fieldValue != null) {
					...
					if (sumNum != null)
						resultMap.put(fieldName, sumNum);
				}
				break;
			case AVG :
				Number newNum = (Number) fieldValue;
				if (newNum != null) {
					BigDecimal newNumBd = (newNum instanceof BigDecimal)
							? (BigDecimal) newNum
							: new BigDecimal(newNum.toString());

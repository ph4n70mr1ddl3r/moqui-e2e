...
public class EntityListIteratorImpl implements EntityListIterator {
	...
	public EntityListIteratorImpl(Connection con, ResultSet rs, EntityDefinition entityDefinition,
			FieldInfo[] fieldInfoArray, EntityFacadeImpl efi, TransactionCache txCache,
			EntityConditionImplBase queryCondition, ArrayList<String> obf) {
		this.efi = efi;
		this.con = con;
		this.rs = rs;
	...
	public void close() {
		if (this.closed) {
			...
			/*
			 * leaving commented as might be useful for future con pool debugging: try { def
			 * dataSource =
			 * efi.getDatasourceFactory(entityDefinition.getEntityGroupName()).getDataSource
			 * () logger.
			 * warn("=========== elii after close pool available size: ${dataSource.poolAvailableSize()}/${dataSource.poolTotalSize()}; ${dataSource.getMinPoolSize()}-${dataSource.getMaxPoolSize()}"
			 * ) } catch (Throwable t) {
	...
	public int currentIndex() {
		try {
			return rs.getRow() + txcListIndex + 1;
		} catch (SQLException e) {
	...
	public boolean relative(final int rows) {
		...
		if (txcListSize > 0)
			throw new EntityException(
					"Cannot go to relative row number when transaction cache is in place and there are augmenting creates; disable the tx cache before this operation");
		try {

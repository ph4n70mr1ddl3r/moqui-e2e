...
public class EntityFindImpl extends EntityFindBase {
	...
	public EntityValueBase oneExtended(EntityConditionImplBase whereCondition, FieldInfo[] fieldInfoArray,
		...
		// NOTE 20200707 don't do this, databases such as Oracle (error ORA-02014) do
		// not allow use of limit/offset with for update: LIMIT/OFFSET clause - for find
		// one always limit to 1: efb.addLimitOffset(1, 0);
		// FOR UPDATE
		if (getForUpdate())
			efb.makeForUpdate();
		...
		try {
			...
			if (rs.next()) {
				...
				if (isTraceEnabled)
					logger.trace(
							"Result set was empty for find on entity " + entityName + " with condition " + condSql);
			}

			if (isTraceEnabled && rs.next())
				logger.trace("Found more than one result for condition " + condSql + " on entity " + entityName);
			queryTextList.add(efb.finalSql);
		} finally {
			try {
				efb.closeAll();
			} catch (SQLException sqle) {
				logger.error("Error closing query", sqle);
			}
		}
	...
	public EntityListIterator iteratorExtended(EntityConditionImplBase whereCondition,
		...
		try {
			...
			ResultSet rs = efb.executeQuery();
			elii = new EntityListIteratorImpl(con, rs, ed, fieldInfoArray, efi, txCache, whereCondition,
					orderByExpanded);
		...
		// no finally block to close ResultSet, etc because contained in
		// EntityListIterator and closed with it
	...
	public long countExtended(EntityConditionImplBase whereCondition, EntityConditionImplBase havingCondition,
		...
		MNode condNode = entityConditionList != null && entityConditionList.size() > 0
				? entityConditionList.get(0)
				: null;
		boolean isDistinct = getDistinct()
				|| (ed.isViewEntity && condNode != null && "true".equals(condNode.attribute("distinct")));

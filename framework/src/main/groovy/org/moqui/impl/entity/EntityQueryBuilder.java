...
public class EntityQueryBuilder implements Runnable {
	...
	public EntityQueryBuilder(EntityDefinition entityDefinition, EntityFacadeImpl efi) {
		...
		if (execWithTimeout)
			this.execTimeout = efi.ecfi.transactionFacade.getTxTimeoutRemainingMillis();
	}

	public EntityDefinition getMainEd() {
	...
	public PreparedStatement makePreparedStatement() {
		...
		// if (this.mainEntityDefinition.getFullEntityName().contains("foo"))
		// logger.warn("========= making crud PreparedStatement for SQL: ${sql}")
		if (isDebugEnabled)
			logger.debug("making crud PreparedStatement for SQL: " + finalSql);
		try {
	...
	public void run() {
		...
		try {
			...
			if (execQuery) {
				rs = ps.executeQuery();
				if (isDebugEnabled)
					logger.debug("Executed query with SQL [" + finalSql + "] and parameters [" + parameters + "] in ["
							+ ((System.currentTimeMillis() - timeBefore) / 1000) + "] seconds");
			} else {
				rowsUpdated = ps.executeUpdate();
				if (isDebugEnabled)
					logger.debug("Executed update with SQL [" + finalSql + "] and parameters [" + parameters + "] in ["
							+ ((System.currentTimeMillis() - timeBefore) / 1000) + "] seconds changing [" + rowsUpdated
							+ "] rows");
			}
		} catch (Throwable t) {
			uncaughtThrowable = t;
	...
	ResultSet executeQuery() throws SQLException {
		if (ps == null)
			throw new IllegalStateException("Cannot Execute Query, no PreparedStatement in place");
		...
		if (execWithTimeout) {
			try {
				...
				// if (execTimeout != 60000L) logger.info("statement with timeout " +
				// execTimeout);
		...
		if (rs == null && uncaughtThrowable == null)
			uncaughtThrowable = new SQLException(
					"JDBC query timed out after " + execTimeout + "ms for SQL " + finalSql);
		try {
			...
			if (queryStats)
				efi.saveQueryStats(mainEntityDefinition, finalSql, System.nanoTime() - beforeQuery, isError);
		}
	...
	int executeUpdate() throws SQLException {
		if (ps == null)
			throw new IllegalStateException("Cannot Execute Update, no PreparedStatement in place");
		...
		if (rowsUpdated == -1 && uncaughtThrowable == null)
			uncaughtThrowable = new SQLException(
					"JDBC update timed out after " + execTimeout + "ms for SQL " + finalSql);
		// try {
		if (uncaughtThrowable != null) {
			// isError = true;
			if (uncaughtThrowable instanceof SQLException) {
		...
		// } finally {
		// if (queryStats) efi.saveQueryStats(mainEntityDefinition, finalSql,
		// System.nanoTime() - beforeQuery, isError);
		// }
	...
	/**
	 * NOTE: this should be called in a finally clause to make sure things are
	 * closed
	 */
	...
	public static String sanitizeColumnName(String colName) {
		...
		while (interim.charAt(0) == '_')
			interim.deleteCharAt(0);
		while (interim.charAt(interim.length() - 1) == '_')
			interim.deleteCharAt(interim.length() - 1);
		int duIdx;
		while ((duIdx = interim.indexOf("__")) >= 0)
			interim.deleteCharAt(duIdx);
	...
	public void makeSqlSelectFields(FieldInfo[] fieldInfoArray, FieldOrderOptions[] fieldOptionsArray,
		...
		if (size > 0) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (appendCloseParen)
					sqlTopLevel.append(")");
				// H2 (and perhaps other DBs?) require a unique name for each selected column,
				// even if not used elsewhere; seems like a bug...
	...
	public void addWhereClause(FieldInfo[] pkFieldArray, LiteStringMap valueMapInternal) {
		...
		for (int i = 0; i < sizePk; i++) {
			...
			if (fieldInfo == null)
				break;
			if (i > 0)
				sqlTopLevel.append(" AND ");
			sqlTopLevel.append(fieldInfo.getFullColumnName()).append("=?");
			parameters.add(new EntityJavaUtil.EntityConditionParameter(fieldInfo,
					valueMapInternal.getByIString(fieldInfo.name, fieldInfo.index), this));
		}
	}
}

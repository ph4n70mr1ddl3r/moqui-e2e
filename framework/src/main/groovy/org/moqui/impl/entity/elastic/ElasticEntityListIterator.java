...
public class ElasticEntityListIterator implements EntityListIterator {
	...
	public ElasticEntityListIterator(Map<String, Object> searchMap, EntityDefinition entityDefinition,
		...
		if (txCache != null && whereCondition != null) {
			orderByComparator = obf != null && obf.size() > 0
					? new CollectionUtilities.MapOrderByComparator(obf)
					: null;
			// add all created values (updated and deleted values will be handled by the
			// next() method
			...
			if (findAugmentInfo.valueListSize > 0) {
				...
				if (orderByComparator != null)
					findAugmentInfo.valueList.sort(orderByComparator);
		...
		// capture the current artifact stack for finalize not closed debugging, has
		// minimal performance impact (still ~0.0038ms per call compared to numbers
		// below)
		artifactStack = efi.ecfi.getEci().artifactExecutionFacade.getStackArray();

		/*
		 * uncomment only if needed temporarily: huge performance impact, ~0.036ms per
		 * call with, ~0.0037ms without (~10x difference!) StackTraceElement[] tempStack
		 * = Thread.currentThread().getStackTrace(); if (tempStack.length > 20)
		 * tempStack = java.util.Arrays.copyOfRange(tempStack, 0, 20); constructStack =
		 * tempStack;
		 */
	}
	...
	boolean isLast() {
		if (resultCount != null) {
			return overallIndex == resultCount - 1;
		} else {
			return false;
		}
	}
	boolean isAfterLast() {
		if (resultCount != null) {
			return overallIndex >= resultCount;
		} else {
			return false;
		}
	}
	...
	boolean nextResult() {
		if (resultCount != null && overallIndex >= resultCount)
			return false;
		overallIndex++;
		...
		// logger.warn("nextResult end resultCount " + resultCount + " overallIndex " +
		// overallIndex + " currentListStartIndex " + currentListStartIndex + "
		// currentDocList.size() " + currentDocList.size());
		return hasCurrentValue();
	}
	@SuppressWarnings("unchecked")
	void fetchNext() {
		if (this.closed)
			throw new IllegalStateException("EntityListIterator is closed, cannot fetch next results");
		...
		if (resultCount != null && curFrom + curSize > resultCount) {
			...
			if (curFrom >= resultCount)
				return;
			curSize = resultCount - curFrom;
		}
		if (maxResultCount != null && curFrom + curSize > maxResultCount) {
			// no more to get, return
			if (curFrom >= maxResultCount)
				return;
			curSize = maxResultCount - curFrom;
		}

		// before doing the search, see if we need a PIT ID: if we can't get all in one
		// fetch
		...
		if (esPitId != null)
			searchMap.put("pit", CollectionUtilities.toHashMap("id", esPitId, "keep_alive", esKeepAlive));
		if (esSearchAfter != null) {
			...
			// if origFromInt has a value always add it just before the query, basically a
			// starting offset for the whole query
			searchMap.put("from", originalFromInt != null ? curFrom + originalFromInt : curFrom);
		}
		searchMap.put("size", curSize);

		// if no resultCount yet then track_total_hits (also set to false for better
		// performance on subsequent requests)
		searchMap.put("track_total_hits", resultCount == null);

		// logger.info("fetchNext request: " +
		// JsonOutput.prettyPrint(JsonOutput.toJson(searchMap)));
		...
		/*
		 * Map resultNoHits = new LinkedHashMap(resultMap); Map hitsNoHits = new
		 * LinkedHashMap(hitsMap); hitsNoHits.remove("hits"); resultNoHits.put("hits",
		 * hitsNoHits); logger.info("fetchNext response: " +
		 * JsonOutput.prettyPrint(JsonOutput.toJson(resultNoHits)));
		 */
		...
		if (totalMap != null) {
			...
			if (!"eq".equals(relation))
				logger.warn("Got non eq total relation " + relation + " with value " + hitsTotal + " for entity "
						+ entityDefinition.fullEntityName);
		...
		if (hitsList != null && hitsList.size() > 0) {
			...
			if (hitCount > fetchSize)
				logger.warn("In ElasticEntityListIterator got back " + hitCount + " hits with fetchSize " + fetchSize);

			if (hitCount < curSize) {
				// we found the end
				int calcTotal = curFrom + hitCount;
				if (resultCount != calcTotal)
					logger.warn("In ElasticEntityListIterator reached end of results at " + calcTotal
							+ " but server claimed " + resultCount + " total hits");
			...
			if (currentListStartIndex == -1) {
				...
				if (avail < hitCount) {
					...
					if (retain == 0) {
						...
						// this is two array copies instead of potential one, but better than iterating
						// manually to move elements or something
			...
			// does the Jackson parser (used in ElasticFacade) use an ArrayList? probably
			// not... Iterator overhead not too bad here anyway
			...
			while (hitsIterator.hasNext()) {
				...
				if (!hitsIterator.hasNext()) {
					...
					if (hitSort != null)
						esSearchAfter = hitSort;
				}
			}
		}

		// logger.warn("fetchNext resultCount " + resultCount + " currentListStartIndex
		// " + currentListStartIndex + " currentDocList size " + currentDocList.size());
	}
	...
	void fetchPrevious() {
		if (this.closed)
			throw new IllegalStateException("EntityListIterator is closed, cannot fetch previous results");
	...
	boolean hasCurrentValue() {
		// if the numbers are such that we have a result (after a fetchPrevious() if
		// needed) then return true
		return overallIndex >= currentListStartIndex && overallIndex < (currentListStartIndex + currentDocList.size());
	}
	void resetCurrentList() {
		// TODO: given multi-fetch space in the current list this could be optimized to
		// avoid future fetch if currentListStartIndex < CUR_LIST_MAX_SIZE
	...
	public void close() {
		if (this.closed) {
			logger.warn("EntityListIterator for entity " + this.entityDefinition.getFullEntityName()
					+ " is already closed, not closing again");
		} else {
			if (esPitId != null) {
				ElasticFacade.ElasticClient elasticClient = edf.getElasticClient();
	...
	public boolean last() {
		...
		/*
		 * if (txcListSize > 0) { try { rs.afterLast(); } catch (SQLException e) { throw
		 * new EntityException("Error moving EntityListIterator to last", e); }
		 * txcListIndex = txcListSize - 1; return true; } else { try { return rs.last();
		 * } catch (SQLException e) { throw new
		 * EntityException("Error moving EntityListIterator to last", e); } }
		 */
	}
	@Override
	...
	public EntityValueBase currentEntityValueBase() {
		...
		// if txCache in place always put in cache for future reference (onePut handles
		// any stale from DB issues too)
		// NOTE: because of this don't use txCache for very large result sets
		if (txCache != null)
			txCache.onePut(newEntityValue, false);
		haveMadeValue = true;
	...
	public int currentIndex() {
		// NOTE: add one because this is based on the JDBC ResultSet object which is 1
		// based
		return overallIndex + txcListIndex + 1;
	}
	@Override
	public boolean absolute(final int rowNum) {
		// TODO: somehow implement this for txcList? would need to know how many rows
		// after last we tried to go
		if (txcListSize > 0)
			throw new EntityException(
					"Cannot go to absolute row number when transaction cache is in place and there are augmenting creates; disable the tx cache before this operation");
	...
	public boolean relative(final int rows) {
		...
		// TODO: somehow implement this for txcList? would need to know how many rows
		// after last we tried to go
		// if (txcListSize > 0) throw new EntityException("Cannot go to relative row
		// number when transaction cache is in place and there are augmenting creates;
		// disable the tx cache before this operation");
	...
	public boolean hasNext() {
		if (isLast() || isAfterLast()) {
			...
			// if not in the first or beforeFirst positions and haven't made any values yet,
	...
	public boolean hasPrevious() {
		if (isFirst() || isBeforeFirst()) {
			...
			// if not in the last or afterLast positions and we haven't made any values yet,
			// the result set is empty
	...
	public EntityValue next() {
		...
		if (nextResult()) {
			...
			if (txcListSize > 0) {
				// txcListIndex should be -1, but instead of incrementing set to 0 just to make
				// sure
				txcListIndex = 0;
				return currentEntityValue();
			} else {
	...
	public EntityList getCompleteList(boolean closeAfter) {
		try {
			...
			if (haveMadeValue && !isBeforeFirst())
				beforeFirst();
	...
	public EntityList getPartialList(int offset, int limit, boolean closeAfter) {
		// TODO: somehow handle txcList after DB list? same issue as absolute() and
		// relative() methods
		if (txcListSize > 0)
			throw new EntityException(
					"Cannot get partial list when transaction cache is in place and there are augmenting creates; disable the tx cache before this operation");
		try {
	...
	public void remove() {
		...
		// TODO: call EECAs
		// efi.getEntityCache().clearCacheForValue((EntityValueBase)
		// currentEntityValue(), false);
		// rs.deleteRow();
	}

...
public class BasicJoinCondition implements EntityConditionImplBase {
	...
	public void makeSearchFilter(List<Map<String, Object>> filterList) {
		...
		if (operator == AND)
			boolMap.put("filter", childList);
		else
			boolMap.put("should", childList);
	...
	public boolean mapMatches(Map<String, Object> map) {
		...
		if (lhsMatches && operator == OR)
			return true;
		if (!lhsMatches && operator == AND)
			return false;

		// handle opposite cases since we know cases above aren't true (ie if OR then
		// lhs=false, if AND then lhs=true
	...
	public String toString() {
		...
		return "(" + lhsInternal.toString() + ") " + EntityConditionFactoryImpl.getJoinOperatorString(this.operator)
				+ " (" + rhsInternal.toString() + ")";
	}
	...
	private int createHashCode() {
		return (lhsInternal != null ? lhsInternal.hashCode() : 0) + operator.hashCode()
				+ (rhsInternal != null ? rhsInternal.hashCode() : 0);
	}
	...
	public boolean equals(Object o) {
		...
		if (!this.lhsInternal.equals(that.lhsInternal))
			return false;

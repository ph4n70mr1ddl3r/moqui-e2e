...
class EntityListIteratorWrapper implements EntityListIterator {
	...
	EntityListIteratorWrapper(List<EntityValue> valList, EntityDefinition entityDefinition, EntityFacadeImpl efi,
		...
		if (txCache != null && queryCondition != null) {
			...
			// next() method
			EntityJavaUtil.FindAugmentInfo tempFai = txCache.getFindAugmentInfo(entityDefinition.getFullEntityName(),
					queryCondition);
			if (tempFai.valueListSize > 0) {
				...
				if (obf != null && obf.size() > 0)
					valueList.sort(new CollectionUtilities.MapOrderByComparator(obf));
			}
		}
	}
	...
	public void close() {
		if (this.closed) {
			logger.warn("EntityListIterator for entity " + entityDefinition.fullEntityName
	...
	public void afterLast() {
		this.internalIndex = valueList.size();
	}
	@Override
	...
	public boolean hasNext() {
		return internalIndex < (valueList.size() - 1);
	}
	@Override
	...
	public EntityValue next() {
		if (internalIndex >= valueList.size())
			return null;
		internalIndex++;
		if (internalIndex >= valueList.size())
			return null;
	...
	public int writeXmlText(Writer writer, String prefix, int dependentLevels) {
		...
		if (haveMadeValue && internalIndex != -1)
			internalIndex = -1;
		EntityValue value;
		while ((value = this.next()) != null)

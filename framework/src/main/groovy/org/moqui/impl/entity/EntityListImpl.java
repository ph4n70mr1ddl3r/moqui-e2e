...
public class EntityListImpl implements EntityList {
	...
	public void writeExternal(ObjectOutput out) throws IOException {
		...
		// don't serialize fromCache, will default back to false which is fine for a
		// copy
	}
	...
	public EntityList filterByDate(String fromDateName, String thruDateName, Timestamp moment) {
		if (fromCache)
			return this.cloneList().filterByDate(fromDateName, thruDateName, moment);

		// default to now
		long momentLong = moment != null ? moment.getTime() : System.currentTimeMillis();
		long momentDateLong = new Date(momentLong).getTime();
		if (fromDateName == null || fromDateName.length() == 0)
			fromDateName = "fromDate";
		if (thruDateName == null || thruDateName.length() == 0)
	...
	public EntityList filterByAnd(Map<String, Object> fields, Boolean include) {
		if (fromCache)
			return this.cloneList().filterByAnd(fields, include);
		...
		for (Map.Entry<String, Object> entry : fields.entrySet()) {
			...
			if (val instanceof Collection) {
				...
				if (!(val instanceof Set))
					values[fieldIndex] = new HashSet<Object>((Collection<? extends Object>) val);
			}
			fieldIndex++;
		}
	...
	@Override
	public EntityList filterByAnd(Object... namesAndValues) {
		if (namesAndValues.length == 0)
			return this;
		if (namesAndValues.length % 2 != 0)
			throw new IllegalArgumentException("Must pass an even number of parameters for name/value pairs");

		if (fromCache)
			return this.cloneList().filterByAnd(namesAndValues);
		...
		for (int i = 0; i < fieldsSize; i++) {
			...
			if (value instanceof Collection) {
				...
				if (!(value instanceof Set))
					value = new HashSet<Object>((Collection<? extends Object>) value);
			}
			values[i] = value;
		}
	...
	public EntityList filter(Closure<Boolean> closure, Boolean include) {
		if (fromCache)
			return this.cloneList().filter(closure, include);
		int valueIndex = 0;
		while (valueIndex < valueList.size()) {
			EntityValue value = valueList.get(valueIndex);
	...
	@Override
	public EntityValue findByAnd(Map<String, Object> fields) {
		// iterate fields once, then use indexes within big loop
		int fieldsSize = fields.size();
		String[] names = new String[fieldsSize];
		Object[] values = new Object[fieldsSize];
	...
	@Override
	public EntityValue findByAnd(Object... namesAndValues) {
		if (namesAndValues.length == 0)
			return getFirst();
		if (namesAndValues.length % 2 != 0)
			throw new IllegalArgumentException("Must pass an even number of parameters for name/value pairs");
	...
	public EntityList filterByCondition(EntityCondition condition, Boolean include) {
		if (fromCache)
			return this.cloneList().filterByCondition(condition, include);
		if (include == null)
			include = true;
		int valueIndex = 0;
		while (valueIndex < valueList.size()) {
			...
			// logger.warn("TOREMOVE filter value [${value}] with condition [${condition}]
			// include=${include}, matches=${matches}")
	...
	public EntityList filterByLimit(Integer offset, Integer limit) {
		if (fromCache)
			return this.cloneList().filterByLimit(offset, limit);
	...
	public EntityList filterByLimit(String inputFieldsMapName, boolean alwaysPaginate) {
		if (fromCache)
			return this.cloneList().filterByLimit(inputFieldsMapName, alwaysPaginate);
		...
		if (alwaysPaginate || inf.get("pageIndex") != null) {
			...
			int pageIndex;
			if (pageIndexObj instanceof Number) {
				pageIndex = ((Number) pageIndexObj).intValue();
			} else {
	...
	public int getPageIndex() {
		return (offset != null ? offset : 0) / getPageSize();
	}
	@Override
	...
	public EntityList orderByFields(List<String> fieldNames) {
		if (fromCache)
			return this.cloneList().orderByFields(fieldNames);
		if (fieldNames != null && fieldNames.size() > 0)
			valueList.sort(new CollectionUtilities.MapOrderByComparator(fieldNames));
		return this;
	}
	@Override
	public void sort(Comparator<? super EntityValue> comparator) {
	...
	public EntityList addIfMissing(EntityValue value) {
		if (fromCache)
			throw new EntityException("Cannot modify EntityList from cache");
	...
	public int writeXmlText(Writer writer, String prefix, int dependentLevels) {
		...
		for (EntityValue ev : this)
			recordsWritten += ev.writeXmlText(writer, prefix, dependentLevels);
		return recordsWritten;
	}

	@Override
	public @Nonnull Iterator<EntityValue> iterator() {
	...
	private class EntityIterator implements Iterator<EntityValue> {
		...
		public boolean hasNext() {
			return (curIndex + 1) < valueList.size();
		}
		@Override
		public EntityValue next() {
			if ((curIndex + 1) >= valueList.size())
				throw new NoSuchElementException(
						"Next is beyond end of list (index " + (curIndex + 1) + ", size " + valueList.size() + ")");
			curIndex++;
			valueRemoved = false;
		...
		public void remove() {
			if (fromCache)
				throw new UnsupportedOperationException("Cannot modify EntityList from cache");
			if (curIndex == -1)
				throw new IllegalStateException("Cannot remove, next() has not been called");
			if (valueRemoved)
				throw new IllegalStateException("Cannot remove, next() has not been called since last remove");
	...
	public List<Map<String, Object>> getPlainValueList(int dependentLevels) {
		...
		for (EntityValue ev : valueList)
			plainRelList.add(ev.getPlainValueMap(dependentLevels));
		return plainRelList;
	}
	@Override
	public List<Map<String, Object>> getMasterValueList(String name) {
		...
		for (EntityValue ev : valueList)
			masterRelList.add(ev.getMasterValueMap(name));
	...
	public EntityList cloneList() {
		...
		// NOTE: when cloning don't clone the fromCache value (normally when from cache
		// will be cloned before filtering)
		return newObj;
	}
	...
	public void setFromCache() {
		fromCache = true;
		for (EntityValue ev : valueList)
			if (ev instanceof EntityValueBase)
				((EntityValueBase) ev).setFromCache();
	}
	...
	@Override
	public boolean addAll(@Nonnull Collection<? extends EntityValue> es) {
		if (fromCache)
			throw new EntityException("Cannot modify EntityList from cache");
		return valueList.addAll(es);
	}
	@Override
	public boolean addAll(int i, @Nonnull Collection<? extends EntityValue> es) {
		if (fromCache)
			throw new EntityException("Cannot modify EntityList from cache");
	...
	@Override
	public @Nonnull ListIterator<EntityValue> listIterator() {
		return valueList.listIterator();
	}
	@Override
	public @Nonnull ListIterator<EntityValue> listIterator(int i) {
		return valueList.listIterator(i);
	}
	@Override
	public @Nonnull List<EntityValue> subList(int start, int end) {

...
public class ElasticEntityFind extends EntityFindBase {
	...
	List<Object> makeSortList(ArrayList<String> orderByExpanded, EntityDefinition ed) {
		...
		if (orderByExpandedSize > 0) {
			...
			for (int i = 0; i < orderByExpandedSize; i++) {
				...
				// to make this more fun, need to look for fields which have: keyword child
				// field, text with no keyword (can't sort)
	...
	public EntityValueBase oneExtended(EntityConditionImplBase whereCondition, FieldInfo[] fieldInfoArray,
		...
		if (ed.isViewEntity)
			throw new EntityException(
					"Multi-entity view entities are not supported, Elastic/OpenSearch does not support joins; single-entity view entities for aggregations are not yet supported (future feature)");

		edf.checkCreateDocumentIndex(ed);
		ElasticFacade.ElasticClient elasticClient = edf.getElasticClient();

		// TODO FUTURE: consider building a JSON string instead of Map/List structure
		// with lots of objects,
		// will perform better and have way less memory overhead, but code will be a lot
		// more complicated
		...
		if (tempHasFullPk) {
			...
			if (whereCondition != null)
				searchMap.put("query", makeQueryMap(whereCondition));
			// _source or fields
			// TODO: use _source or fields to get partial documents, some possible oddness
			// to it:
	...
	@Override
	public EntityListIterator iteratorExtended(EntityConditionImplBase whereCondition,
		...
		if (havingCondition != null)
			throw new EntityException(
					"Having condition not supported, no view-entity support yet (future feature along with single-entity view entities for aggregations)");
		...
		if (sortList == null) {
			// if no sort, sort by PK fields by default (for pagination over large queries a
			// sort order is always required)
		...
		// from & size
		if (this.offset != null)
			searchMap.put("from", this.offset);
		if (this.limit != null)
			searchMap.put("size", this.limit);
	...
	public long countExtended(EntityConditionImplBase whereCondition, EntityConditionImplBase havingCondition,
			FieldInfo[] fieldInfoArray, EntityJavaUtil.FieldOrderOptions[] fieldOptionsArray) throws EntityException {
		EntityDefinition ed = this.getEntityDef();
		if (ed.isViewEntity)
			throw new EntityException(
					"Multi-entity view entities are not supported, Elastic/OpenSearch does not support joins; single-entity view entities for aggregations are not yet supported (future feature)");
		...
		// query
		if (whereCondition != null)
			countMap.put("query", makeQueryMap(whereCondition));
		// NOTE: if no where condition don't need to add default all query map,
		// ElasticClient.countResponse() does this (used by count())

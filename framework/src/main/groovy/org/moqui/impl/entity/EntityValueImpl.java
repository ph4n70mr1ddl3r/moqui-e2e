...
public class EntityValueImpl extends EntityValueBase {
	...
	public EntityValue cloneValue() {
		...
		if (this.dbValueMap != null)
			newObj.setDbValueMap(this.dbValueMap);
		// don't set immutable (default to mutable even if original was not) or modified
		// (start out not modified)
		return newObj;
	}
	...
	public EntityValue cloneDbValue(boolean getOld) {
		...
		for (FieldInfo fieldInfo : getEntityDefinition().entityInfo.allFieldInfoArray)
			newObj.putKnownField(fieldInfo,
					getOld ? getOldDbValue(fieldInfo.name) : getOriginalDbValue(fieldInfo.name));
	...
	public void createExtended(FieldInfo[] fieldInfoArray, Connection con) throws SQLException {
		...
		try {
			...
			for (int i = 0; i < size; i++) {
				...
				if (fieldInfo == null)
					break;
				eqb.setPreparedStatementValue(i + 1, valueMapInternal.getByIString(fieldInfo.name, fieldInfo.index),
						fieldInfo);
			}

			// if (ed.entityName == "Subscription") logger.warn("Create ${this.toString()}
			// tx ${efi.getEcfi().transaction.getTransactionManager().getTransaction()} con
			// ${eqb.connection}")
			eqb.executeUpdate();
			setSyncedWithDb();
			...
			try {
				...
				if (logger.isTraceEnabled())
					logger.trace("Error getting transaction name: " + txe.toString());
			}
			logger.warn("Error creating " + this.toString() + " tx " + txName + " con " + eqb.connection.toString()
					+ ": " + e.toString());
			throw e;
		} finally {
			try {
	...
	public void updateExtended(FieldInfo[] pkFieldArray, FieldInfo[] nonPkFieldArray, Connection con)
		...
		for (int i = 0; i < size; i++) {
			...
			parameters.add(new EntityConditionParameter(fieldInfo,
					valueMapInternal.getByIString(fieldInfo.name, fieldInfo.index), eqb));
		}
		...
		try {
			...
			// if (ed.entityName == "Subscription") logger.warn("Update ${this.toString()}
			// tx ${efi.getEcfi().transaction.getTransactionManager().getTransaction()} con
			// ${eqb.connection}")
			if (eqb.executeUpdate() == 0)
				throw new EntityException(
						"Tried to update a value that does not exist [" + this.toString() + "]. SQL used was "
								+ eqb.sqlTopLevel.toString() + ", parameters were " + eqb.parameters.toString());
			setSyncedWithDb();
		} catch (SQLException e) {
			String txName = "[could not get]";
			try {
			...
			logger.warn("Error updating " + this.toString() + " tx " + txName + " con " + eqb.connection.toString()
					+ ": " + e.toString());
			throw e;
		} finally {
			try {
	...
	public void deleteExtended(Connection con) throws SQLException {
		...
		try {
			...
			logger.warn("Error deleting " + this.toString() + " tx " + txName + " con " + eqb.connection.toString()
					+ ": " + e.toString());
			throw e;
		} finally {
			try {
	...
	public boolean refreshExtended() throws SQLException {
		...
		// NOTE: this simple approach may not work for view-entities, but not
		// restricting for now

		FieldInfo[] pkFieldArray = entityInfo.pkFieldInfoArray;
		FieldInfo[] allFieldArray = entityInfo.allFieldInfoArray;
		// NOTE: even if there are no non-pk fields do a refresh in order to see if the
		...
		eqb.makeSqlSelectFields(allFieldArray, null,
				"true".equals(efi.getDatabaseNode(ed.groupName).attribute("add-unique-as")));
		...
		try {
			...
			// efi.getEntityDbMeta().checkTableRuntime(ed)
			// if this is a view-entity and any table in it exists check/create all or will
			// fail with optional members, etc
			if (ed.isViewEntity)
				efi.getEntityDbMeta().checkTableRuntime(ed);
			...
			if (rs.next()) {
				...
				if (logger.isTraceEnabled())
					logger.trace("No record found in refresh for entity [" + resolveEntityName() + "] with values ["
							+ String.valueOf(getValueMap()) + "]");
			}
		} catch (SQLException e) {
			String txName = "[could not get]";
			try {
			...
			logger.warn("Error finding " + this.toString() + " tx " + txName + " con " + eqb.connection.toString()
					+ ": " + e.toString());
			throw e;
		} finally {
			try {
				...
				logger.error("Error in JDBC close in refresh of " + this.toString(), sqle);
			}
		}

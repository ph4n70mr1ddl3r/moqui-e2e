...
public class ListCondition implements EntityConditionImplBase {
	...
	public boolean mapMatches(Map<String, Object> map) {
		for (int i = 0; i < conditionListSize; i++) {
			...
			if (conditionMatches && this.operator == OR)
				return true;
			if (!conditionMatches && this.operator == AND)
				return false;
		}
		// if we got here it means that it's an OR with no trues, or an AND with no
		// falses
	...
	public EntityConditionImplBase filter(String entityAlias, EntityDefinition mainEd) {
		...
		for (int i = 0; i < conditionListSize; i++) {
			...
			if (filterCond != null)
				filteredList.add(filterCond);
		}
		int filteredSize = filteredList.size();
		if (filteredSize == conditionListSize)
			return this;
		if (filteredSize == 0)
			return null;
		// keep OR conditions together: return all if entityAlias is null (top-level
		// where) or null if not (sub-select where)
		if (operator == OR) {
			if (entityAlias == null)
				return this;
			return null;

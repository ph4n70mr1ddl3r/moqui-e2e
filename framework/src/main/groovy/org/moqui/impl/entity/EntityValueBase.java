...
public abstract class EntityValueBase implements EntityValue {
	...
	// NOTE: don't change these unless there is a really good reason, will break
	// localization
	...
	public void writeExternal(ObjectOutput out) throws IOException {
		// NOTE: found that the serializer in Hazelcast is slow with writeUTF(), uses
		// String.charAt() in a for loop
		// NOTE2: in Groovy this results in castToType() overhead anyway, so for now use
		// writeUTF/readUTF as other serialization might be more efficient
	...
	protected EntityFacadeImpl getEntityFacadeImpl() {
		// handle null after deserialize; this requires a static reference in Moqui.java
		...
		if (efiTransient == null) {
			...
			if (ecfi == null)
				throw new EntityException(
						"No ExecutionContextFactory found, cannot get EntityFacade for new EVB for entity "
								+ entityName);
	...
	private TransactionCache getTxCache(ExecutionContextFactoryImpl ecfi) {
		if (txCacheInternal == null)
			txCacheInternal = ecfi.transactionFacade.getTransactionCache();
	...
	public LiteStringMap<Object> getValueMap() {
		return valueMapInternal;
	}
	protected LiteStringMap<Object> getDbValueMap() {
		return dbValueMap;
	}
	...
	protected void setDbValueMap(Map<String, Object> map) {
		...
		// copy all fields, including pk to fix false positives in the old approach of
		// only non-pk fields
		for (int i = 0; i < allFields.length; i++) {
			FieldInfo fi = allFields[i];
			if (!map.containsKey(fi.name))
				continue;
			...
			if (!valueMapInternal.containsKeyIString(fi.name, fi.index))
				valueMapInternal.putByIString(fi.name, curValue, fi.index);
		}
		isFromDb = true;
	}
	...
	@Override
	public String resolveEntityNamePretty() {
		return StringUtilities.camelCaseToPretty(getEntityDefinition().getEntityName());
	}
	@Override
	...
	private boolean isFieldModifiedIString(String name) {
		...
		/*
		 * if (!valueMapInternal.containsKey(name)) return false; if (dbValueMap == null
		 * || !dbValueMap.containsKey(name)) return true; Object valueMapValue =
		 * valueMapInternal.get(name); Object dbValue = dbValueMap.get(name); return
		 * (valueMapValue == null && dbValue != null) || (valueMapValue != null &&
		 * !valueMapValue.equals(dbValue));
		 */
	}
	@Override
	public EntityValue touchField(String name) {
		if (!getEntityDefinition().isField(name))
			throw new IllegalArgumentException(
					"Cannot touch field name " + name + ", does not exist on entity " + entityName);
		modified = true;
		if (touchedFields == null)
			touchedFields = new HashSet<>();
	...
	public Map<String, Object> getMap() {
		...
		for (int i = 0; i < allFieldInfosSize; i++) {
			...
			// NOTE DEJ20151117 also put nulls in Map, make more complete, removed: if
			// (fieldValue != null)
		...
		if (ed.isViewEntity) {
			...
			if (pqExpressionNodeMap != null)
				for (String fieldName : pqExpressionNodeMap.keySet()) {
	...
	public Object get(final String name) {
		...
		// if this is not a valid field name but is a valid relationship name, do a
		// getRelated or getRelatedOne to return an EntityList or an EntityValue
		...
		// logger.warn("========== relInfo Map keys:
		// ${ed.getRelationshipInfoMap().keySet()}, relInfoList:
		// ${ed.getRelationshipsInfo(false)}")
		throw new EntityException(
				"The name [" + name + "] is not a valid field name or relationship name for entity " + entityName);
	}

	public Object getKnownField(FieldInfo fieldInfo) {
		EntityDefinition ed = fieldInfo.ed;
		// if this is a simple field (is field, no l10n, not user field) just get the
		// value right away (vast majority of use)
			...
			if (localeStr != null) {
				...
				if (localizedByLocaleByField == null) {
					...
					if (localizedByLocale != null) {
						...
						if (cachedLocalized != null && cachedLocalized.length() > 0) {
							// logger.warn("======== field ${name}:${internalValue} found cached localized
							// ${cachedLocalized}")
				...
				if (!knownNoLocalized) {
					...
					if (ed.isViewEntity && !ed.entityInfo.isDynamicView) {
						// NOTE: there are issues with dynamic view entities here, may be possible to
						// fix them but for now not running for EntityDynamicView
					...
					if (pks.size() == 1) {
						...
						if (aliasNode != null) {
							pk = null;
							Map<String, String> pkToAliasMap = ed
									.getMePkFieldToAliasNameMap(aliasNode.attribute("entity-alias"));
							Set<String> pkSet = pkToAliasMap.keySet();
							if (pkSet.size() == 1)
								pk = pkToAliasMap.get(pkSet.iterator().next());
						}

						String pkValue = pk != null ? (String) valueMapInternal.get(pk) : null;
						if (pkValue != null) {
							// logger.warn("======== field ${name}:${internalValue} finding
							// LocalizedEntityField, localizedByLocaleByField=${localizedByLocaleByField}")
							String entityName = ed.getFullEntityName();
							String fieldName = name;
							if (aliasNode != null) {
								entityName = memberEntityName;
								final String fieldAttr = aliasNode.attribute("field");
								fieldName = fieldAttr != null && !fieldAttr.isEmpty()
										? fieldAttr
										: aliasNode.attribute("name");
								// logger.warn("localizing field for ViewEntity ${ed.fullEntityName} field
								// ${name}, using entityName: ${entityName}, fieldName: ${fieldName}, pkValue:
								// ${pkValue}, locale: ${localeStr}")
							}
							...
							if (localeStr.contains("_")) {
								...
								if (lefValue != null) {
									String localized = (String) lefValue.get("localized");
									CollectionUtilities.addToMapInMap(name, localeStr, localized,
											localizedByLocaleByField);
									return localized;
								}
							}
					...
					// we didn't find a localized value, remember that so we don't do the queries
					// again (common case)
					CollectionUtilities.addToMapInMap(name, localeStr, null, localizedByLocaleByField);
					// logger.warn("======== field ${name}:${internalValue} remembering no
					// localized, localizedByLocaleByField=${localizedByLocaleByField}")
				}
	...
	protected Object getOldDbValue(String name) {
		if (oldDbValueMap != null && oldDbValueMap.containsKey(name))
			return oldDbValueMap.get(name);
	...
	public Map<String, Object> getPrimaryKeys() {
		/*
		 * don't use cached internalPkMap, would have to make sure to capture all set,
		 * put, setFields, setFieldsEv, etc to invalidate otherwise may be stale is just
		 * as fast to recreate by index gets on valueMapInternal vs cloning the cached
		 * LiteStringMap protected transient LiteStringMap<Object> internalPkMap = null;
	...
	public String getPrimaryKeysString() {
		...
		if (pkFieldInfoArray.length == 1) {
			...
			for (int pki = 0; pki < pkFieldInfoArray.length; pki++) {
				...
				// NOTE: separator of '::' matches separator used for combined PK String in
				// EntityDefinition.getPrimaryKeysString() and EntityDataDocument.makeDocId()
				if (pkCombinedSb.length() > 0)
					pkCombinedSb.append("::");
				pkCombinedSb
						.append(ObjectUtilities.toPlainString(this.valueMapInternal.getByIString(fi.name, fi.index)));
			}
	...
	public EntityValue setBytes(String name, byte[] theBytes) {
		try {
			if (theBytes != null)
				set(name, new SerialBlob(theBytes));
		} catch (Exception e) {
	...
	public EntityValue setSequencedIdPrimary() {
		...
		putKnownField(ed.entityInfo.pkFieldInfoArray[0],
				entityPrefix != null ? entityPrefix + sequenceValue : sequenceValue);
		return this;
	}
	...
	public EntityValue setSequencedIdSecondary() {
		...
		if (pkFields.size() < 2)
			throw new EntityException("Cannot call setSequencedIdSecondary() on entity " + entityName
					+ ", must have at least 2 primary key fields.");
		...
		int paddedLength = 2;
		if (paddedLengthStr != null && paddedLengthStr.length() > 0)
			paddedLength = Integer.valueOf(paddedLengthStr);
		...
		// temporarily disable authz for this, just doing lookup to get next value and
		// to allow for a
		...
		// logger.warn("TOREMOVE in setSequencedIdSecondary ef
		// WHERE=${ef.getWhereEntityCondition()}")
		...
		for (EntityValue curValue : allValues) {
			...
			if (currentSeqId != null && !currentSeqId.isEmpty()) {
				try {
					...
					logger.warn("Error in secondary sequenced ID converting SeqId [" + currentSeqId + "] in field ["
							+ seqFieldName + "] from entity [" + resolveEntityName() + "] to a number: "
							+ e.toString());
				}
			}
		}
	...
	public int compareTo(EntityValue that) {
		...
		// next compare all fields (will compare PK fields first, generally first in
		// list)
	...
	public EntityValue createOrUpdate() {
		...
		if (isFromDb) {
			...
			// make sure PK fields with defaults are filled in BEFORE doing the refresh to
			// see if it exists
			checkSetFieldDefaults(getEntityDefinition(), getEntityFacadeImpl().ecfi.getEci(), true);
		}

		// logger.warn("createOrUpdate isFromDb " + isFromDb + " pkModified " +
		// pkModified);
	...
	private void handleAuditLog(boolean isUpdate, LiteStringMap<Object> oldValues, EntityDefinition ed,
			ExecutionContextImpl ec) {
		if ((isUpdate && oldValues == null) || !ed.entityInfo.needsAuditLog
		...
		for (int i = 0; i < fieldInfoList.length; i++) {
			...
			if ((!isLogUpdate && "true".equals(fieldInfo.enableAuditLog)) || (isUpdate && isLogUpdate)) {
				...
				if (!this.valueMapInternal.containsKeyIString(fieldInfo.name, fieldInfo.index))
					continue;
				...
				// if set to log updates and old value is null don't consider it an update (is
				// initial set of value)
				if (isLogUpdate && oldValue == null)
					continue;
				if (isUpdate) {
					// if isUpdate but old value == new value, then it hasn't been updated, so skip
					// it
					if (value == null) {
						if (oldValue == null)
							continue;
				...
				// logger.warn("EntityAuditLog field " + fieldName + " old " + oldValue + " (" +
				// (oldValue != null ? oldValue.getClass().getName() : "null") + ") new " +
				// value + " (" + (value != null ? value.getClass().getName() : "null") + ")");

				// don't skip for this, if a field was reset then we want to record that: if
				// (!value) continue
				...
				if (stackNameString.length() > 4000)
					stackNameString = stackNameString.substring(0, 4000);
				...
				if (changeReason != null)
					parms.put("changeReason", changeReason);
				...
				if (value != null) {
					...
					if (fieldInfo.encrypt)
						newValueText = EntityJavaUtil.enDeCrypt(newValueText, true, ec.getEntityFacade());
					if (newValueText.length() > 4000)
						newValueText = newValueText.substring(0, 4000);
					parms.put("newValueText", newValueText);
				}
				if (oldValue != null) {
					String oldValueText = ObjectUtilities.toPlainString(oldValue);
					if (fieldInfo.encrypt)
						oldValueText = EntityJavaUtil.enDeCrypt(oldValueText, true, ec.getEntityFacade());
					if (oldValueText.length() > 4000)
						oldValueText = oldValueText.substring(0, 4000);
					parms.put("oldValueText", oldValueText);
				}

				// set all pk fields by name to support EntityAuditLog extensions for specific
				// pk fields, will usually all get ignored
				parms.putAll(pksValueMap);

				// logger.warn("TOREMOVE: in handleAuditLog for [${ed.entityName}.${fieldName}]
				// value=[${value}], oldValue=[${oldValue}], oldValues=[${oldValues}]", new
				// Exception("AuditLog location"))

				// NOTE: if this is changed to async the time zone on nowTimestamp gets messed
				// up (user's time zone lost)
	...
	private void addThreeFieldPkValues(Map<String, Object> parms, EntityDefinition ed) {
		// get pkPrimaryValue, pkSecondaryValue, pkRestCombinedValue (just like the
		// AuditLog stuff)
		...
		for (int i = 0; i < pkFieldList.size(); i++) {
			...
			pkTextSb.append(curFieldInfo.name).append(":'")
					.append(EntityDefinition.getFieldStringForFile(curFieldInfo, getKnownField(curFieldInfo)))
					.append("'");
		}
		String pkText = pkTextSb.toString();

		if (firstPkField != null)
			parms.put("pkPrimaryValue", getKnownField(firstPkField));
		if (secondPkField != null)
			parms.put("pkSecondaryValue", getKnownField(secondPkField));
		if (!pkText.isEmpty())
			parms.put("pkRestCombinedValue", pkText);
	}
	...
	public EntityList findRelated(final String relationshipName, Map<String, Object> byAndFields, List<String> orderBy,
		...
		if (relInfo == null)
			throw new EntityException("Relationship " + relationshipName + " not found in entity " + entityName);
	...
	private EntityList findRelated(final EntityJavaUtil.RelationshipInfo relInfo, Map<String, Object> byAndFields,
		...
		if (keyMap == null || keyMap.size() == 0)
			throw new EntityException("Relationship " + relInfo.relationshipName + " in entity " + entityName
					+ " has no key-map sub-elements and no default values");

		// make a Map where the key is the related entity's field name, and the value is
		// the value from this entity
		...
		if (byAndFields != null && byAndFields.size() > 0)
			condMap.putAll(byAndFields);

		EntityFind find = getEntityFacadeImpl().find(relatedEntityName);
		return find.condition(condMap).orderBy(orderBy).useCache(useCache)
				.forUpdate(forUpdate != null ? forUpdate : false).list();
	}
	...
	private EntityValue findRelatedOne(final EntityJavaUtil.RelationshipInfo relInfo, Boolean useCache,
			Boolean forUpdate) {
		String relatedEntityName = relInfo.relatedEntityName;
		Map<String, String> keyMap = relInfo.keyMap;
		if (keyMap == null || keyMap.size() == 0)
		...
		// logger.warn("========== findRelatedOne ${relInfo.relationshipName}
		// keyMap=${keyMap}, condMap=${condMap}")
	...
	public EntityList findRelatedFk(Set<String> skipEntities) {
		...
		for (int i = 0; i < relInfoListSize; i++) {
			...
			if (reverseInfo == null || !reverseInfo.isTypeOne
					|| (skipEntities != null && (skipEntities.contains(reverseInfo.fromEd.fullEntityName)
							|| skipEntities.contains(reverseInfo.fromEd.getShortAlias())
							|| skipEntities.contains(reverseInfo.fromEd.getEntityName()))))
				continue;
	...
	public boolean deleteWithRelated(Set<String> relationshipsToDelete) {
		if (relationshipsToDelete == null)
			relationshipsToDelete = new HashSet<>();
		ArrayList<EntityJavaUtil.RelationshipInfo> relInfoList = getEntityDefinition().getRelationshipsInfo(false);
		int relInfoListSize = relInfoList.size();

		// look for related records that exist and that we won't delete, if any return
		// true
		...
		for (int i = 0; i < relInfoListSize; i++) {
			...
			if (relInfo.isTypeOne)
				continue;
			if (relationshipsToDelete.contains(relInfo.shortAlias)
					|| relationshipsToDelete.contains(relInfo.relationshipName))
				continue;

			if (findRelatedCount(relInfo.relationshipName, false) > 0) {
				if (logger.isInfoEnabled())
					logger.info("Not deleting entity " + entityName + " value with PK " + getPrimaryKeys()
							+ ", found record in relationship " + relInfo.relationshipName);
				foundNonDeleteRelated = true;
				break;
			}
		}
		if (foundNonDeleteRelated)
			return false;
	...
	public void deleteWithCascade(Set<String> clearRefEntities, Set<String> validateAllowDeleteEntities) {
		...
		int relInfoListSize = relInfoList.size();
		for (int i = 0; i < relInfoListSize; i++) {
			// find relationships with a type one reverse (relationships for records that
			// depend on this)
			EntityJavaUtil.RelationshipInfo relInfo = relInfoList.get(i);
			EntityJavaUtil.RelationshipInfo reverseInfo = relInfo.findReverse();
			if (reverseInfo == null || !reverseInfo.isTypeOne)
			...
			boolean clearRef = clearRefEntities != null && (clearRefEntities.contains(relEd.fullEntityName)
					|| clearRefEntities.contains(relEd.getShortAlias())
					|| clearRefEntities.contains(relEd.getEntityName()));
			// find records
				...
				if (clearRef) {
					for (String fieldName : reverseInfo.keyMap.keySet()) {
						if (relEd.isPkField(fieldName))
							throw new EntityException("In deleteWithCascade on entity " + resolveEntityName()
									+ " related entity " + relEd.fullEntityName + " is in the clear ref set but field "
									+ fieldName + " is a primary key field and cannot be cleared");
						relVal.set(fieldName, null);
					}
					relVal.update();
				} else {
					// if we should validate entities we are attempting to delete do that now
					if (validateAllowDeleteEntities != null
							&& !validateAllowDeleteEntities.contains(relEd.fullEntityName))
						throw new EntityException("Cannot delete " + resolveEntityNamePretty() + " " + getPrimaryKeys()
								+ ", found " + relVal.resolveEntityNamePretty() + " " + relVal.getPrimaryKeys()
								+ " that depends on it");
					// delete with cascade
	...
	public boolean checkFks(boolean insertDummy) {
		...
		for (EntityJavaUtil.RelationshipInfo relInfo : getEntityDefinition().getRelationshipsInfo(false)) {
			if (!"one".equals(relInfo.type))
				continue;

			EntityValue value = findRelatedOne(relInfo, false, false);
			// if (resolveEntityName().contains("foo")) logger.info("Checking fk " +
			// resolveEntityName() + ':' + relInfo.relationshipName + " value: " + value);
			if (value == null) {
				if (insertDummy) {
					noneMissing = false;
					...
					if (keyMap == null || keyMap.isEmpty())
						throw new EntityException("Relationship " + relInfo.relationshipName + " in entity "
								+ entityName + " has no key-map sub-elements and no default values");
					...
					if (newValue.containsPrimaryKey()) {
						...
						newValue.create();
						logger.warn(
								"Created dummy " + newValue.resolveEntityName() + " PK " + newValue.getPrimaryKeys());
					}
				} else {
					return false;
				}
	...
	@Override
	@SuppressWarnings("unchecked")
	public long checkAgainstDatabaseInfo(List<Map<String, Object>> diffInfoList, List<String> messages,
			String location) {
		...
		try {
			...
			if (!dbValue.refresh()) {
				...
				// alternative object based, more efficient but way less convenient:
				// diffInfoList.add(new EntityJavaUtil.EntityValueDiffInfo(resolveEntityName(),
				// getPrimaryKeys()));
				return 0;
			}
			...
			for (String nonpkFieldName : this.getEntityDefinition().getNonPkFieldNames()) {
				...
				if ("lastUpdatedStamp".equals(nonpkFieldName))
					continue;

				final Object checkFieldValue = this.get(nonpkFieldName);
				final Object dbFieldValue = dbValue.get(nonpkFieldName);

				// use compareTo if available, generally more lenient (for BigDecimal ignores
				// scale, etc)
				if (checkFieldValue != null) {
					...
					if (checkFieldValue instanceof Comparable && dbFieldValue != null) {
						...
						if (cfComp.compareTo(dbFieldValue) != 0)
							areSame = false;
					} else {
						if (!checkFieldValue.equals(dbFieldValue))
							areSame = false;
					}
					if (!areSame) {
						Map<String, Object> diffInfo = new HashMap<>();
						...
						// alternative object based, more efficient but way less convenient:
						// diffInfoList.add(new EntityJavaUtil.EntityValueDiffInfo(resolveEntityName(),
						// getPrimaryKeys(), nonpkFieldName, checkFieldValue, dbFieldValue));
					}
				}
				fieldsChecked++;
			}
			...
			String errMsg = "Error checking entity " + resolveEntityName() + " with pk " + getPrimaryKeys() + ": "
					+ t.toString();
			if (messages != null)
				messages.add(errMsg);
		...
		try {
			...
			for (String nonpkFieldName : this.getEntityDefinition().getNonPkFieldNames()) {
				...
				if (checkFieldValue != null) {
					...
					if (!areSame)
						messages.add("Field " + resolveEntityName() + "." + nonpkFieldName
								+ " did not match; check (file) value [" + checkFieldValue + "], db value ["
								+ dbFieldValue + "] for primary key " + getPrimaryKeys());
				}
				fieldsChecked++;
			}
		} catch (EntityException e) {
	...
	public Element makeXmlElement(Document document, String prefix) {
		...
		if (document != null)
			element = document.createElement(prefix + entityName);
		if (element == null)
			return null;
	...
	public int writeXmlText(Writer pw, String prefix, int dependentLevels) {
		...
		try {
			...
			throw new EntityException(
					"Error writing XML test for entity " + entityName + " dependent levels " + dependentLevels);
		}
	}
	...
	@SuppressWarnings("unchecked")
	private static int plainMapXmlWriter(Writer pw, String prefix, String objectName, Map<String, Object> plainMap,
			int level) throws IOException, SerialException {
		if (prefix == null)
			prefix = "";
		// if a CDATA element is needed for a field it goes in this Map to be added at
		// the end
		Map<String, String> cdataMap = new LinkedHashMap<>();
		Map<String, Object> subPlainMap = new LinkedHashMap<>();
		String curEntity = objectName != null && objectName.length() > 0
				? objectName
				: (String) plainMap.get("_entity");

		for (int i = 0; i < level; i++)
			pw.append(indentString);
		// mostly for relationship names, see opposite code in the
		// EntityDataLoaderImpl.startElement
		if (curEntity.contains("#"))
		...
		for (Entry<String, Object> entry : plainMap.entrySet()) {
			...
			// leave this out, not needed for XML where the element name represents the
			// entity or relationship
			if ("_entity".equals(fieldName))
				continue;
				...
				if (entryVal instanceof List) {
					for (Object listEntry : (List) entryVal) {
						if (listEntry instanceof Map) {
							...
							logger.warn("In entity auto create for entity " + curEntity + " found list for sub-object "
									+ entryKey + " with a non-Map entry: " + String.valueOf(listEntry));
						}
					}
	...
	private Map<String, Object> internalPlainValueMap(int dependentLevels, Set<String> parentPkFields) {
		...
		if (dependentLevels > 0) {
			...
			// keep track of all parent PK field names, even not part of this entity's PK,
			// they will be inherited when read
			if (parentPkFields != null)
				curPkFields.addAll(parentPkFields);
			...
			for (EntityJavaUtil.RelationshipInfo relInfo : relInfoList) {
				...
				if (relInfo.isTypeOne) {
					...
					if (relEv != null)
						vMap.put(entryName,
								((EntityValueBase) relEv).internalPlainValueMap(dependentLevels - 1, curPkFields));
				} else {
	...
	public Map<String, Object> getMasterValueMap(final String name) {
		...
		if (masterDefinition == null)
			throw new EntityException(
					"No master definition found for name [" + name + "] in entity [" + entityName + "]");
		return internalMasterValueMap(masterDefinition.getDetailList(), null, null);
	}

	private Map<String, Object> internalMasterValueMap(ArrayList<EntityDefinition.MasterDetail> detailList,
			Set<String> parentPkFields, EntityJavaUtil.RelationshipInfo parentRelInfo) {
		Map<String, Object> vMap = new HashMap<>(valueMapInternal);
		CollectionUtilities.removeNullsFromMap(vMap);
		if (parentPkFields != null) {
			if (parentRelInfo != null) {
				// handle cases like the Product toAssocs relationship where
				// ProductAssoc.productId != Product.productId, needs to look at relationship
				// field map
				for (String pkField : parentPkFields) {
					String relatedName = parentRelInfo.keyMap.get(pkField);
					if (pkField.equals(relatedName))
						vMap.remove(pkField);
				}
			} else {
				for (String pkField : parentPkFields)
		...
		if (detailList != null && !detailList.isEmpty()) {
			...
			for (int i = 0; i < detailListSize; i++) {
				...
				if (relInfo.isTypeOne) {
					EntityValue relEv = findRelatedOne(relationshipName, null, false);
					if (relEv != null)
						vMap.put(entryName, ((EntityValueBase) relEv).internalMasterValueMap(detail.getDetailList(),
								curPkFields, relInfo));
				} else {
					EntityList relList = findRelated(relationshipName, null, null, null, false);
					if (relList != null && !relList.isEmpty()) {
						...
						for (int rlIndex = 0; rlIndex < relListSize; rlIndex++) {
							...
							plainRelList.add(((EntityValueBase) relEv).internalMasterValueMap(detail.getDetailList(),
									curPkFields, relInfo));
						}
						vMap.put(entryName, plainRelList);
					}
	...
	public boolean containsKey(Object o) {
		return valueMapInternal.containsKey(o);
	}
	@Override
	...
	public Object get(Object o) {
		if (o instanceof CharSequence) {
			// This may throw an exception, and let it; the Map interface doesn't provide
			// for EntityException
	...
	public Object putNoCheck(final String name, Object value) {
		// NOTE: for performance with LiteStringMap this is no longer useful, and
		// invalid field names not allowed, so just use put()
		FieldInfo fieldInfo = getEntityDefinition().getFieldInfo(name);
		if (fieldInfo == null)
			throw new EntityException("The field name " + name + " is not valid for entity " + entityName);
		return putKnownField(fieldInfo, value);
	...
	protected Object putKnownField(final FieldInfo fieldInfo, Object value) {
		if (!mutable)
			throw new EntityException(
					"Cannot set field " + fieldInfo.name + ", this entity value is not mutable (it is read-only)");
		...
		if (isFromDb) {
			...
			if (curValue == null) {
				...
				if (!curValue.equals(value)) {
					modified = true;
					if (dbValueMap == null)
						dbValueMap = new LiteStringMap<>(getEntityDefinition().allFieldNameList.size())
								.useManualIndex();
	...
	public @Nonnull Collection<Object> values() {
		// everything needs to go through the get method, so iterate through the fields
		// and get the values
	...
	private boolean doDataFeed(ExecutionContextImpl ec) {
		if (ec.artifactExecutionFacade.entityDataFeedDisabled())
			return false;
	...
	private void checkSetFieldDefaults(EntityDefinition ed, ExecutionContext ec, Boolean pks) {
		// allow updating a record without specifying default PK fields, so don't check
		// this: if (isCreate) {
		Map<String, String> pkDefaults = ed.entityInfo.pkFieldDefaults;
		if ((pks == null || pks) && pkDefaults != null && pkDefaults.size() > 0)
			for (Entry<String, String> entry : pkDefaults.entrySet())
	...
	private String makeErrorMsg(String baseMsg, String expandMsg, EntityDefinition ed, ExecutionContextImpl ec) {
		...
		// TODO: need a different approach for localization, getting from DB may not be
		// reliable after an error and may cause other errors (especially with Postgres
		// and the auto rollback only)
		...
		if (errorMessage == null)
			errorMessage = baseMsg + " " + ed.getEntityName() + " " + getPrimaryKeys();
		return errorMessage;
	}
	...
	private void registerMutateLock() {
		...
		// add EntityRecordLock for each type one (with FK) relationship where FK fields
		// not null
		...
		for (int ri = 0; ri < relInfoListSize; ri++) {
			...
			if (keyFieldSize == 1) {
				...
				if (keyValue != null)
					pkString = ObjectUtilities.toPlainString(keyValue);
			} else {
				...
				for (int ki = 0; ki < keyFieldSize; ki++) {
					...
					if (keyValue == null) {
						...
						// use relInfo.keyMap to get the field name of the PK field on the related
						// entity
						relFieldValues.put(relInfo.keyMap.get(keyFieldName), keyValue);
					}
				}
				if (hasAllValues)
					pkString = relInfo.relatedEd.getPrimaryKeysString(relFieldValues);
			}

			if (pkString != null) {
				tfi.registerRecordLock(
						new EntityRecordLock(relInfo.relatedEd.getFullEntityName(), pkString, stackArray));
			}
		}
	}
	...
	public EntityValue create() {
		...
		// check/set defaults
		if (entityInfo.hasFieldDefaults)
			checkSetFieldDefaults(ed, ec, null);
		...
		if (lastUpdatedStampInfo != null
				&& valueMapInternal.getByIString(lastUpdatedStampInfo.name, lastUpdatedStampInfo.index) == null)
			valueMapInternal.putByIString(lastUpdatedStampInfo.name, new Timestamp(lastUpdatedLong),
					lastUpdatedStampInfo.index);

		// do the artifact push/authz
		ArtifactExecutionInfoImpl aei = new ArtifactExecutionInfoImpl(entityName, ArtifactExecutionInfo.AT_ENTITY,
				ArtifactExecutionInfo.AUTHZA_CREATE, "create").setParameters(valueMapInternal);
		...
		try {
			...
			if (doDataFeed(ec))
				efi.getEntityDataFeed().dataFeedCheckAndRegister(this, false, valueMapInternal, null);

			// if there is not a txCache or the txCache doesn't handle the create, call the
			// abstract method to create the main record
			TransactionCache curTxCache = getTxCache(ecfi);
			if (curTxCache == null || !curTxCache.create(this)) {
				// NOTE: calls basicCreate() instead of createExtended() directly so don't
				// register lock here

				this.basicCreate(null);
			}

			// NOTE: cache clear is the same for create, update, delete; even on create need
			// to clear one cache because it
	...
	/**
	 * This method should create a corresponding record in the datasource. NOTE:
	 * fieldInfoArray may have null values after valid ones, the length is not the
	...
	public EntityValue update() {
		...
		if (hasFieldDefaults)
			checkSetFieldDefaults(ed, ec, true);

		// if there is one or more DataFeed configs associated with this entity get info
		// about them
		boolean curDataFeed = doDataFeed(ec);
		if (curDataFeed) {
			ArrayList<EntityDataFeed.DocumentEntityInfo> entityInfoList = efi.getEntityDataFeed()
					.getDataFeedEntityInfoList(entityName);
		...
		if (hasFieldDefaults)
			checkSetFieldDefaults(ed, ec, false);

		// Save original values before anything is changed for DataFeed and audit log
		LiteStringMap<Object> originalValues = dbValueMap != null && !dbValueMap.isEmpty()
				? new LiteStringMap<>(dbValueMap).useManualIndex()
				: null;

		// do the artifact push/authz
		ArtifactExecutionInfoImpl aei = new ArtifactExecutionInfoImpl(entityName, ArtifactExecutionInfo.AT_ENTITY,
				ArtifactExecutionInfo.AUTHZA_UPDATE, "update").setParameters(valueMapInternal);
		...
		try {
			...
			for (int i = 0; i < size; i++) {
				...
				if (isFieldModifiedIString(fieldInfo.name)) {
					if (fieldInfo.isLastUpdatedStamp) {
						...
						if (dbValueMap == null || dbValueMap.getByIString(fieldInfo.name, fieldInfo.index) == null)
							continue;
					...
					if (createOnlyAny && fieldInfo.createOnly) {
						if (changedCreateOnlyFields == null)
							changedCreateOnlyFields = new ArrayList<>();
						changedCreateOnlyFields.add(fieldInfo.name);
					}
				}
			}

			// if (ed.getEntityName() == "foo") logger.warn("================ evb.update()
			// ${resolveEntityName()}
			// nonPkFieldList=${nonPkFieldList};\nvalueMap=${valueMap};\noldValues=${oldValues}")
			if (nonPkFieldArrayIndex == 0 || (nonPkFieldArrayIndex == 1 && modifiedLastUpdatedStamp)) {
				if (logger.isTraceEnabled())
					logger.trace("Not doing update on entity with no changed non-PK fields; value=" + this.toString());
				return this;
			}

			// do this after the empty nonPkFieldList check so that if nothing has changed
			// then ignore the attempt to update
			if (changedCreateOnlyFields != null && changedCreateOnlyFields.size() > 0)
				throw new EntityException("Cannot update create-only (immutable) fields " + changedCreateOnlyFields
						+ " on entity " + resolveEntityName());

			// check optimistic lock with lastUpdatedStamp; if optimisticLock() dbValueMap
			// will have latest from DB
			FieldInfo lastUpdatedStampInfo = ed.entityInfo.lastUpdatedStampInfo;
			if (optimisticLock) {
				...
				if (valueLus != null && dbLus != null && !dbLus.equals(valueLus))
					throw new EntityException("This record was updated by someone else at " + dbLus
							+ " which was after the version you loaded at " + valueLus
							+ ". Not updating to avoid overwriting data.");
			}
			...
			if (!modifiedLastUpdatedStamp && lastUpdatedStampInfo != null) {
				...
				// never gets used after this point, but if ever does will need to:
				// nonPkFieldArrayIndex++
			}
			...
			// if there is not a txCache or the txCache doesn't handle the update, call the
			// abstract method to update the main record
			if (curTxCache == null || !curTxCache.update(this)) {
				...
				// if ("OrderHeader".equals(ed.getEntityName()) &&
				// "55500".equals(valueMapInternal.get("orderId"))) logger.warn("Called
				// updateExtended order " + this.valueMapInternal.toString());
			}
			...
			// save audit log(s) if applicable
			if (needsAuditLog)
				handleAuditLog(true, originalValues, ed, ec);
			// run EECA after rules
	...
	public void basicUpdate(Connection con) throws SQLException {
		...
		/*
		 * Shouldn't need this any more, was from a weird old issue: boolean
		 * dbValueMapFromDb = false // it may be that the oldValues map is full of null
		 * values because the EntityValue didn't come from the db if (dbValueMap) for
		 * (Object val in dbValueMap.values()) if (val != null) { dbValueMapFromDb =
		 * true; break }
		 */
	...
	/**
	 * This method should update the corresponding record in the datasource. NOTE:
	 * fieldInfoArray may have null values after valid ones, the length is not the
	 * actual number of fields.
	 */
	...
	public EntityValue delete() {
		...
		// NOTE: this is create-only on the entity, ignores setting on fields (only
		// considered in update)
		if (entityInfo.createOnly)
			throw new EntityException(
					"Entity [" + resolveEntityName() + "] is create-only (immutable), cannot be deleted.");

		// do the artifact push/authz
		ArtifactExecutionInfoImpl aei = new ArtifactExecutionInfoImpl(entityName, ArtifactExecutionInfo.AT_ENTITY,
				ArtifactExecutionInfo.AUTHZA_DELETE, "delete").setParameters(valueMapInternal);
		aefi.pushInternal(aei, !entityInfo.authorizeSkipTrue, false);
		...
		try {
			...
			// check DataDocuments to update (if not primary entity) or delete (if primary
			// entity)
			efi.getEntityDataFeed().dataFeedCheckDelete(this);

			// if there is not a txCache or the txCache doesn't handle the delete, call the
			// abstract method to delete the main record
			TransactionCache curTxCache = getTxCache(ecfi);
	...
	public boolean refresh() {
		...
		if (pkFieldList.size() == 0) {
			// throw new EntityException("Entity ${resolveEntityName()} has no primary key
			// fields, cannot do refresh.")
			if (logger.isTraceEnabled())
				logger.trace("Entity " + resolveEntityName() + " has no primary key fields, cannot do refresh.");
			return false;
		}
		...
		// do the artifact push/authz
		ArtifactExecutionInfoImpl aei = new ArtifactExecutionInfoImpl(entityName, ArtifactExecutionInfo.AT_ENTITY,
				ArtifactExecutionInfo.AUTHZA_VIEW, "refresh").setParameters(valueMapInternal);
		aefi.pushInternal(aei, !ed.entityInfo.authorizeSkipView, false);

		boolean retVal = false;
		try {
			// find EECA rules deprecated, not worth performance hit:
			// efi.runEecaRules(fullEntityName, this, "find-one", true);

			// if there is not a txCache or the txCache doesn't handle the refresh, call the
			// abstract method to refresh
			TransactionCache curTxCache = getTxCache(ecfi);
			if (curTxCache != null)
				retVal = curTxCache.refresh(this);
			// call the abstract method
			if (!retVal) {
				retVal = this.refreshExtended();
				if (retVal && curTxCache != null)
					curTxCache.onePut(this, false);
			}

			// find EECA rules deprecated, not worth performance hit:
			// efi.runEecaRules(fullEntityName, this, "find-one", false);
		} catch (SQLException e) {
	...
	private static class EntityFieldEntry implements Entry<String, Object> {
		...
		public boolean equals(Object obj) {
			...
			if (!fi.name.equals(other.fi.name))
				return false;

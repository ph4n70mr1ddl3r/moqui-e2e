...
public class FieldInfo {
	...
	public FieldInfo(EntityDefinition ed, MNode fieldNode, int index) {
		...
		// NOTE: intern a must here for use with LiteStringMap, without this all sorts
		// of bad behavior, not finding any fields sort of thing
		...
		String colNameToUse = columnNameAttr != null && columnNameAttr.length() > 0
				? columnNameAttr
		...
		if (databaseNode != null) {
			...
			for (int i = 0; i < nameReplaceNodes.size(); i++) {
				...
				if (colNameToUse.equalsIgnoreCase(nameReplaceNode.attribute("original"))) {
					...
					logger.info("Replacing column name " + colNameToUse + " with replace name " + replaceName
		...
		if ((typeAttr == null || typeAttr.length() == 0)
				&& (fieldNode.hasChild("complex-alias") || fieldNode.hasChild("case"))
				&& fnAttrs.get("function") != null) {
		...
		enableAuditLog = enableAuditLogAttr != null
				? enableAuditLogAttr
				: ed.internalEntityNode.attribute("enable-audit-log");
		...
		if (ed.isViewEntity) {
			...
			for (int i = 0; i < cafListSize; i++) {
				...
				if (cafEntityAlias != null && cafEntityAlias.length() > 0)
					entityAliasUsedSet.add(cafEntityAlias);
			}
			...
			hasAggregateFunction = isAggregateAttr != null
	...
	/**
	 * Full column name for complex finds on view entities; plain entity column
	 * names are never expanded
	 */
	public String getFullColumnName() {
		if (fullColumnNameInternal != null)
			return fullColumnNameInternal;
		...
		try {
			switch (typeValue) {
				...
				case 2 : // outValue = java.sql.Timestamp.valueOf(value);
					...
					if (outValue == null)
						throw new BaseArtifactException("The value [" + value + "] is not a valid date/time for field "
								+ entityName + "." + name);
					break;
				case 3 : // outValue = java.sql.Time.valueOf(value);
					...
					if (outValue == null)
						throw new BaseArtifactException(
								"The value [" + value + "] is not a valid time for field " + entityName + "." + name);
					break;
				case 4 : // outValue = java.sql.Date.valueOf(value);
					...
					if (outValue == null)
						throw new BaseArtifactException(
								"The value [" + value + "] is not a valid date for field " + entityName + "." + name);
					break;
				...
				case 9 : // outValue = new BigDecimal(value); break
					...
					if (bdVal == null) {
						throw new BaseArtifactException("The value [" + value + "] is not valid for type " + javaType
								+ " for field " + entityName + "." + name);
					} else {
				...
				case 12 :
					try {
						...
						throw new BaseArtifactException("Error creating SerialBlob for value [" + value + "] for field "
								+ entityName + "." + name);
					}
					break;
				case 13 :
				...
				case 14 :
					...
					break;
				// better way for Collection (15)? maybe parse comma separated, but probably
			...
			switch (typeValue) {
				...
				case 9 :
					if (value instanceof BigDecimal)
						value = safeStripZeroes((BigDecimal) value);
				...
				case 12 :
					if (value instanceof byte[]) {
						...
						logger.info("Field on entity is not of type byte[], is [" + value
								+ "] so using plain toString() for field " + entityName + "." + name);
						outValue = value.toString();
					}
					break;
				case 13 :
	...
	void getResultSetValue(ResultSet rs, int index, LiteStringMap<Object> valueMap, EntityFacadeImpl efi)
			throws EntityException {
		if (typeValue == -1)
			throw new EntityException("No typeValue found for " + entityName + "." + name);

		Object value = null;
		try {
			switch (typeValue) {
				case 1 :
					// getMetaData and the column type are somewhat slow (based on profiling), and
					// String values are VERY
					// common, so only do for text-very-long
					if (isTextVeryLong) {
						ResultSetMetaData rsmd = rs.getMetaData();
						if (Types.CLOB == rsmd.getColumnType(index)) {
							// if the String is empty, try to get a text input stream, this is required for
							// some databases
							...
							if (valueReader != null) {
								...
								try {
									...
									throw new EntityException("Error reading long character stream for field " + name
											+ " of entity " + entityName, e);
								}
				...
				case 3 :
					...
					break;
				// for Date don't pass 2nd param efi.getCalendarForTzLc(), causes issues when
				// Java TZ different from DB TZ
				// when the JDBC driver converts a string to a Date it uses the TZ from the
				// Calendar but we want the Java default TZ
				case 4 :
				...
				case 8 :
					...
					break;
				case 9 :
					BigDecimal bdVal = rs.getBigDecimal(index);
					if (!rs.wasNull())
						value = safeStripZeroes(bdVal);
					break;
				case 10 :
				...
				case 11 :
					...
					if (originalBytes != null && originalBytes.length <= 0) {
						logger.warn("Got byte array back empty for serialized Object with length ["
								+ originalBytes.length + "] for field [" + name + "] (" + index + ")");
					}
					if (binaryInput != null) {
						...
						try {
							...
							if (logger.isTraceEnabled())
								logger.trace("Unable to read BLOB from input stream for field [" + name + "] (" + index
										+ "): " + ex.toString());
							...
							if (inStream != null) {
								try {
									...
									logger.error("Unable to close binary input stream for field [" + name + "] ("
											+ index + "): " + e.toString(), e);
								}
							}
						}
				...
				case 12 :
					...
					try {
						// NOTE: changed to try getBytes first because Derby blows up on getBlob and on
						// then calling getBytes for the same field, complains about getting value twice
						byte[] fieldBytes = rs.getBytes(index);
						if (!rs.wasNull())
							sblob = new SerialBlob(fieldBytes);
						// fieldBytes = theBlob != null ? theBlob.getBytes(1, (int) theBlob.length()) :
						// null
					} catch (SQLException e) {
						if (logger.isTraceEnabled())
			...
			throw new EntityException("SQL Exception while getting value for field: " + name + " (" + index + ")",
					sqle);
		}
		...
		if (value != null && encrypt) {
			if (typeValue != 1)
				throw new EntityException("The encrypt attribute was set to true on non-String field " + name
			...
			try {
				...
				// NOTE DEJ 20200310 instead of using encrypted value return very clear fake
				// placeholder; this is a bad design
				// because it uses a magic value that can't change as other code may use it; an
				// alternative might be to have
				// the EntityValue internally handle it with a Set of fields that failed to
				// decrypt, but this doesn't carry
	...
	public void setPreparedStatementValue(PreparedStatement ps, int index, Object value, EntityDefinition ed,
		...
		if (value != null) {
			if (checkPreparedStatementValueType && !ObjectUtilities.isInstanceOf(value, javaType)) {
				// this is only an info level message because under normal operation for most
				// JDBC
				// drivers this will be okay, but if not then the JDBC driver will throw an
				// exception
				// and when lower debug levels are on this should help give more info on what
				// happened
				...
				if (isTraceEnabled)
					logger.trace("Type of field " + ed.getFullEntityName() + "." + name + " is " + fieldClassName
							+ ", was expecting " + javaType + " this may "
							+ "indicate an error in the configuration or in the class, and may result "
							+ "in an SQL-Java data conversion error. Will use the real field type: " + fieldClassName
							+ ", not the definition.");
			...
			if (encrypt) {
				...
				if (decryptFailedMagicString.equals(original)) {
					throw new EntityException("To prevent data loss, not allowing decrypt failed placeholder for field "
		...
		if (localTypeValue == 11 || localTypeValue == 12) {
			useBinaryTypeForBlob = ("true"
					.equals(efi.getDatabaseNode(ed.getEntityGroupName()).attribute("use-binary-type-for-blob")));
		}
		// if a count function used set as Long (type 6)
		if (ed.isViewEntity) {
			String function = fieldNode.attribute("function");
			if (function != null && function.startsWith("count"))
				localTypeValue = 6;
		}
	...
	private void setPreparedStatementValue(PreparedStatement ps, int index, Object value, int localTypeValue,
		...
		try {
			// allow setting, and searching for, String values for all types; JDBC driver
			// should handle this okay
			if (value instanceof CharSequence) {
				...
				switch (localTypeValue) {
					...
					case 2 :
						if (value != null) {
							...
							if (valClass == Timestamp.class) {
								...
								throw new EntityException("Class " + valClass.getName()
										+ " not allowed for date-time (Timestamp) fields, for field " + entityName + "."
										+ name);
							}
							// NOTE for Calendar use with different MySQL vs Oracle JDBC drivers:
							// https://bugs.openjdk.java.net/browse/JDK-4986236
							// the TimeZone is treated differently; should stay consistent but may result in
							// unexpected times when looking at
							// timestamp values directly in the database; not a huge issue, something to
							// keep in mind when configuring the DB TimeZone
							// NOTE that some JDBC drivers clone the Calendar, others don't; see the
							// efi.getCalendarForTzLc() which now uses a ThreadLocal<Calendar>
						} else {
					...
					case 3 :
						Time tm = (Time) value;
						// logger.warn("=================== setting time tm=${tm} tm
						// long=${tm.getTime()}, cal=${cal}")
						if (value != null) {
					...
					case 4 :
						if (value != null) {
							...
							if (valClass == java.sql.Date.class) {
								...
								// logger.warn("=================== setting date dt=${dt} dt
								// long=${dt.getTime()}, cal=${cal}")
								ps.setDate(index, dt);
								// NOTE: don't pass Calendar, Date was likely generated in Java TZ and that's
								// what we want, if DB TZ is different we don't want it to use that
								...
								throw new EntityException("Class " + valClass.getName()
										+ " not allowed for date fields, for field " + entityName + "." + name);
							}
						} else {
					...
					case 9 :
						if (value != null) {
							...
							if (valClass == BigDecimal.class) {
								...
								throw new EntityException("Class " + valClass.getName()
										+ " not allowed for number-decimal (BigDecimal) fields, for field " + entityName
										+ "." + name);
							}
						} else {
							ps.setNull(index, Types.NUMERIC);
						...
						if (value != null) {
							try {
								...
								throw new EntityException(
										"Error setting serialized object, for field " + entityName + "." + name, ex);
							}
						} else {
					...
					case 12 :
						if (value instanceof byte[]) {
							...
							/*
							 * } else if (value instanceof ArrayList) { ArrayList valueAl = (ArrayList)
							 * value; byte[] theBytes = new byte[valueAl.size()]; valueAl.toArray(theBytes);
							...
							// calling setBytes instead of setBlob - old github.com/moqui/moqui repo issue
							// #28 with Postgres JDBC driver
							...
							if (value != null) {
								throw new EntityException("Type not supported for BLOB field: "
										+ value.getClass().getName() + ", for field " + entityName + "." + name);
							} else {
								if (useBinaryTypeForBlob) {
									ps.setNull(index, Types.BINARY);
								} else {
			...
			throw new EntityException("SQL Exception while setting value [" + value + "]("
					+ (value != null ? value.getClass().getName() : "null") + "), type " + type + ", for field "
					+ entityName + "." + name + ": " + sqle.toString(), sqle);
		} catch (Exception e) {
			throw new EntityException(
					"Error while setting value for field " + entityName + "." + name + ": " + e.toString(), e);
		}
	}

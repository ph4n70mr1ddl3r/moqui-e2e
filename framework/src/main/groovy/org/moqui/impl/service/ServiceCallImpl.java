...
public class ServiceCallImpl implements ServiceCall {
	...
	public ServiceCallImpl(ServiceFacadeImpl sfi) {
		this.sfi = sfi;
	}

	protected void serviceNameInternal(String serviceName) {
		if (serviceName == null || serviceName.isEmpty())
			throw new ServiceException("Service name cannot be empty");
		...
		if (sd != null) {
			...
			// if the service is not found must be an entity auto, but if there is a path
			// then error
	...
	protected void serviceNameInternal(String path, String verb, String noun) {
		...
		if (noSd) {
			...
			sd = sfi.getServiceDefinition(serviceName);
			if (sd == null)
				throw new ServiceException("Service not found with name " + serviceName + " (path: " + path + ", verb: "
						+ verb + ", noun: " + noun + ")");
			serviceNameNoHash = sd.serviceNameNoHash;
		}
	}
	...
	public void validateCall(ExecutionContextImpl eci) {
		// Before scheduling the service check a few basic things so they show up sooner
		// than later:
		...
		if (sd != null) {
			...
			if (sr == null)
				throw new ServiceException("Could not find service runner for type [" + serviceType + "] for service ["
						+ getServiceName() + "]");
			// validation
		...
		// logger.warn("=========== async call ${serviceName}, parameters:
		// ${parameters}")
	}
}

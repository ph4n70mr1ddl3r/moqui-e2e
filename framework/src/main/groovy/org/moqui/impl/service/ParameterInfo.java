...
/**
 * This is a dumb data holder class for framework internal use only; in Java for
 * efficiency as it is used a LOT
 */
public class ParameterInfo {
	protected final static Logger logger = LoggerFactory.getLogger(ParameterInfo.class);

	public enum ParameterAllowHtml {
		ANY, SAFE, NONE
	}
	public enum ParameterType {
		STRING, INTEGER, LONG, FLOAT, DOUBLE, BIG_DECIMAL, BIG_INTEGER, TIME, DATE, TIMESTAMP, LIST, SET, MAP
	}
	public static Map<String, ParameterType> typeEnumByString = new HashMap<>();
	static {
		typeEnumByString.put("String", ParameterType.STRING);
		typeEnumByString.put("java.lang.String", ParameterType.STRING);
		typeEnumByString.put("Integer", ParameterType.INTEGER);
		typeEnumByString.put("java.lang.Integer", ParameterType.INTEGER);
		typeEnumByString.put("Long", ParameterType.LONG);
		typeEnumByString.put("java.lang.Long", ParameterType.LONG);
		typeEnumByString.put("Float", ParameterType.FLOAT);
		typeEnumByString.put("java.lang.Float", ParameterType.FLOAT);
		typeEnumByString.put("Double", ParameterType.DOUBLE);
		typeEnumByString.put("java.lang.Double", ParameterType.DOUBLE);
		typeEnumByString.put("BigDecimal", ParameterType.BIG_DECIMAL);
		typeEnumByString.put("java.math.BigDecimal", ParameterType.BIG_DECIMAL);
		typeEnumByString.put("BigInteger", ParameterType.BIG_INTEGER);
		typeEnumByString.put("java.math.BigInteger", ParameterType.BIG_INTEGER);

		typeEnumByString.put("Time", ParameterType.TIME);
		typeEnumByString.put("java.sql.Time", ParameterType.TIME);
		typeEnumByString.put("Date", ParameterType.DATE);
		typeEnumByString.put("java.sql.Date", ParameterType.DATE);
		typeEnumByString.put("Timestamp", ParameterType.TIMESTAMP);
		typeEnumByString.put("java.sql.Timestamp", ParameterType.TIMESTAMP);
		typeEnumByString.put("Collection", ParameterType.LIST);
		typeEnumByString.put("java.util.Collection", ParameterType.LIST);
		typeEnumByString.put("List", ParameterType.LIST);
		typeEnumByString.put("java.util.List", ParameterType.LIST);
		typeEnumByString.put("Set", ParameterType.SET);
		typeEnumByString.put("java.util.Set", ParameterType.SET);
		typeEnumByString.put("Map", ParameterType.MAP);
		typeEnumByString.put("java.util.Map", ParameterType.MAP);
	}
	...
	public ParameterInfo(ServiceDefinition sd, MNode parameterNode) {
		...
		if (defaultTmp != null && defaultTmp.isEmpty())
			defaultTmp = null;
		defaultStr = defaultTmp;
		String defaultValTmp = parameterNode.attribute("default-value");
		if (defaultValTmp != null && defaultValTmp.isEmpty())
			defaultValTmp = null;
		...
		if ("any".equals(allowHtmlStr))
			allowHtml = ParameterAllowHtml.ANY;
		else if ("safe".equals(allowHtmlStr))
			allowHtml = ParameterAllowHtml.SAFE;
		else
			allowHtml = ParameterAllowHtml.NONE;
		...
		for (MNode child : parameterNode.getChildren()) {
			if ("description".equals(child.getName()) || "parameter".equals(child.getName()))
				continue;
	...
	Object convertType(String namePrefix, Object parameterValue, boolean isString, ExecutionContextImpl eci) {
		...
		if (parmType != null && isString && !isEmptyString) {
			...
			// try some String to XYZ specific conversions for parsing with format, locale,
			// etc
			switch (parmType) {
				case INTEGER :
				case LONG :
				...
				case BIG_INTEGER :
					...
					if (bdVal == null) {
						eci.messageFacade.addValidationError(null, namePrefix + name, serviceName,
								MessageFormat.format(eci.getL10n().localize(
										"Value entered ({0}) could not be converted to a {1}{2,choice,0#|1# using format [}{3}{2,choice,0#|1#]}"),
										valueStr, type, (format != null ? 1 : 0), (format == null ? "" : format)),
								null);
					} else {
						switch (parmType) {
							case INTEGER :
								converted = bdVal.intValue();
								break;
							case LONG :
								converted = bdVal.longValue();
								break;
							case FLOAT :
								converted = bdVal.floatValue();
								break;
							case DOUBLE :
								converted = bdVal.doubleValue();
								break;
							case BIG_INTEGER :
								converted = bdVal.toBigInteger();
								break;
							default :
								converted = bdVal;
						}
		...
		if (converted == null && !isEmptyString)
			converted = ObjectUtilities.basicConvert(parameterValue, type);
	...
	public static Document.OutputSettings outputSettings = new Document.OutputSettings().charset("UTF-8")
			.prettyPrint(true).indentAmount(4);
	private String canonicalizeAndCheckHtml(ServiceDefinition sd, String namePrefix, String parameterValue,
			ExecutionContextImpl eci) {
		// NOTE DEJ20161114 Jsoup.clean() does not have a way to tell us if anything was
		// filtered, so to avoid reformatting other
		...
		for (int i = 0; i < valueLength; i++) {
			...
			/*
			 * if (curChar == '%' || curChar == '&') { indexOfEscape = i; if
			 * (indexOfLessThan >= 0) break; } else
			 */
			if (curChar == '<') {
				indexOfLessThan = i;
				break;
		...
		if (indexOfLessThan >= 0) {
			if (allowSafe) {
				...
				eci.getMessage().addValidationError(null, namePrefix + name, sd.serviceName, eci.getL10n()
						.localize("HTML not allowed including less-than (<), greater-than (>), etc symbols"), null);
			}
		}
	 ...
	 * 
	 * in framework/build.gradle: // OWASP Java HTML Sanitizer compile
	 * 'com.googlecode.owasp-java-html-sanitizer:owasp-java-html-sanitizer:20160924.
	 * 1' // New BSD & Apache 2.0
	 * 
	 * SafeHtmlChangeListener changes = new SafeHtmlChangeListener(eci, sd); String
	 * cleanHtml = EbayPolicyExample.POLICY_DEFINITION.sanitize(parameterValue,
	 * changes, namePrefix.concat(name)); List<String> cleanChanges =
	 * changes.getMessages(); // use message instead of error, accept cleaned up
	 * HTML if (cleanChanges.size() > 0) { for (String cleanChange: cleanChanges)
	 * eci.getMessage().addMessage(cleanChange);
	 * logger.info("Service parameter safe HTML messages for " + sd.serviceName +
	 * "." + name + ": " + cleanChanges); return cleanHtml; } else { // nothing
	 * changed, return null return null; }
	 * 
	 * private static class SafeHtmlChangeListener implements
	 * HtmlChangeListener<String> { private ExecutionContextImpl eci; private
	 * ServiceDefinition sd; private List<String> messages = new LinkedList<>();
	 * SafeHtmlChangeListener(ExecutionContextImpl eci, ServiceDefinition sd) {
	 * this.eci = eci; this.sd = sd; } List<String> getMessages() { return messages;
	 * }
	 * 
	 * @SuppressWarnings("NullableProblems")
	 * 
	 * @Override public void discardedTag(@Nullable String context, String
	 * elementName) { messages.add(MessageFormat.format(eci.getL10n().
	 * localize("Removed HTML element {0} from field {1} in service {2}"),
	 * elementName, context, sd.serviceName)); }
	 * 
	 * @SuppressWarnings("NullableProblems")
	 * 
	 * @Override public void discardedAttributes(@Nullable String context, String
	 * tagName, String... attributeNames) { for (String attrName: attributeNames)
	 * messages.add(MessageFormat.format(eci.getL10n().
	 * localize("Removed attribute {0} from HTML element {1} from field {2} in service {3}"

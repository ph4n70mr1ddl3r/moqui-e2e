...
public class ServiceDefinition {
	...
	public ServiceDefinition(ServiceFacadeImpl sfi, String path, MNode sn) {
		...
		if (authzActionAttr != null && !authzActionAttr.isEmpty())
			tempAction = ArtifactExecutionInfo.authzActionByName.get(authzActionAttr);
		if (tempAction == null)
			tempAction = verbAuthzActionEnumMap.get(verb);
		if (tempAction == null)
			tempAction = ArtifactExecutionInfo.AUTHZA_ALL;
		...
		if (serviceNode.hasChild("implements"))
			for (MNode implementsNode : serviceNode.children("implements")) {
				...
				if (implRequired != null && implRequired.isEmpty())
					implRequired = null;
				ServiceDefinition sd = sfi.getServiceDefinition(implServiceName);
				if (sd == null)
					throw new ServiceException("Service " + implServiceName
							+ " not found, specified in service.implements in service " + serviceName);

				// while most attributes aren't passed through do pass through
				// no-remember-parameters if true
				if (sd.noRememberParameters)
					noRememberParmsTemp = true;
				...
				if (implInParms != null && implInParms.hasChild("parameter")) {
					for (MNode parameter : implInParms.children("parameter")) {
						...
						if (implRequired != null)
							newParameter.getAttributes().put("required", implRequired);
		...
		// expand auto-parameters and merge parameter in in-parameters and
		// out-parameters
		// if noun is a valid entity name set it on parameters with valid field names on
		// it
		...
		if (serviceNode.hasChild("in-parameters"))
			serviceNode.remove("in-parameters");
		serviceNode.append(inParameters);
		if (serviceNode.hasChild("out-parameters"))
			serviceNode.remove("out-parameters");
		...
		if (inParametersNode != null)
			for (MNode parameter : inParametersNode.children("parameter")) {
		...
		if (outParametersNode != null)
			for (MNode parameter : outParametersNode.children("parameter")) {
				String parameterName = parameter.attribute("name");
	...
	private void mergeAutoParameters(MNode parametersNode, MNode autoParameters) {
		...
		if (entityName == null || entityName.isEmpty())
			entityName = noun;
		if (entityName == null || entityName.isEmpty())
			throw new ServiceException("Error in auto-parameters in service " + serviceName
					+ ", no auto-parameters.@entity-name and no service.@noun for a default");
		EntityDefinition ed = sfi.ecfi.entityFacade.getEntityDefinition(entityName);
		if (ed == null)
			throw new ServiceException("Error in auto-parameters in service " + serviceName
					+ ", the entity-name or noun [" + entityName + "] is not a valid entity name");
		...
		if (allowHtmlStr == null || allowHtmlStr.isEmpty())
			allowHtmlStr = "none";
		for (String fieldName : ed.getFieldNames("all".equals(includeStr) || "pk".equals(includeStr),
				"all".equals(includeStr) || "nonpk".equals(includeStr))) {
	...
	private void mergeParameter(MNode parametersNode, MNode overrideParameterNode, EntityDefinition ed) {
		MNode baseParameterNode = mergeParameter(parametersNode, overrideParameterNode.attribute("name"),
				overrideParameterNode.getAttributes());
		...
		for (MNode childNode : overrideParameterNode.getChildren()) {
			if ("description".equals(childNode.getName())) {
				if (baseParameterNode.hasChild(childNode.getName()))
					baseParameterNode.remove(childNode.getName());
			}
			...
			if ("auto-parameters".equals(childNode.getName())) {
				...
				// is a validation, just add it in, or the original has been removed so add the
				// new one
	...
	private static MNode mergeParameter(MNode parametersNode, final String parameterName,
	...
	public static String makeServiceName(String path, String verb, String noun) {
		return (path != null && !path.isEmpty() ? path + "." : "") + verb
				+ (noun != null && !noun.isEmpty() ? "#" + noun : "");
	}
	...
	public static ArtifactExecutionInfo.AuthzAction getVerbAuthzActionEnum(String theVerb) {
		// default to require the "All" authz action, and for special verbs default to
	...
	public Map<String, Object> convertValidateCleanParameters(Map<String, Object> parameters,
			ExecutionContextImpl eci) {
		// logger.warn("BEFORE ${serviceName} convertValidateCleanParameters:
		// ${parameters.toString()}")
		...
		// logger.warn("AFTER ${serviceName} convertValidateCleanParameters:
		// ${parameters.toString()}")
	}
	...
	private Map<String, Object> nestedParameterClean(String namePrefix, Map<String, Object> parameters,
		...
		// a copy of the parameters Map to retain unknown entries to add at the end when
		// NOT validating (pass through unknown parameters)
		...
		for (int i = 0; i < parameterInfoArray.length; i++) {
			...
			if (hasParameter && parametersCopy != null)
				parametersCopy.remove(parameterName);
			...
			if (parameterIsEmpty) {
				if (parameterInfo.hasDefault) {
					...
					if (parameterInfo.defaultStr != null) {
						...
						if (parameterInfo.defaultValueNeedsExpand) {
							Map<String, Object> combinedMap = new HashMap<>(parameters);
							combinedMap.putAll(newMap);
							stringValue = eci.resourceFacade.expand(parameterInfo.defaultValue, null, combinedMap,
									false);
						} else {
					...
					// if empty but not null and types don't match set to null instead of trying to
					// convert
					if (parameterValue != null) {
						boolean typeMatches;
						if (parameterInfo.parmClass != null) {
							typeMatches = parameterClass == parameterInfo.parmClass
									|| parameterInfo.parmClass.isInstance(parameterValue);
						} else {
				...
				if (parameterIsEmpty && validate && parameterInfo.required)
					eci.messageFacade.addValidationError(null, namePrefix + parameterName, serviceName,
							eci.getL10n().localize("Field cannot be empty"), null);
			}
			...
			if (!parameterIsEmpty) {
				...
				if (!typeMatches) {
					// convert type, at this point parameterValue is not empty and doesn't match
					// parameter type
				...
				if (validate) {
					if ((isString || isCollection) && ParameterInfo.ParameterAllowHtml.ANY != parameterInfo.allowHtml) {
						Object htmlValidated = parameterInfo.validateParameterHtml(namePrefix, parameterValue, isString,
								eci);
					...
					// check against validation sub-elements (do this after the convert so we can
					// deal with objects when needed)
					if (parameterInfo.validationNodeList != null) {
						...
						for (int valIdx = 0; valIdx < valListSize; valIdx++) {
							...
							// NOTE don't break on fail, we want to get a list of all failures for the user
							// to see
							try {
								// validateParameterSingle calls eci.message.addValidationError as needed so
								// nothing else to do here
								validateParameterSingle(valNode, parameterName, parameterValue, eci);
							} catch (Throwable t) {
								logger.error("Error in validation", t);
								Map<String, Object> map = new HashMap<>(3);
								map.put("parameterValue", parameterValue);
								map.put("valNode", valNode);
								map.put("t", t);
								eci.getMessage().addValidationError(null, parameterName, serviceName,
										eci.getResource().expand(
												"Value entered failed ${valNode.name} validation: ${t.message}", "",
												map),
										null);
							}
						}
				...
				if (isMap && parameterInfo.childParameterInfoArray != null
						&& parameterInfo.childParameterInfoArray.length > 0) {
		...
		// if we are not validating and there are parameters remaining, add them to the
		// newMap
		...
		if ("val-or".equals(validateName)) {
			...
			if (!(pv instanceof CharSequence)) {
				...
				eci.getMessage().addValidationError(null, parameterName, serviceName, eci.getResource()
						.expand("Value entered (${pv}) is not a string, cannot do matches validation.", "", map), null);
				return false;
			}
			...
			if (regexp != null && !regexp.isEmpty() && !pvString.matches(regexp)) {
				// a message attribute should always be there, but just in case we'll have a
				// default
				...
				eci.getMessage().addValidationError(null, parameterName, serviceName,
						eci.getResource()
								.expand(message != null && !message.isEmpty()
										? message
				...
				if ("false".equals(valNode.attribute("min-include-equals"))) {
					if (bdVal.compareTo(min) <= 0) {
						...
						if (message == null || message.isEmpty())
							message = "Value entered (${pv}) is less than or equal to ${min}, must be greater than.";
						eci.getMessage().addValidationError(null, parameterName, serviceName,
					...
					if (bdVal.compareTo(min) < 0) {
						...
						map.put("min", min);
						if (message == null || message.isEmpty())
							message = "Value entered (${pv}) is less than ${min} and must be greater than or equal to.";
						eci.getMessage().addValidationError(null, parameterName, serviceName,
								eci.getResource().expand(message, "", map), null);
						return false;
					}
			...
			if (maxStr != null && !maxStr.isEmpty()) {
				...
				if ("true".equals(valNode.attribute("max-include-equals"))) {
					if (bdVal.compareTo(max) > 0) {
						...
						map.put("max", max);
						if (message == null || message.isEmpty())
							message = "Value entered (${pv}) is greater than ${max} and must be less than or equal to.";
						eci.getMessage().addValidationError(null, parameterName, serviceName,
								eci.getResource().expand(message, "", map), null);
						return false;
					}
					...
					if (bdVal.compareTo(max) >= 0) {
						...
						map.put("max", max);
						if (message == null || message.isEmpty())
							message = "Value entered (${pv}) is greater than or equal to ${max} and must be less than.";
						eci.getMessage().addValidationError(null, parameterName, serviceName,
								eci.getResource().expand(message, "", map), null);
						return false;
					}
			...
			for (char c : str.toCharArray()) {
				if (!Character.isLetter(c)) {
					...
					map.put("str", str);
					eci.getMessage().addValidationError(null, parameterName, serviceName,
							eci.getResource().expand("Value entered (${str}) must have only letters.", "", map), null);
					return false;
				}
			}
			...
			if (after != null && !after.isEmpty()) {
				// handle after date/time/date-time depending on type of parameter, support
				// "now" too
				Calendar compareCal;
				if ("now".equals(after)) {
					compareCal = eci.getL10n()
							.parseDateTime(eci.getL10n().format(eci.getUser().getNowTimestamp(), format), format);
				} else {
			...
			if (!ccv.isValid(str)) {
				...
				map.put("str", str);
				eci.getMessage().addValidationError(null, parameterName, serviceName,
						eci.getResource().expand("Value entered is not a valid credit card number.", "", map), null);
				return false;
			}
